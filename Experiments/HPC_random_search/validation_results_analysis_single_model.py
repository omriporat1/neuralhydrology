...
def main():
    ...
    job_num = "0" # This specifies which job to analyze, can be set to None for all jobs
    run_num = None  # Set to None to analyze all runs, or specify a job number like "001"
    if job_num:
        job_dir = results_dir / f"job_{job_num}"
        if run_num:
            base_run_dir = job_dir / f"run_{run_num}*"
            run_dirs = [d for d in base_run_dir.iterdir() if d.is_dir()]
        else:
            run_dirs = []
            for run_folder in job_dir.glob("run_*_a"):
                run_dirs.extend([d for d in run_folder.iterdir() if d.is_dir()])
    else:
        run_dirs = []
        for job_folder in results_dir.glob("job_*"):
            for run_folder in job_folder.glob("run_*_a"):
                run_dirs.extend([d for d in run_folder.iterdir() if d.is_dir()])
