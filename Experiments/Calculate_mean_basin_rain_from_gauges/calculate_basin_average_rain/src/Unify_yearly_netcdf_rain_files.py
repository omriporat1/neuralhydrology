...
def unify_yearly_netcdf_files(output_dir, merged_filename="rain_grid_full.nc"):
    """
    ...
    existing_files = [f for f in nc_files if os.path.isfile(f)]
    missing_files = [f for f in nc_files if not os.path.isfile(f)]
    if missing_files:
