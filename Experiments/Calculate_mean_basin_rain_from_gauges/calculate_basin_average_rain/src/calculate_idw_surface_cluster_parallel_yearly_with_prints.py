...
def idw_interpolation_grid(gauges_data, grid_edges, power=2, max_radius=50000, output_dir="output", date_range=None, grid_resolution=1000):
    ...
    # For global progress, allow passing global_chunk_idx/global_total_chunks (default: per year only)
    global_chunk_offset = 0
    global_total_chunks = total_chunks
    if 'global_chunk_offset' in locals():
    ...
    for chunk_idx, idxs in enumerate(chunk_indices):
        ...
        # If global_chunk_offset/global_total_chunks are set, use them; else, per year only
...
def main():
    ...
    for year in years:
        ...
        mask = (gauges_data['datetime'] >= year_start) & (gauges_data['datetime'] <= year_end)
        gauges_data_year = gauges_data[mask]
        if gauges_data_year.empty:
            all_chunk_counts.append(0)
            year_chunk_indices.append([])
            continue
        times = np.sort(gauges_data_year['datetime'].unique())
        times_pd = pd.to_datetime(times)
        chunk_keys = times_pd.normalize()
        unique_days = np.unique(chunk_keys)
        chunk_indices = [np.where(chunk_keys == day)[0] for day in unique_days]
    ...
    for i, year in enumerate(years):
        ...
        def idw_interpolation_grid_with_global(gauges_data, grid_edges, power=2, max_radius=50000, output_dir="output", date_range=None, grid_resolution=1000, year_idx=None, total_years=None, year=None):
            ...
            logging.info(f"Saved interpolated grid and gauge counts to {nc_path}")
            return ds
        # Use the patched function for this year
