...
def main():
    ...
    print(f"Loaded {len(gauges)} station metadata rows.", flush=True)
    basins = gpd.read_file('/sci/labs/efratmorin/omripo/PhD/Data/Caravan/shapefiles/il/il_basin_shapes.shp')
    print(f"Loaded {len(basins)} basin polygons.", flush=True)
    ...
    print(f"Found {len(all_files)} gauge data files.", flush=True)

    # Combine all files into a single DataFrame
    gauges_data = pd.concat((pd.read_csv(f) for f in all_files), ignore_index=True)
    print(f"Loaded {len(gauges_data)} rows of gauge data.", flush=True)
    ...
    print(f"After merge, gauge data has {gauges_data.shape[0]} rows and columns: {list(gauges_data.columns)}", flush=True)

    # Process the combined data
    gauges_data['datetime'] = pd.to_datetime(gauges_data['datetime'], dayfirst=True)
    print(f"Converted 'datetime' column to pandas datetime. Unique timesteps: {gauges_data['datetime'].nunique()}", flush=True)
    ...
    if gauges_data['datetime'].dt.tz is not None:
        ...
        print("Removed timezone info from datetimes.", flush=True)
