...
def idw_interpolation_grid(gauges_data, grid_edges, power=2, max_radius=50000, output_dir="output", date_range=None, grid_resolution=1000):
    ...
    def interpolate_single_timestep(args):
        i, t = args
        frame = gauges_data[gauges_data['datetime'] == t]
        ...
        if np.all(np.isnan(result)):
            ...
            print(f"Grid min: {np.nanmin(result)}, max: {np.nanmax(result)}")
        return result

    with concurrent.futures.ProcessPoolExecutor() as executor:
        results = list(executor.map(interpolate_single_timestep, [(i, t) for i, t in enumerate(times)]))
