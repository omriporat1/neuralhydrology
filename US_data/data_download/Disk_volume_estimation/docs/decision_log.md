...
## 2026-09-17 — RD1-C4-F hydrograph legend-clipping repair + performance-stratified selection family PRODUCED (Moriah job `46185914`) — C4-F evidence-rendering repair/expansion only; no classifier/winner/promotion/tolerance decision; does not close RD1-C4

**[DESCRIPTIVE HYDROGRAPH RENDERING REPAIR + REPRESENTATIVE-EXAMPLE EXPANSION — C4-F EVIDENCE-RENDERING ONLY; NOT TRAINING/INFERENCE/RESCORING/OBJECTIVE-ALTERING; NOT W&B/CONTROLLER CONTACT; NOT SEALED-SCOPE CONSUMPTION; NOT A NEW EVALUATION PRODUCT; NOT A SCIENTIFIC CLASSIFIER, WINNER, PROMOTION, OR TOLERANCE DECISION; NOT A C4/C5/C6 CLOSURE]** This entry supersedes the intermediate legend-clipping evidence from job `46185815` (below, historical only, do not cite as final).

**Legend-clipping defect found and fixed (self-diagnosed via mandatory visual review).** Direct inspection of all 6 figures from job `46185815` (the prior arm-difference-family repair plus the first performance-stratified render) found the fig-level legend clipped at both the left and right canvas edges for long candidate labels. Root cause: `fig.tight_layout()` only reserves layout space for `Axes` objects, not for a `fig.legend()` (figure-level) artist, and `render_multi_candidate_basin_panel`'s `fig.savefig(out_path, dpi=130)` call passed no `bbox_inches` argument — so a legend wider than the fixed 10in figsize was silently truncated at save time rather than included. Fix in `src/baseline/hydrograph_rendering.py`: (1) legend column cap lowered from `ncol=4` to `ncol=min(max(len(labels),1),3)`, since realistic labels (e.g. `random_control_proposal009 (random_control@9, random_control@12)`) do not fit 4 side-by-side columns; (2) `bbox_inches="tight"` added to the `fig.savefig(...)` call, which makes matplotlib compute the true rendered extent of every artist including the legend and expand the saved canvas accordingly, independent of label length. A new regression test, `test_render_multi_candidate_basin_panel_saves_with_tight_bbox_so_wide_legend_is_not_clipped`, was added to `tests/test_hydrograph_rendering.py`, asserting `bbox_inches="tight"` is passed to `Figure.savefig` via a monkeypatched spy, using realistic long production-style legend labels and 6 candidates. Full `tests/test_hydrograph_rendering.py` suite: 76/76 passed; `tests/test_rd1_c4_f_hydrograph_runner.py`: 6/6 passed. Committed at `4431027c2ac77cd1e97454bfd235a96e06f5cdbc` (2 files changed: `src/baseline/hydrograph_rendering.py`, `tests/test_hydrograph_rendering.py`; verified via `git status --porcelain`/`git diff --stat` that no other tracked file was touched), pushed to `origin/master` (`c114316..4431027`), and fast-forwarded into the isolated Moriah clone `flash-nh-rd1c4-d1` (`c114316` → `4431027`, confirmed via `HEAD_AFTER_PULL`).

**Rerun (Moriah job `46185914`).** Same 5 real, hash-verified inputs as job `46185815` (trial roster sha256 `01569743a7fe69f180f2ca4a2be2f518e45ed70b2b2822642894b8e22cc52e4e`; fixed-support contract sha256 `99fdc4f44768779661a735d25974e25b64a5d7f502ec8ffa4c5d2bab42bc34d6`; package root `stage1_scientific_package_v002`; selection manifest sha256 `abc80b5e307898f7b3c0b4398f3ed272b46389e054259cfa7c5cda38deb827b2`; configuration-level table sha256 `054b5e7d7ab337877fccddb9ebd600b21a47345c6cb7321d50e953fdde601884`; performance metrics sha256 `277d0998c9921ec7d18b6441aef25a7a9ec1fd2d3e2ad07499c6a9ef0f086e18`), fresh output `rd1_c4_f_hydrograph_legendfix_20260917T125816Z`. `sacct -j 46185914`: `COMPLETED`, `ExitCode=0:0`, `Elapsed=00:00:57`, `MaxRSS=553016K`. In-job commit gate confirmed `ACTUAL_COMMIT == EXPECTED_COMMIT == 4431027c2ac77cd1e97454bfd235a96e06f5cdbc`; all 5 input hashes matched exactly; `{"n_figures": 6}` reported.

**Two additive, non-overlapping selection families now rendered.** Arm-difference family (percentile of per-basin `bayesian_median_nse − random_control_median_nse`, from job `46185509`'s frozen rule): p10=`06600100` (−0.0556), p50=`01464000` (0.0127), p90=`02303205` (0.0818). New performance-stratified family (percentile of per-basin median NSE across all 24 authenticated configurations combined, both arms together): low/p10=`12086500` (−0.1551), typical/p50=`14199704` (0.4057), high/p90=`03151400` (0.6495). All 6 basins are distinct — zero overlap observed between the two families in this real 400-basin population; no deduplication was triggered. Both families reuse the same deterministic machinery: 72-hour observed-peak-centered window (earliest-tie rule, confirmed `actual_window_hours=72.0`, unclipped both sides for all 6 basins), the same 6 unique best-so-far incumbent trials across `{bayesian, random_control} x proposal_order {1,3,6,9,12}`, shared per-basin axis scale across candidates, and the same MRMS QPE precipitation panel (`mrms_qpe_1h_mm`, mm h⁻¹) aligned to hydrograph timestamps.

**Evidence bundle (hash-verified, 0 mismatches).** Retrieved into `.scratch_local/rd1_c4_f_hydrograph_legendfix_46185914_evidence/` via `scp -O`: `manifest.json` (sha256 `2a73c369bf04e5b41e238579a23c2ce0ce7094d0b414dbb08ca463837dd441c7`), 6 figures (`hydrograph_arm_diff_p10_06600100.png` sha256 `304fe893740fe9028c3ea56ac57280345a657605f8484f31b25fe64474400029`; `hydrograph_arm_diff_p50_01464000.png` sha256 `8eaca4fa9ed093822b1430984ce837060f86b99594693ee5a305d40e5b485534`; `hydrograph_arm_diff_p90_02303205.png` sha256 `92f70ee44843b135ceac2a71751ac7762c21a76134bffed7dcdc27aca0db88b6`; `hydrograph_low_performance_12086500.png` sha256 `6512ca4a173b4dd113d28d936d6398aaa7c0ef64c249c78523b556b0bcaf5352`; `hydrograph_typical_performance_14199704.png` sha256 `de022ef27132b0dba7dab08ac7df76a1663d053a4085d0c492ae4341bebc72e4`; `hydrograph_high_performance_03151400.png` sha256 `e37a4688fe3951b5afb3c44bc9b3133c049baeec9559bf3794abc95d58ce306c`), job log, and `sacct` accounting. Every file's local SHA-256 matches the remote `sha256sum` output exactly. All 6 figures visually re-inspected via the Read tool per this task's mandatory pre-documentation review step: legend fully within canvas bounds on all 6 (no clipping), never overlapping plotted data, arm-aware color palettes consistent (bayesian in blue shades, random_control in orange/red shades), observed series the dominant neutral trace throughout, precipitation panel present and correctly labeled on all 6. A compact selection table (`selection_table.csv`, 6 rows: basin_id, family, label, percentile, value, peak time/value, window bounds, clipping flags, forcing source, figure path/hash) and an overview document (`selection_families_overview.md`, explaining both selection families, their independence, and the confirmed zero overlap) were built from the verified manifest.

**Descriptive observation (interpretation only, not a scientific decision).** Peak discharge magnitudes across the 6 example basins span roughly 3 orders of magnitude (`14199704` peak ≈0.27 m³/s to `06600100` peak ≈1161 m³/s), consistent with the fixed-support contract's known basin-scale heterogeneity; no candidate or search arm visually stands out as a systematic winner across the low/typical/high-performance examples. This is descriptive/interpretive only and is not a classifier, ranking, or promotion signal.

**Scope.** This entry records a rendering-defect repair and an additive representative-example expansion, both descriptive run-progress evidence only. No training, inference, rescoring, objective change, W&B/controller contact, or sealed-scope (SHARED-A5, temporal test, spatial holdout, California) access occurred. No classifier, winner, promotion, or tolerance decision was made. RD1-C4 is not closed by this entry, nor is C5/C6 started.

## 2026-09-17 — RD1-C4-F descriptive hydrograph supplement PRODUCED (Moriah job `46185509`, prior local basin-ID identifier-representation gate repaired first) — C4-F evidence-rendering only; no classifier/winner/promotion/tolerance decision; does not close RD1-C4

**[DESCRIPTIVE HYDROGRAPH SUPPLEMENT — C4-F EVIDENCE-RENDERING ONLY; NOT TRAINING/INFERENCE/RESCORING/OBJECTIVE-ALTERING; NOT W&B/CONTROLLER CONTACT; NOT SEALED-SCOPE CONSUMPTION; NOT A NEW EVALUATION PRODUCT; NOT A SCIENTIFIC CLASSIFIER, WINNER, PROMOTION, OR TOLERANCE DECISION; NOT A C4/C5/C6 CLOSURE]** Before any hydrograph work, a mandatory local gate was completed and verified: a basin-ID identifier-representation defect (pandas default dtype inference silently stripping leading zeros from an untyped 8-digit USGS basin-ID column) was corrected as an identifier-representation/provenance repair only — no change to selected records, percentiles, metrics, or the frozen selection rule.

**Hydrograph extraction implemented.** New module `src/baseline/rd1_c4_f_hydrograph_runner.py` adds the production CLI entry point (`produce_rd1_c4_f_hydrograph_supplement`, `build_parser`, `main`) on top of the prior session's extraction/rendering wiring. It reuses, never re-implements, the qualified `evaluate_fixed_support_raw_space_metrics`/`derive_canonical_package_observed_series`/`qualify_package_identity`/`authenticate_trial_roster` paths, plus `build_incumbent_order_mapping`/`validate_selected_basins_canonical` from `stage1_rd1_c4_f_hydrograph_supplement.py`. For each canonically selected basin (p10=`06600100`, p50=`01464000`, p90=`02303205`, from the frozen `hydrograph_basin_selection_manifest.json` percentile-of-difference rule, ties broken lexicographically) it renders one same-panel, multi-candidate overlay figure via the reused `hydrograph_rendering.render_multi_candidate_basin_panel` primitive, using one frozen ±36h peak-centered window (clipped, not shifted, to series bounds) and one shared axis scale across candidates. Candidates are the 6 unique best-so-far incumbent trial IDs across `{bayesian, random_control} x proposal_order {1,3,6,9,12}`, resolved against the same real 24-trial roster and configuration-level table used for the RD1-C4-E formal run. New Slurm entry point `scripts/rd1_c4_f_hydrograph/rd1_c4_f_hydrograph.sbatch` (modeled on `scripts/rd1_c4_results/rd1_c4_results.sbatch`): refuses missing inputs, non-directory package root, sealed-scope path substrings, and a pre-existing non-empty output directory; echoes git state of both the isolated worktree clone and the canonical `flash-nh` clone to prove the canonical repo stays untouched.

**Legend/label rendering defect found and fixed (self-diagnosed via mandatory visual review, no user involvement).** The first successful run (Moriah job `46185466`, exit 0) produced 3 real figures, but `candidate_labels_from_manifest` used the full raw `trial_id` string (~150 characters) as legend text, causing every legend box and the panel title to overflow far outside the canvas bounds, clipped and unreadable — confirmed by direct visual inspection of all 3 PNGs via the Read tool, per this task's mandatory pre-documentation review step. Root cause fixed by adding `_short_trial_label`/`_short_order_key` helpers producing compact `arm_proposalNNN (arm@order, ...)` legend labels (e.g. `bayesian_proposal001 (bayesian@1)`); the full `trial_id` remains authoritative in `manifest.json`. `tests/test_rd1_c4_f_hydrograph_runner.py` updated to match (5/5 passing). Fix re-synced to the Moriah clone and hash-verified.

**Real inputs used (identity-confirmed, not guessed).** Same trial roster as RD1-C4-E (sha256 `01569743a7fe69f180f2ca4a2be2f518e45ed70b2b2822642894b8e22cc52e4e`), same fixed-support contract (sha256 `99fdc4f44768779661a735d25974e25b64a5d7f502ec8ffa4c5d2bab42bc34d6`), same package root `stage1_scientific_package_v002`; real selection manifest (sha256 `abc80b5e307898f7b3c0b4398f3ed272b46389e054259cfa7c5cda38deb827b2`) and configuration-level table (sha256 `054b5e7d7ab337877fccddb9ebd600b21a47345c6cb7321d50e953fdde601884`) staged onto Moriah and hash-verified against their local sources at `.scratch_local/rd1_c4_f_review_packet_v001/tables/`.

**Fixed re-run (Moriah job `46185509`).** `COMPLETED`, `ExitCode=0:0`, `Elapsed=00:00:35`, fresh output `rd1_c4_f_hydrograph_20260917T120950Z`, canonical `flash-nh` clone confirmed untouched by the in-log git-state echo. `manifest.json`: `n_lookups=10`, `n_unique_trials=6`, full `incumbent_order_manifest`/`peak_windows_by_percentile`/`figures_by_percentile` populated; peak windows all `actual_window_hours=72.0`, unclipped both sides for all 3 basins.

**Evidence bundle (hash-verified).** Retrieved into `.scratch_local/rd1_c4_f_hydrograph_46185509_evidence/` via `scp -O`; every file's local SHA-256 matches the remote `sha256sum` and the manifest exactly (0 mismatches): `manifest.json`, `figures/hydrograph_p10_06600100.png`, `figures/hydrograph_p50_01464000.png`, `figures/hydrograph_p90_02303205.png`, plus the job log. All 3 figures visually re-inspected via the Read tool and confirmed legible (legend/title fully within canvas bounds; p90's legend box overlaps some trace lines near the peak but every label remains readable — not the prior off-canvas clipping defect).

**Descriptive observation (interpretation only, not a scientific decision).** In all 3 example basins, observed peak discharge substantially exceeds every one of the 6 candidates' reconstructed peaks during the frozen ±36h window, consistent with the previously documented shared high-flow/Q98 weakness across both search arms (see the RD1-C4-E `fig3_q98_diagnostics_by_arm.png` entry above). No candidate or search arm visually stands out as a systematic winner across these 3 examples; this is descriptive/interpretive only and is not a classifier, ranking, or promotion signal.

**Scope.** This entry records descriptive run-progress hydrograph evidence only. No training, inference, rescoring, objective change, W&B/controller contact, or sealed-scope (SHARED-A5, temporal test, spatial holdout, California) access occurred. No classifier, winner, promotion, or tolerance decision was made. RD1-C4 is not closed by this entry, nor is C5/C6 started.

## 2026-09-17 — RD1-C4-E provenance-envelope exceedance made report-only (not fatal) at `11e79faead3aaf3ab7abce8f07feeeece388ae32`; formal 24-trial results-production COMPLETED (Moriah job `46183269`) producing the frozen full-support NSE/KGE/Q98 evidence bundle — final audit policy, scientific classification, and C4 closure remain OPEN and unmade

**[FORMAL RESULTS BUNDLE PRODUCED — EVIDENCE-ONLY OPERATIONAL/MEASUREMENT RESULT; NOT A SCIENTIFIC CLASSIFIER, TOLERANCE, WINNER, PROMOTION, OR AUDIT-POLICY DECISION; NOT A C4 OR C5/C6 CLOSURE; NO SEALED-SCOPE, TRAINING, W&B, OR CONTROLLER CONTACT]** This entry reconciles the binding policy implied by the frozen RD1-C4-D1 evidence (above): the PROVISIONAL provenance-envelope exceedance found there (`PROVENANCE_RTOL`/`PROVENANCE_ATOL_M3S`, explicitly report-only per that entry) was, until now, still wired as a **fatal** check inside `_audit_admitted_observation_provenance()` in the formal `assemble_rd1_hydrological_review()` consumer — so the first attempt at the formal 24-trial results-production run (job `46178103`, prior HEAD `7925e00a27d07018f0e2de6bc52c42397788b596`) aborted after 1:51:26 wall clock with `HydrologicalConsumerError` at basin `06911000`, Bayesian proposal002 (5/8,436 elements exceeding, max abs diff `7.58034e-06 m^3/s`), confirmed by direct inspection of hash-verified `.scratch_local/rd1_c4_job46178103_evidence/` (stdout/log/`sacct`, `sacct -j 46178103` → `FAILED`, `ExitCode=3:0`).

**Reconciliation implemented.** `_audit_admitted_observation_provenance()` now returns a structured `ProvenanceAuditRecord` (`n_compared`, `n_exceeding`, `envelope_exceeded`, `max_abs_diff_m3s`, `worst_position`, `worst_package_value_m3s`, `worst_admitted_value_m3s`) instead of raising on an envelope exceedance; it still raises `HydrologicalConsumerError` (fail-closed, unchanged) on a timestamp/date-array mismatch or an observed-array shape mismatch — both remain genuine hard identity contradictions. `HydrologicalReviewResult` gained an immutable `provenance_audit_by_trial_id` field (one record per trial×basin cell, covering every compared cell, not only exceedances). `rd1_c4_results_runner.write_rd1_c4_results()` now serializes a flat `provenance_audit.csv` (one row per cell) plus an aggregate `provenance_audit_summary` block in `review_identity.json`, included in the output manifest. No change was made to the canonical package-derived Q98 facts, the official objective, or the NSE/KGE/Q98 computation paths; no new threshold, classifier, tolerance, or exclusion rule was added. Focused tests added/updated in `tests/test_stage1_v2_12plus12_hydrological_consumer.py` and `tests/test_rd1_c4_results_runner.py` prove: envelope exceedance is retained with exact diagnostic metadata without aborting formal assembly; timestamp and shape mismatches still fail closed; canonical Q98 facts are unaffected by an exceedance; the results writer serializes complete per-cell rows and correct aggregate totals in the manifest. Full focused suite: 85/85 passed. Committed at `11e79faead3aaf3ab7abce8f07feeeece388ae32` (4 files changed: the consumer, the results runner, and their two test modules only), pushed to `origin/master`, and pulled fast-forward into the isolated Moriah clone `flash-nh-rd1c4-d1` (`7925e00` → `11e79fa`, clean pre-pull state).

**Formal rerun (Moriah job `46183269`).** Same frozen 24-trial roster (`rd1_c4_d1_trial_roster_benchmark_only.json`, sha256 `01569743a7fe69f180f2ca4a2be2f518e45ed70b2b2822642894b8e22cc52e4e`), same fixed-support contract (`stage1_v2_common120_fixed_support_contract_v001.json`, sha256 `99fdc4f44768779661a735d25974e25b64a5d7f502ec8ffa4c5d2bab42bc34d6`), and same qualified package root (`stage1_scientific_package_v002`) as job `46178103`, into a fresh output directory `results_11e79fa_20260917T074711Z`. `sacct -j 46183269`: `COMPLETED`, `ExitCode=0:0`, `Elapsed=02:01:55`, `MaxRSS=2,574,444K` — the envelope exceedance recurred (as expected, it is a real numerical fact) but no longer aborted the run.

**Evidence bundle (hash-verified).** Retrieved into `.scratch_local/rd1_c4_e_rerun_46183269_evidence/` via `scp -O`; every file's local SHA-256 matches both the bundle's own `manifest.json` and an independent remote `sha256sum` re-computation (0 mismatches): `per_basin_metrics.csv`, `q98_diagnostics.csv`, `basin_distribution_summary.csv`, `canonical_q98_facts.csv`, `provenance_audit.csv`, `review_identity.json`, plus the production log/stdout/stderr and `sacct` accounting. `review_identity.json`: `n_configurations=24` (`n_bayesian=12`, `n_random_control=12`), `n_basins=400`, full metric coverage (`n_finite=9,600/9,600`, `n_unavailable=0` for every one of `nse`/`kge`/`kge_r`/`kge_alpha`/`kge_beta`/`q98_normalized_rmse`/`relative_volume_bias`/`observed_peak_time_magnitude_error`/`high_flow_nse`/`high_flow_sample_count`). `provenance_audit_summary`: `n_cells=9,600`, `n_cells_with_exceedance=152`, `n_elements_compared=80,349,432`, `n_elements_exceeding=11,352` (`provisional_envelope_is_report_only: true`), `worst_cell` at basin `02137727` (max abs diff `1.18463e-04 m^3/s`) — the element/comparison totals match the frozen RD1-C4-D1 reduction (`46176640`) exactly, confirming consistency between the D1 diagnostic and this formal C4 run. By search arm: bayesian `79/4,800` cells exceeding (`4,767/40,174,716` elements, `0.0119%`), random_control `73/4,800` cells exceeding (`6,585/40,174,716` elements, `0.0164%`).

**Figures (evidence-backed, untracked, C4-scope only).** Generated from the hash-verified bundle into `.scratch_local/rd1_c4_e_rerun_46183269_evidence/figures/`: (1) `fig1_nse_ecdf_by_arm.png` — per-basin×trial NSE ECDF, bayesian vs. random_control (n=4,800 each); (2) `fig2_kge_ecdf_by_arm.png` — same for KGE; (3) `fig3_q98_diagnostics_by_arm.png` — ECDFs of `q98_normalized_rmse`, `relative_volume_bias`, `observed_peak_time_magnitude_error`, `high_flow_nse` by arm, with coverage noted in the title; (4) `fig4_provenance_envelope_diagnostic.png` — per-cell max-abs-diff ECDF (log scale) plus the aggregate exceedance table by arm, explicitly titled/labeled as PROVENANCE metadata, not skill failure, and marked not a pass/fail threshold. No classifier, winner, or C5/C6 content in any figure.

**Authority/provenance boundaries preserved, unchanged.** Canonical package-derived `qobs_m3s` remains the sole authority for Q98 facts; the validation-pickle-derived admitted series remains provenance/consistency evidence only; official objective/NSE/KGE computation paths were not touched. No sealed scope (SHARED-A5, temporal test, spatial holdout, California) was accessed. No training, inference, W&B, or controller contact occurred.

**Scope.** This entry records the reconciliation implementation and a complete, auditable formal-results-production run and its evidence. It sets no tolerance, makes no classifier/winner/promotion decision, and does not close RD1-C4. What remains open: the final scientific audit-policy decision on the provisional envelope, scientific interpretation of the per-arm NSE/KGE/Q98 distributions, and any subsequent RD1-C5/C6 work — all reserved to the user.

## 2026-09-16 — RD1-C4-D1 full 24-trial array + reduction COMPLETED at `a81873c751eb8f13d4209030882e146a57d80ac9` (Moriah array job `46176236`, reducer job `46176640`); evidence-only operational/measurement result — final audit policy, scientific classification, and C4 closure remain OPEN and unmade

**[FULL ARRAY + REDUCTION COMPLETED — EVIDENCE-ONLY OPERATIONAL/MEASUREMENT RESULT; NOT A SCIENTIFIC CLASSIFIER, TOLERANCE, WINNER, PROMOTION, OR AUDIT-POLICY DECISION; NOT A Q98/NSE/KGE PUBLICATION; NOT A C4 OR D1 CLOSURE; NO SEALED-SCOPE, TRAINING, W&B, OR CONTROLLER CONTACT]** Following the accepted disposable benchmark (below), the full 24-trial D1 array and its single reduction were run to completion, using the same authoritative inputs and identities already accepted for the benchmark. This entry records only that all 24 trials produced qualified receipts, the reducer published a complete 9,600-cell reduction with zero cell-level errors, and the resulting evidence was transferred and hash-verified. It draws no scientific conclusion, sets no tolerance, and selects no winner from the numbers it reports.

**Readiness (Phase A).** Isolated clone (`flash-nh-rd1c4-d1`) was found 2 commits behind `origin/master` (`8330c0d` vs `a81873c`) and was fast-forwarded with no conflicts; `bash -n` passed for all D1 launchers post-sync; the previously-landed atomic reduction-publish fix (commit `a81873c`, no `D1_OUT_DIR` in the array's `mkdir -p`) was confirmed present. The canonical clone (`flash-nh`) remained untouched throughout (only its pre-existing, unrelated untracked `wandb/` entry present). The full 24-trial roster (`rd1_c4_d1_trial_roster_benchmark_only.json`, despite its name — confirmed via `grep -c trial_id = 24` and `roster_generation_report.json`) was already proven to authenticate correctly by the accepted P2 benchmark, since `authenticate_trial_roster()` authenticates every roster entry on every invocation regardless of which single trial is resolved. Conclusion: the array/reducer path was already functionally safe; no repair commit was made for Phase A (per instruction not to patch merely for completeness).

**Array (Phase B).** `sbatch --array=0-23%4 scripts/rd1_c4_d1/rd1_c4_d1_array.sbatch`, job `46176236`, submitted from `flash-nh-rd1c4-d1` at HEAD `a81873c751eb8f13d4209030882e146a57d80ac9` (== `origin/master`), same `D1_TRIAL_LIST` (sha256 `01569743a7fe69f180f2ca4a2be2f518e45ed70b2b2822642894b8e22cc52e4e`), `D1_CONTRACT` (raw-file sha256 `99fdc4f44768779661a735d25974e25b64a5d7f502ec8ffa4c5d2bab42bc34d6`), and `D1_PACKAGE_ROOT` (`stage1_scientific_package_v002`) as the accepted benchmark, into a fresh never-before-used store root `full_a81873c_20260916T144219Z/store`, resource request 1 CPU/8GB/4h per task with a `%4` concurrency cap (four one-CPU tasks matching the four-CPU operational envelope). All 24 tasks reached `COMPLETED`, `ExitCode=0:0` (`sacct -j 46176236`); receipt count on the filesystem (`find .../receipts -name '*.json' | wc -l`) reached exactly 24, tracked in lockstep with task completion (4 → 8 → 20 → 24) across the monitoring window. Zero task failures occurred; no retry pass was needed; only this one 0-23 array was ever submitted.

**Reduction (Phase C).** First submission, job `46176609`, FAILED after 2s (`ExitCode=2:0`): the ad hoc submission script exported `D1_TRIAL_LIST`/`D1_CONTRACT`/`D1_STORE_ROOT`/`D1_OUT_DIR` but omitted `D1_PACKAGE_ROOT`, which `rd1_c4_d1_require_inputs` requires even though the `reduce` CLI subcommand itself takes no `--package-root` argument. This was a launcher-plumbing/submission-script omission (not a repository code defect — no file under `src/` or `scripts/` was changed) and was corrected directly per the autonomous-recovery guidance for ordinary operational failures. Because the reducer refuses to run if `D1_OUT_DIR` already exists and had refused before creating it, the same fresh path (`full_a81873c_20260916T144219Z/reduction`) was reused for the corrected resubmission rather than choosing a new one. Second submission, job `46176640` (same clone, same HEAD, same store, `D1_PACKAGE_ROOT` added), COMPLETED in `00:01:36`, exit `0:0`, publishing the output directory atomically.

**Reduction result (from the reducer's own `summary.json`, hash-verified post-transfer).** `n_trials = 24`, `n_basins_per_trial = 400`, `n_cells = 9,600` (exact 24×400 cross-product), `coverage.fraction_cells_compared = 1.0`. `cell_status_counts`: **`ok = 9,600`**, and every other status (`support_mismatch`, `package_missing`, `package_checksum_mismatch`, `nonfinite_canonical`, `pickle_load_error`, `area_derivation_inconsistent`, `basin_missing_from_trial`, `other_error`) `= 0`. `search_arm_counts`: `bayesian = 4,800` cells, `random_control = 4,800` cells — exactly 12 trials per arm. Single `contract_identity` across all cells (`common120_raw_space_nse_v001`, `cb4ebe86afa501ef3d5929ead5b455f8df06e7d38b58ebf4148f8545fe6851ef`). `q98_consequences` aggregate over 9,600 records: `n_basins_with_changed_frozen_decision = n_basins_with_changed_membership = n_basins_with_changed_peak_timestamp = 0`. Package-vs-pickle provenance diagnostic (nonfatal per the frozen scope's own rules, not a validity ruling): `n_elements_compared = 80,349,432`, `n_elements_bitwise_equal = 704,584`, `n_elements_exceeding_provisional_envelope = 11,352` (`provisional_envelope_is_report_only: true`); 38,704 extreme-element rows recorded (152 `first_envelope_exceedance` / 152 `max_envelope_exceedance` among them, i.e. exceedances occurred in some but not all of the 9,600 cells). `final_rd1_c4_audit_policy: "unresolved"`; `scientific_classification: "not performed — this reduction reports measurements only and selects no threshold"` (both fields are the reducer's own, verbatim).

**Evidence integrity (Phase D).** The reducer's `manifest.csv` and `checksums.sha256` independently agree on 8 output files' sha256/size. All 10 reduction-output files (~20.2 MB total: `cell_status.csv`, `cells.parquet`, `checksums.sha256`, `extreme_elements.csv`, `manifest.csv`, `q98_consequences.csv`, `receipt_index.csv`, `reduction.log`, `summary.json`, `ulp_or_source_precision_histogram.csv`) were transferred via `scp -O` into `.scratch_local/rd1_c4_d1_full_46176236_reduce_46176640/`; the 8 manifest-listed files were re-hashed locally (`Get-FileHash -Algorithm SHA256`) against `manifest.csv` — **8/8 OK, 0 mismatches**. This is the complete compact reduced-output layer; no per-shard/per-element detail parquet (referenced by hash/size in `receipt_index.csv`'s `detail_sha256`/`detail_size_bytes` columns) was transferred, and it remains on Moriah only. A compact, evidence-only, non-interpretive results packet was written to `.scratch_local/rd1_c4_d1_full_46176236_reduce_46176640/EVIDENCE_PACKET.md`, separating operational facts from scientific interpretation and explicitly stating no classifier/winner/promotion decision was made.

**Authority/provenance boundaries preserved, unchanged.** Same as the accepted benchmark entry below: the validation pickle/evaluator remains authoritative for historical official-objective reproduction and frozen-support NSE/KGE; the package's own lead-shift-aligned `qobs_m3s` remains authoritative for canonical Q98 observations; package-vs-pickle differences remain nonfatal provenance diagnostics; identity/timestamp/dimension/support contradictions remain fail-closed. No sealed scope (SHARED-A5, temporal test, spatial holdout, California) was accessed. No training, inference, W&B, or controller contact occurred. No second full 0-23 array was submitted, and no existing valid receipt or output directory was ever overwritten.

**Scope.** This entry records a complete, auditable operational/measurement result for the full D1 array and its one reduction. It sets no tolerance, makes no classifier/winner/promotion decision, and does not close RD1-C4 or C4-D1. What remains open: the final scientific audit-policy decision on the provisional envelope, scientific interpretation of the package-vs-pickle diagnostic and Q98-consequence aggregates, and any subsequent RD1-C5/C6 work — all reserved to the user.

## 2026-09-16 — RD1-C4-D1 disposable single-trial benchmark ACCEPTED at `8330c0d8748e858bd6704c84e9dcefa9e3b65893` (Moriah job `46176003`); operational/resource/evidence result only — the 24-trial array, reducer, final audit policy, and C4 closure remain OPEN and unauthorized

**[DISPOSABLE BENCHMARK ACCEPTED — OPERATIONAL/RESOURCE/EVIDENCE RESULT ONLY; NOT A SCIENTIFIC CONCLUSION; NOT A Q98/NSE/KGE PUBLICATION; NOT ARRAY OR REDUCER AUTHORIZATION; NOT A C4 OR D1 CLOSURE; NO NEW COMPUTE, TRAINING, W&B, CONTROLLER, OR SEALED-SCOPE CONTACT; NO COMMIT OR PUSH BY THIS DOCS TASK]** Following the benchmark-gate qualification recorded below, one disposable, isolated, one-trial resource benchmark was run and accepted. This entry records only that a real single-trial run completed with authenticated identity, ran the full 400-basin observation-provenance path once, and produced usable operational/resource/evidence data. It draws no scientific conclusion from the diagnostic numbers it reports.

**Code/repair history.** `af7f92eb7d522ed5a1f6966fbdfbc10f262958e8` fixed Slurm spool-copy resolution used by the D1 launch path. `8330c0d8748e858bd6704c84e9dcefa9e3b65893` — the commit the accepted benchmark ran at — narrowly extends the D1 evaluation path to safely accept a real validation-result variable laid out as `('date', 'time_step')` when `time_step` is singleton, while package-canonical observations continue to require the strict one-dimensional layout. Neither change is general production hardening; each addresses the one specific defect named here.

**Accepted benchmark.** Moriah job `46176003`, partition `glacier`, node `glacier-17`, ran the exact authenticated P2 frozen-roster trial (`stage1_phase_b_sweep_v2_six_axis_common120_v001__bayesian__proposal002__sweep_v2_cfg_dd9f7b847381f637439d__mf12x50000__seedA967139__attempt001`). Job COMPLETED, exit `0:0`. Scheduler elapsed `00:10:22`; process-internal (`/usr/bin/time`) wall clock `10:07.38`. `sacct TotalCPU = 07:40.8` (User 367.14s + System 91.69s ≈ 07:38.8 per GNU time; ≈75% single-core utilization on the requested 1 CPU, per GNU time's own `Percent of CPU this job got: 75%`). `sacct MaxRSS = 605,700 KB`; GNU-time peak RSS ≈ 778 MB — both comfortably inside the 8 GB request. Cell-status outcome: `ok: 400`, every other status `0`. `detail_rows = 3,279,806`. `n_q98_records = 400` (one record per evaluated basin). Progress publication was ordered end to end (`task_start → source_qualification_complete → trial_load_complete → 16× basin_progress → detail_finalization_complete → shard_finalization_complete → receipt_finalization_complete → task_end(outcome="published")`) with no gap or reordering. The store (`p2_8330c0d_20260916T130938Z`) was freshly created for this run and reused no prior shard. All seven receipt-declared component hashes (`cells.jsonl`, `detail/detail_index.json`, `detail/elements.parquet`, `extremes.jsonl`, `human.log`, `q98_consequences.jsonl`, `trial_summary.json`) were independently reproduced via `sha256sum` on Moriah and matched the receipt exactly. The compact evidence transferred to `.scratch_local/rd1_c4_d1_benchmark_p2_46176003/` (receipt, progress log, trial summary, detail index, `cells.jsonl`, `human.log`, `extremes.jsonl`, `q98_consequences.jsonl`, stdout/stderr) was hash-verified locally against the transfer manifest with no mismatch. The `sacct TotalCPU`/`MaxRSS` figures above were independently re-queried postrun via `sacct -j 46176003` on Moriah and transferred as a hash-verified supplemental record at `.scratch_local/rd1_c4_d1_benchmark_p2_46176003/postrun_supplemental/postrun_supplemental_46176003.txt` (local SHA-256 `e3e2a9f0edca3669ec4c05a43eccd90752d733b899f93ef72a15c0f3c4f891bf`, matching the Moriah-side manifest exactly); the transferred `stdout.out` independently corroborates the same figures via GNU time's own User+System time and peak-RSS fields, cited above. The ~76 MB detailed `elements.parquet` remained on Moriah; its size (76,259,998 bytes), zstd-internal compression, and SHA-256 (`20ee32434223c5ec2c5ea6ab547c9508cd56d93a0bf716fefb82e447de8f6689`) are recorded rather than the file itself.

**Resource/evidence decision.** For this one successful P2 task, the provisional `1 CPU / 8 GB / 4-hour` request is retained as operationally appropriate — it left substantial memory headroom (≈7.4 GB unused of 8 GB) and wall-time headroom (≈3h50m unused of 4h). This does **not** set full-array concurrency or a final array resource policy; that sizing decision remains open and unmade. The receipt/component evidence layout (a per-component hash-and-size manifest embedded in the receipt, plus a separate ordered progress log) is operationally usable for this scale of evidence.

**Authority/provenance boundaries preserved, unchanged.** The validation pickle / formal evaluator remains authoritative for exact historical official-objective reproduction and for frozen-support NSE/KGE. The frozen package's own `qobs_m3s`, lead-shift-aligned to each basin's fixed-support contract timestamps, remains authoritative for canonical Q98 observations. Package-versus-pickle numerical differences remain nonfatal provenance diagnostics, not a scientific finding. Identity, timestamp, dimension, and support contradictions remain fail-closed. This benchmark's two distinct contract identities were recorded separately and are not compared for equality: the semantic support-contract checksum `cb4ebe86afa501ef3d5929ead5b455f8df06e7d38b58ebf4148f8545fe6851ef` (as embedded in the receipt/roster identity) and the raw contract-file SHA-256 `99fdc4f44768779661a735d25974e25b64a5d7f502ec8ffa4c5d2bab42bc34d6` for the actual `--contract` path passed at job launch (`/sci/labs/efratmorin/omripo/Flash-NH/data/fixed_support_contracts/stage1_v2_common120_fixed_support_contract_v001.json`, per the transferred `stdout.out` command line), independently re-derived postrun via `sha256sum` on Moriah and included in the same hash-verified supplemental record cited above. The receipt's own `receipt.json` carries only the semantic checksum; the raw file hash is postrun supplemental evidence, not part of the original compact transfer.

**Roster-filename clarification (operational path only).** The accepted job used the actual versioned staged roster file, `rd1_c4_d1_trial_roster_benchmark_only.json`, under the already-approved versioned benchmark-input directory `benchmark_inputs/p2_af7f92e_20260916T113634Z/`. An earlier operator prompt referred to this file by the shorthand name `trial_list.json`. This is recorded only as a path clarification for future D1 operational work; it does not stage a new default runtime path, does not change any script, and does not authorize array use.

**Scope/state.** Exactly one accepted disposable benchmark exists (job `46176003`). Jobs `46175271` and `46175818` are not accepted benchmark evidence and yielded no scientific conclusion. RD1-C4 remains open. No 24-trial array and no reducer has run. No scientific classifier, tolerance, final NSE/KGE/Q98 publication, C5/C6 work, or sealed-scope access is authorized by this entry. Known full-array blockers remain open, including: atomic shard/reducer publication and integrity, durability/race handling, final ULP/storage correction, final Slurm hardening, the array-concurrency sizing decision, and the final scientific audit policy.

## 2026-09-16 — RD1-C4-D1 benchmark gate QUALIFIED for exactly one disposable, isolated benchmark trial; the benchmark itself has NOT run; the 24-trial array, reducer, and C4 closure remain OPEN and unauthorized

**[BENCHMARK-GATE QUALIFICATION ONLY — NOT A BENCHMARK RUN; NOT AN ARRAY OR REDUCER AUTHORIZATION; NOT A C4 OR D1 CLOSURE; NO SCIENTIFIC RESULT]** Correction Pass A's authoritative-input interfaces (execution-receipt-authenticated trial roster, path/hash-bound validation pickle via `AuthenticatedPeriodResults`, fixed-support-contract-qualified package, exact dimension/timestamp/lead-alignment enforcement, and source-binding/re-hash-on-read) were independently reviewed. The focused combined local suite covering trial authentication, package qualification, fixed-support contract, the formal hydrological consumer, the D1 diagnostic, the reducer fixture, and launch guards produced **379 passed**, with one pre-existing environment `RuntimeWarning` (`numpy.ndarray size changed, may indicate binary incompatibility`) — a known NumPy ABI warning, not a test failure.

**Dangling-symlink benchmark-store guard corrected and qualified.** The independent review identified that `_d1_benchmark_validate_store_root()` in `rd1_c4_d1_benchmark.sbatch` canonicalized `D1_BENCHMARK_STORE_ROOT` via `realpath -m` before checking existence, so a dangling symbolic link at that raw path would be silently canonicalized away rather than refused. A raw-path `[[ -e || -L ]]` check was added immediately before canonicalization, ahead of and independent from the existing post-canonicalization `[ -e "${resolved}" ]` check. After this correction, the local launch-guard suite produced **34 passed, 1 skipped** — the sole skip is the new behavioral dangling-symlink test, conditioned exactly on Windows `winerror` 1314 (`ERROR_PRIVILEGE_NOT_HELD`, i.e. this Windows development session lacking `SeCreateSymbolicLinkPrivilege`), with every other exception path re-raising rather than skipping.

**That one skipped test was qualified on real Linux.** Moriah Slurm job `46173923` (partition `glacier`, 1 CPU, 1 GB, 10-minute limit) ran, inside the documented Flash-NH Moriah conda environment: `bash -n scripts/rd1_c4_d1/rd1_c4_d1_benchmark.sbatch` (syntax passed) and, as the only pytest node selected, `test_benchmark_store_root_guard_refuses_a_dangling_symlink`, which produced `1 passed in 0.16s`. Job state `COMPLETED`, exit `0:0`, elapsed `00:00:20`. Transferred source files (`rd1_c4_d1_benchmark.sbatch`, `rd1_c4_d1_common.sh`, the test module) and the pulled-back evidence bundle (submitted Slurm script, stdout/stderr, `sacct` row) had local and remote SHA-256 hashes verified equal in both directions. No dangling-symlink target or other unexpected output was created by the guard function during that run.

**What this qualifies, precisely.** This clears the benchmark-store-isolation guard for exactly one disposable, operator-named, one-trial resource benchmark, run outside any Slurm array, writing only into a fresh operator-supplied path under the approved benchmark area (`${FLASHNH_BASE}/tmp/rd1_c4_d1/benchmarks`) that must never be reused, reduced, or promoted into the shared array/default store. **The benchmark itself has not been run** — no benchmark Slurm job has been submitted, no benchmark shard exists, and no resource measurement (elapsed time, MaxRSS) has been produced for a real trial. The full 24-trial array and the reducer remain unauthorized and unrun. Correction-B (atomic publication), receipt-reuse integrity, reducer-integrity, durability/fsync, ULP conventions, and final launch-hardening work remain open and unresolved. No final audit tolerance, no 24x400 conclusion, no scientific result, and no RD1-C4 closure exists. No full-repository test suite was run in this checkpoint; the 379-test figure and the 34/1-skip figure are the two focused suites actually run, and Moriah execution was scoped to exactly the one qualifying pytest node plus a syntax check.

## 2026-09-15 — RD1-C4 Correction Pass A authoritative-input interfaces locally implemented and synthetically tested; D1 and C4 remain OPEN; no benchmark authorized

**[CORRECTION PASS A LOCALLY IMPLEMENTED AND SYNTHETICALLY TESTED — D1 NOT RUN; C4 OPEN; NO SCIENTIFIC/METHODOLOGY CHANGE; NO PASS VERDICT; NO BENCHMARK AUTHORIZED; NO COMMIT OR PUSH]** The formal RD1-C4 Q98 evaluation target is corrected so that the frozen package NetCDF's own `qobs_m3s`, lead-shift-aligned to each basin's frozen fixed-support contract timestamps via the already-qualified algebraic identity (`qobs_mm_per_h_lead{L}[t] == discharge_m3s_to_runoff_mm_per_h(qobs_m3s[t+L], area_km2)`), is the canonical raw-space observed series for every trial and basin. Per-trial `validation_results.p` observations are retained as provenance/consistency-audit evidence only and never determine the formal metric target or any Q98 fact. The official NSE/KGE/Q98 mathematics, candidate population, predictions, checkpoints, fixed-support timestamps, package contents, trial-validity policy, and all training/run products are unchanged.

**Status: Correction-A input authentication is locally complete; the scientific work remains unresolved.** Execution receipts now authenticate the exact P1–P12/R1–R12 proposal roster; receipt-derived run directory, epoch, objective, proposal/configuration identity, campaign/domain/fidelity, evaluation scope and sealed-scope state are bound through to the validation pickle. The validation pickle is path/hash-bound through `AuthenticatedPeriodResults`; package manifest, checksum manifest, provenance, schema, layout, contract and every consumed basin file are qualified; array dimensions, coordinate identity, timestamps and the canonical lead shift fail closed. These are local interface results only. No RD1-C4 closure, audit policy, pass/fail threshold, numerical tolerance, automatic classifier, scientific result, commit, or push follows from them.

**Job 46168080 (Moriah, authorized rescore) FAILED — exit 1.** All 24 trial sources were built and every official objective reproduced exactly (P1-P12, R1-R12; evidence: `.scratch_local/rd1_c4_job46168080_recovery/rd1_c4_q98_repro_rescore_human.log`, `input_provenance_manifest.csv`). The run then terminated inside `_audit_admitted_observation_provenance` at basin `06911000`, trial `...bayesian__proposal002...` (P2), with max abs diff `7.58034e-06 m^3/s` against `rtol = 64 * float32 eps = 7.63e-06` and `atol = 1e-06 m^3/s`. The 24x400 rescore therefore does not exist; no scored product was produced.

**Targeted element-level diagnostic (job 46169186, basin 06911000 / P2).** Over 8,436 admitted elements the provisional reference envelope rejected 5 (fraction 5.93e-04). The artifact records `reported_max_abs_diff_element_is_itself_rejected = false` and `extreme_elements_all_same_element = false`: **the reported maximum absolute difference was not the failing element**, and the maximum-absolute, maximum-relative and maximum-float32-scale elements are three different elements. The uniform-scale test gives `k - 1 = 1.03e-08` (0.086 float32-eps units) with `residual_max_over_original_max = 1.071` — removing a single multiplicative offset does not reduce the discrepancy, so it is not explained by one uniform rescale, and it must not be described as a single rounding step. Evidence: `.scratch_local/rd1_c4_job46168080_recovery/targeted_06911000_p2/targeted_summary.json`, `targeted_elements.csv`, `targeted_completion_receipt.json`.

**The element that actually failed is a low-flow element, and the envelope failed it on `atol`, not on magnitude.** All five rejected elements carry the same package value `0.018972286954522133 m^3/s` (smallest and largest rejected package value are identical; earliest at index 7142, `2024-11-02T02:00:00`), with abs diff `1.2622488e-06` against a combined tolerance of `1.1447471e-06` (`atol = 1e-06` plus `rtol * |package| = 1.4475e-07`) -- an exceedance factor of `1.103`, and an ordered float32 distance of about 678 steps. The element carrying the `7.58034e-06` maximum absolute difference quoted in the failure message passed the same envelope comfortably, because at high flow `rtol * |package|` dominates. The stop was therefore produced by a fixed absolute floor applied at a value near 0.019 m^3/s, not by a large disagreement; that is a property of the provisional envelope's scale behaviour and is one more reason it must not be treated as a qualified criterion.

**Q98 consequence at that basin/trial: none.** Threshold `42.47526931762695` (package) versus `42.47526813291115` (reconstructed), abs diff `1.18e-06`; high-flow counts 176 and 176; high-flow mask symmetric difference 0 and mask SHA-256 equal on both sides; observed-peak index and timestamp equal (`2024-07-07T19:00:00`); peak tie-set size 4 on both sides; `any_frozen_q98_membership_or_peak_decision_changes = false`. The discrepancy that stopped the job changed no frozen Q98 decision **at this one basin and trial**.

**Evidence scope: one basin, one trial, one epoch.** Nothing above generalizes to the 24x400 population. The earlier "roughly a 30x safety margin" justification is withdrawn: it was derived from a single basin's maxima and was contradicted by job 46168080, whose failing element exceeded the same envelope.

**The tolerance is provisional and report-only.** `PROVENANCE_RTOL = 64 * np.finfo(np.float32).eps` (~7.63e-06) and `PROVENANCE_ATOL_M3S = 1e-06` are a reported comparison field. They are not a qualified audit criterion, not a scientific classifier, and not a closure threshold; the final RD1-C4 audit policy is the user's decision and remains unmade.

**Correction-A defects addressed locally.** (1) An arbitrary trial-list JSON can no longer assert a target: every entry opens and hashes its named execution receipt, and the authenticated receipts must form the exact P1–P12/R1–R12 proposal roster. (2) The formal consumer cannot read a package other than the package qualified against its fixed-support contract. (3) observation/simulation variables must have the exact authoritative one-dimensional temporal dimension and exact timestamps, and period results must be file-backed/authenticated rather than a plain mapping. (4) package authority metadata and checksum rows are strictly parsed and cross-checked against the producer schema. (5) every normal consumed-basin read re-hashes the NetCDF, so post-qualification mutation is detected without a special option. A vertical synthetic test exercises receipt → roster → authenticated target → authenticated pickle → fixed-support contract → qualified package → exact dimension/timestamp/lead alignment → D1 cell comparison.

**D1 is implemented locally and has NOT been run.** The 24x400 package-versus-pickle observation diagnostic (`src/baseline/rd1_c4_observation_diagnostic.py`, `..._reduce.py`, `..._cli.py`, `src/baseline/atomic_shard_store.py`, `src/baseline/source_precision.py`, `scripts/rd1_c4_d1/`) has been exercised only with small synthetic fixtures. No Slurm job was submitted, no benchmark was run, and no D1 shard, receipt or reduction exists for any real trial. The earlier one-basin evidence remains exactly one basin/trial/epoch and does not define a 24x400 audit policy, tolerance, or classifier.

**Correction-A local test evidence (2026-09-16).** The combined affected suite collected and passed **328 tests** in 105.67 s: trial authentication, package qualification, fixed-support contract, formal hydrological consumer, D1 diagnostic, reducer fixture, and launch guards. One environment warning recurred (`RuntimeWarning: numpy.ndarray size changed, may indicate binary incompatibility`). The dedicated vertical authenticated-source-to-D1 test passed separately. No full-suite run was performed and no full-suite success is claimed.

**Still open / not authorized.** Correction-B atomic publication, reducer publication/integrity gates, receipt-reuse integrity, durability/fsync, Windows rename races, ULP conventions/storage, and final Slurm hardening remain unresolved and were not repaired here. D1 has not run; C4 remains open; no benchmark is authorized. Nothing here authorizes a rescore, real D1 execution, a commit, a push, or an RD1-C4 closure.

## 2026-09-14 — RD1-C2 configuration-level analysis core CLOSED: implemented, independently reviewed, committed, and pushed at `e03e957109769ca7e34f034668da4ff5e463c268` (parent `34c49873be57c01d331bab0472dd2bbb19f2c606`); Codex final resolution review APPROVE after four earlier MAJOR interface defects were corrected pre-commit; focused RD1-C2 suite 67 passed, relevant v1/v2 regression suite 124 passed; machinery milestone only — no real 12+12 scientific review, no Bayesian-vs-random verdict, no winner/promotion, no search-space/range change, no seq_length > 120, no P13+/R13+, no SHARED-A5 consumption, no new training/NH-evaluation/W&B/controller/Slurm/Moriah contact, no sealed/temporal-test/spatial-holdout/California access; RD1-C3 (basin-analysis primitives) is next, separately planned/authorized

**[CLOSED — MACHINERY MILESTONE ONLY; DOCS CLOSURE FOR AN ALREADY-PUSHED IMPLEMENTATION COMMIT; NO REAL 12+12 SCIENTIFIC REVIEW; NO BAYESIAN-VS-RANDOM VERDICT; NO WINNER / PROMOTION / SELECTION; NO EXACT TOP-GROUP ORDERING CONCLUSION; NO SEARCH-SPACE / RANGE CHANGE; NO SEQ_LENGTH > 120; NO P13+ / R13+ / FURTHER SEARCH; NO FULL-DEVELOPMENT EVALUATION; NO SHARED-A5 CONSUMPTION; NO NEW TRAINING / NH-EVALUATION / W&B / CONTROLLER / SLURM / MORIAH / REMOTE-RETRIEVAL CONTACT; NO SEALED / TEMPORAL-TEST / SPATIAL-HOLDOUT / CALIFORNIA ACCESS; NO COMMIT OR PUSH BY THIS DOCS TASK]** RD1-C2 — the configuration-level analysis core for the frozen Stage-1 v2 12+12 scientific review (RD1-C1, `docs/stage1_v2_12plus12_review_design_v001.md`, untouched by this entry) — is implemented, independently reviewed, committed, and pushed to `origin/master` at commit `e03e957109769ca7e34f034668da4ff5e463c268` ("Implement RD1-C2 configuration analysis core"), parent `34c49873be57c01d331bab0472dd2bbb19f2c606`. The implementation commit contains exactly two files, `src/baseline/stage1_v2_12plus12_review_analysis.py` and `tests/test_stage1_v2_12plus12_review_analysis.py` (2,067 insertions, no other changed file). No design document, campaign config, execution script, or campaign evidence file changed in that commit. Carried by two commits: the implementation commit itself and this docs-only closure (`docs/decision_log.md` + the matching `docs/FLASHNH_CURRENT_STATE.md` entry).

**Independent review status.** The final independent Codex resolution review returned **APPROVE**. An earlier review pass had identified four MAJOR interface defects, all corrected before commit: (1) malformed exact-GPU-hour validation; (2) missing pooled P2-margin reporting; (3) incomplete proposal-aligned compute/yield consumer outputs; (4) incomplete proposal/configuration identity validation.

**Test evidence.** Final verified: RD1-C2 focused suite `tests/test_stage1_v2_12plus12_review_analysis.py` — **67 passed**; relevant v1/v2 regression suite (`test_sweep_v2_six_axis_review_analysis.py`, `test_sweep_v2_six_axis_config.py`, `test_sweep_v1_review_analysis.py`, `test_policy_v2_six_axis.py`) — **124 passed**. No claim of full-repository test-suite success is made.

**What C2 now authoritatively provides.** An explicit in-memory qualified-table consumer contract (`validate_review_table(...)`, `analyze_review_table(...)`), reusing the authoritative v2 six-axis legality/canonicalization already qualified in `sweep_v2_six_axis_campaign.py`. Frozen six-axis normalized geometry (learning rate log10, hidden size log2, embedding dropout linear, output dropout linear, batch size log2, seq_length linear/ordinal) feeding a six-axis RMS configuration distance, Bayesian novelty, and first-achieved Bayesian incumbent-distance semantics. Threshold-free top-k/top-3 summaries; frozen P2 near-incumbent yields at `0.01` and `0.005` with pooled P2-margin membership/counts; whole-sequence cumulative descriptors; duplicate-coordinate accounting without collapsing proposals; exact-GPU-hour validation/coverage; proposal-aligned cumulative exact compute; near-incumbent yield versus exact compute; champion-discovery and final-top-3 discovery compute; post-incumbent descriptors; and an explicit separation of random-arm descriptive ordering from Bayesian optimizer-learning semantics.

**Authority / interface boundary.** The upstream authorities remain `sweep_v2_six_axis_campaign.py` (legal six-axis domain and canonicalization) and a **future qualified evidence assembler** (arm identity, proposal order, proposal/configuration identities, the six hyperparameters, the official Common-120 objective, and optional already-qualified exact GPU-hours). RD1-C2's `validate_review_table(...)` and `analyze_review_table(...)` are the **consumer entry points** for that qualified in-memory table, not its producer; C2 then produces transparent configuration-level derived facts for later RD1 consumers.

**Semantics preserved.** The official Common-120 raw-space median per-basin NSE objective is unchanged; C2 does not qualify campaign validity; C2 does not infer exact GPU-hours from runtime; missing exact compute is not treated as zero; a malformed populated compute value fails the table contract; exact coordinate duplicates remain real proposals rather than being deduplicated away; exact objective ties retain the first-achieved incumbent; the random-control arm has no Bayesian novelty / incumbent-learning interpretation; the P2 `0.01` / `0.005` margins remain descriptive only, never equivalence bounds or promotion thresholds; equal weighting in the normalized six-axis distance is descriptive geometry, not evidence of equal scientific importance.

**Explicitly NOT authorized / NOT done by this closure.** RD1-C2 is a **machinery milestone only** — it does not mean the formal 12+12 scientific review has been performed, and no real P1–P12 / R1–R12 scientific analysis was performed by C2. Nothing here authorizes or records: a Bayesian-vs-random scientific verdict; a winner or promotion; exact top-group ordering; search-space narrowing or range changes; `seq_length > 120`; P13+ / R13+ or additional search; full-development evaluation; SHARED-A5 consumption; new training; NeuralHydrology inference/evaluation; W&B / controller / sweep activity; Moriah / h2o / Slurm activity; real `sacct` parsing; basin-level NSE analysis; P2/R2 basin comparisons; KGE; Q98; axis-confidence conclusions; rendering or a final review packet; sealed / temporal-test / spatial-holdout / California access. The implementation itself was pure, local, and synthetic-tested.

**Next.** The correct next implementation milestone is **RD1-C3 — basin-analysis primitives**, per the frozen sequence in `docs/stage1_v2_12plus12_review_design_v001.md`. This docs closure does not implement C3, does not pre-authorize later C4/C5/C6 work, and does not authorize any remote evidence retrieval or real campaign-evidence assembly. C3 is next for separate planning/authorization; the formal 12+12 scientific review remains downstream of the required RD1 implementation/data-consumer sequence.

## 2026-09-10 — Stage-1 v2 six-axis 12+12 campaign EXECUTION/EVIDENCE CLOSURE: the user-authorized Bayesian continuation P9–P12 (controller `wta85z3b`) and the frozen random-control Wave 3 R9–R12 ran at `5a09bce` and each adjudicated VALID from checksum-valid local compact evidence; the planned 12 Bayesian + 12 random-control HARD STOP is reached and verified; RD1-C1 frozen methodology (`docs/stage1_v2_12plus12_review_design_v001.md`, parent `1e32d577`) untouched; the evidence required for the RD1 scientific review is complete and preserved; docs-only; NO scientific 12+12 interpretation, NO Bayesian-vs-random comparison, NO winner / promotion / selection / axis-narrowing / sequence-extension / >120 seq_length, NO P13+ / R13+ / further proposals, NO objective / fidelity / search-space / seed-policy change, NO SHARED-A5 consumption, NO new compute / W&B / controller / Slurm / Moriah / training / NH-evaluation / remote-retrieval contact by this task, NO sealed / temporal-test / spatial-holdout / California access, NO commit or push by this task

**[CLOSED — EXECUTION/EVIDENCE CLOSURE ONLY; DOCS-ONLY; NO SCIENTIFIC 12+12 INTERPRETATION; NO BAYESIAN-VS-RANDOM COMPARISON; NO WINNER / PROMOTION / SELECTION; NO AXIS-NARROWING / SEQUENCE-EXTENSION / >120 SEQ_LENGTH; NO P13+ / R13+ / FURTHER PROPOSALS; NO OBJECTIVE / FIDELITY / SEARCH-SPACE / SEED-POLICY CHANGE; NO SHARED-A5 CONSUMPTION; NO NEW COMPUTE / W&B / CONTROLLER / SLURM / MORIAH / TRAINING / NH-EVALUATION / REMOTE-RETRIEVAL CONTACT BY THIS TASK; NO SEALED / TEMPORAL-TEST / SPATIAL-HOLDOUT / CALIFORNIA ACCESS; NO COMMIT OR PUSH BY THIS TASK]** The user-authorized Bayesian continuation (Proposals 9–12, controller `wta85z3b`, one new draw per proposal) and the frozen random-control Wave 3 (IID manifest rows 8–11, R9–R12) ran at repo commit `5a09bcee034748d85ed13fea8d0b098326e8988c` and completed earlier. This entry records the **execution/evidence closure only**: the campaign has reached the planned 12+12 hard stop, and the evidence required for the separate RD1 scientific review is complete and preserved. It is docs-only, launches no compute, and contacts no execution system. It performs **no** Bayesian-vs-random scientific comparison — that is the RD1 review lane's exclusive responsibility. Compact adjudication/closure evidence (untracked, `.gitignore` `.scratch_local/`): `.scratch_local/v2_p9_p12_wave3_5a09bce/` — `ADJUDICATION_LOG.md`, `PRESERVATION_RD1.md`, per-trial compact bundles + `SHA256SUMS.txt`. Carried as one docs-only change (this log entry + the matching `docs/FLASHNH_CURRENT_STATE.md` entry); no tracked source/config/test/manifest/scientific-design change.

**RD1-C1 frozen methodology is untouched.** `docs/stage1_v2_12plus12_review_design_v001.md`, frozen in parent commit `1e32d577032fcdb33fd1d52ffd77f57fdd9cf8cf` (2026-09-09 entry below), remains the authoritative frozen 12+12 review contract and is not modified or weakened by this closure.

**Full valid 12+12 complete — exact counts.** Bayesian arm: **exactly 12 valid proposals, P1–P12**, no P13. Random-control arm: **exactly 12 valid runs, R1–R12**, no R13. P1–P8 / R1–R8 were adjudicated in the 2026-09-08 post-P8 checkpoint; P9–P12 and R9–R12 are each independently **VALID** here from checksum-valid local compact evidence (all bundle checksums re-verified: p9 27/27, p10 29/29, p11 29/29, p12 29/29, wave3 62/62; 176 files, 0 failures).
- **P9–P12:** `execution_status = VALID`, `provenance_stage = VALID`, `artifact_identity_status = PASS`, `objective_eligible = true`, `review_records.workflow_status = pass`, strict-load `all_checks_pass = true` at `expected_commit = 5a09bce`, `sealed_scope = false`, a distinct internal launch-manifest per proposal, Seed A `967139`, fidelity `mf12x50000`, fixed-support contract SHA-256 `cb4ebe86…`, screening basin-ids SHA-256 `d4395d93…`, 12/12 model + 12/12 optimizer + 12/12 per-epoch `validation_results.p`, fixed-support epoch trajectory 1..12, `retry_history = []`. W&B runs `ugt88sfz` (P9), `pq6bc8a2` (P10), `d2qftrg1` (P11), `g4ueybn4` (P12), all `finished` / `flashnh/valid = true`. Controller run-count progressed 8→9→10→11→12, one new run per proposal, each gated by the per-proposal read-only Part-D controller reconciliation; forbidden run-ids never queried.
- **R9–R12:** `execution_status = VALID`, `provenance_stage = VALID`, `search_arm = random_control`, `objective_eligible = true`, `workflow_status = pass`, `git_commit = 5a09bce…`, `sealed_scope` false, `wandb_sweep_id = None` / `wandb_run_id = None` (zero W&B contact), support-contract SHA-256 `cb4ebe86…`, 12/12 per-epoch `validation_results.p`, trajectory 1..12. Slurm jobs `46120427`–`46120430`.
- **Objective values (best eligible epoch; screening-400 median per-basin raw-space NSE on the frozen Common-120 support).** Bayesian: P9 `0.44306`, P10 `0.43314`, P11 `0.42579`, P12 `0.40324` (P1–P8 as recorded 2026-09-08). Random-control: R9 `0.44349`, R10 `0.41878`, R11 `0.42987`, R12 `0.41359` (R1–R8 as recorded 2026-09-08). Recorded for evidence-completeness only; **no ranking, winner, or Bayesian-vs-random conclusion is drawn here** — screening-400 is not authoritative for ordering (SHARED-A5), and any comparison is the RD1 review's task.

**Random-control manifest remained frozen.** `config/stage1_phase_b_sweep_v2_six_axis_random_control_v001_random_control_manifest.json`, SHA-256 `59be6726b60863aeed1c25e86782bd7a5e1623434ce23c956e66eb54c527c095`, is git-tracked and unmodified (`git status` clean for `config/`); it carries exactly 12 rows (`random_control_count = 12`; manifest metadata records `prospective_pre_outcome_frozen: false` and `frozen_after_bayesian_observation: 1` — the frozen authoritative 12-row sequence was frozen after Bayesian observation 1, not prospectively before all Bayesian outcomes, and remained unchanged through R1–R12). Wave 3 consumed rows 8–11 in frozen manifest order; no row was reordered, replaced, or added, and there is no replacement search.

**Hard stop honored — no silent-extension path.** Verified from local evidence: no Bayesian proposal beyond P12 (P12's `wandb_run_final.json` shows `sweep_run_count = 12` and 12 distinct sweep run-ids post-completion; `bayes_run_registry_v3.json` holds 11 rows P1–P11 with no P12 append, by design as the last proposal); no random-control run beyond R12 (frozen manifest has 12 rows, no Wave 4 bundle exists). The tracked launcher `scripts/submit_sweep_v2_six_axis_wandb_agent_moriah.sh` is a manual one-shot wrapper (`exec sbatch` of one known job per invocation); `scripts/run_sweep_v2_six_axis_wandb_agent_moriah.sbatch` ends in `wandb agent --count 1` (one controller proposal per job, then exit) behind a `max_agents = 1` validator gate; no cron / at / systemd / daemon invokes either. The completion pollers are bounded read-only `squeue`/`sacct` waiters that only write `POLLER_DONE.sentinel` and exit (they submit nothing); the P10/P11/P12 pollers and the re-armed Wave-3 collector transitioned on genuine `slurm_state = COMPLETED` and exited 0 (P9 and the first Wave-3 collector hit the documented premature-sentinel hazard, were archived `*.PREMATURE_*` and re-armed with no re-draw). The W&B bayes controller `wta85z3b` is pull-based: with no agent process alive and no queued Slurm job, it cannot itself issue a run. A fresh read-only controller query was judged unnecessary and not performed.

**Evidence complete and preserved for the RD1 scientific review.** All 24 configs (P1–P12, R1–R12) retain 12/12 per-epoch `validation_results.p` (400-basin screening), 12/12 model checkpoints, 12/12 optimizer checkpoints, and provenance JSONs (`PRESERVATION_RD1.md` §2; each retained bundle's `checkpoint_inventory.txt`). The best-epoch validation-result product is therefore retained for every config regardless of which epoch the review fixes as official best. The Moriah run directories remain the **sole** copy of the full per-epoch `validation_results.p` pickles; the RD1 preservation hold (`_RD1_PRESERVE_DO_NOT_DELETE.txt` markers at both protected roots) stays binding until the 12+12 scientific-review lane closes. This closure takes no remote action; the execution lane — not this task — verifies live remote retention.

**Exact GPU-hour source available for all 24 — no gap.** Authoritative Slurm accounting (`sacct`) retains every `omripo` record back to 2026-08-09; per-trial `ElapsedRaw` is available for all 24 configs (`PRESERVATION_RD1.md` §4 job-ID ↔ config map; per-row `sacct.txt` in each retained bundle). No gap blocks exact per-trial GPU-hour reconstruction. Indicative totals (1× NVIDIA L4 per trial): Bayesian P1–P12 ≈ 33.7 GPU-h, random-control R1–R12 ≈ 26.6 GPU-h, campaign ≈ 60.4 GPU-h. Wall-clock campaign elapsed time is not substituted for GPU-hours.

**Stability diagnostics — factual, not retroactive invalidity.** Authoritative best-minus-final magnitudes: **P7 ≈ 0.39477** (best epoch 8; epoch-12 ≈ 0.033), **P12 ≈ 0.32547** (best epoch 5; epoch-12 ≈ 0.078), **P4 ≈ 0.12008** (best epoch 5; epoch-12 ≈ 0.301). These three are the only large late-epoch declines among the 24 runs; the next-largest is P2 ≈ 0.040 and the largest random-control decline is R2 ≈ 0.032. **P12 is another severe late-training-deterioration case** and strengthens the existing high-learning-rate instability MONITOR concern: it occurred at approximately `learning_rate = 8.30e-4`, `batch_size = 128`, extending the earlier P4/P7 pattern (both in the `{learning_rate ≥ 8e-4 AND batch_size = 256}` corner) beyond `batch_size = 256`; P12's training loss also rose after epoch 7 (avg_loss e7 ≈ 6.9 → e8 8.35 → e12 9.18). This is an observed pattern across three cases, **not** a causal single-axis claim, and it does **not** establish `learning_rate ≥ 8e-4` as a proven universal failure threshold. P4, P7, and P12 each remain **VALID** under the frozen best-eligible-epoch objective: the official screening objective is the best eligible epoch within the complete frozen 12-epoch fidelity, and in each case that best epoch precedes the divergence (no evidence is asserted that the best-epoch objective itself was depressed by the instability). `best_minus_final` and the retained scalar epoch trajectories capture the late behaviour; its scientific meaning is left to the RD1 review. No early-stopping rule, fidelity, search-space bound, or scoring rule is changed. The earlier local `ADJUDICATION_LOG.md` draft's "~0.02–0.05 slips" figure for P4/P7 was a drafting error and has been corrected in that untracked record to the authoritative P4 ≈ 0.120 / P7 ≈ 0.395 values.

**No scientific decision made here.** No winner, no model/configuration promotion or selection, no causal hyperparameter inference, no search-space / range / `seq_length > 120` change, no narrowed ranges, no further Bayesian proposals (P13+), no additional random controls (R13+), no full-development evaluation, no SHARED-A5 consumption beyond standing conclusions. SHARED-A5 remains binding: screening-400 is the economical screening population, not authoritative for exact top-group ordering, absolute NSE, checkpoint selection, or promotion; the full-development evaluation / checkpoint-selection / promotion gate remains mandatory downstream and no 12+12 finding may bypass it. The `seq_length = 120` boundary-pressure tally is a factual diagnostic for the RD1 review (7/12 Bayesian proposals at the 120 ceiling; P9–P11 at 120, P12 at 108) and authorizes nothing.

**Intended next state: `READY FOR 12+12 SCIENTIFIC REVIEW`.** The next phase is implementation and use of the prospectively frozen RD1 machinery — the next local milestone is **RD1-C2** (configuration-level analysis core), per the frozen design document's sequence — followed by the formal ChatGPT + user 12+12 scientific review.

> _(Note added 2026-09-14: RD1-C2 is now CLOSED — implemented, independently reviewed (Codex APPROVE), committed, and pushed at `e03e957`. See the 2026-09-14 "RD1-C2 configuration-level analysis core CLOSED" entry above. RD1-C3 (basin-analysis primitives) is next, separately planned/authorized. This does not change any 12+12 execution/evidence finding recorded in this entry.)_

## 2026-09-09 — Prospective Stage-1 v2 12+12 scientific-review design FROZEN (RD1-C1): user-authorized durable docs contract `docs/stage1_v2_12plus12_review_design_v001.md`, frozen before any substantive Proposal 9–12 / random-control Wave-3 outcome was examined in the scientific-design lane; docs-only; no analysis code, no compute, no W&B / controller / Moriah / h2o / Slurm / training / NH-evaluation / remote-retrieval contact; NOT proposal authorization; no winner / promotion / selection; no objective / fidelity / search-space / seed-policy change; no >120 seq_length / range change / P13+ / R13+ / full-development evaluation / SHARED-A5 consumption; no sealed / temporal-test / spatial-holdout / California access

**[CLOSED — PROSPECTIVE DESIGN FREEZE ONLY (RD1-C1); DOCS-ONLY; NO ANALYSIS CODE; NO COMPUTE; NO REMOTE / W&B / CONTROLLER / SLURM / TRAINING / NH-EVALUATION CONTACT; NOT PROPOSAL AUTHORIZATION; NO WINNER / PROMOTION / SELECTION; NO OBJECTIVE / FIDELITY / SEARCH-SPACE / SEED-POLICY CHANGE; NO SEALED / TEMPORAL-TEST / SPATIAL-HOLDOUT / CALIFORNIA ACCESS; NO COMMIT OR PUSH BY THIS TASK]** After the read-only RD1-A interface archaeology (which changed no file and inspected no substantive P9–P12 / R9–R12 outcome), the user has frozen the prospective 12+12 scientific-review methodology as a durable design contract. The authoritative detailed specification is the new document **`docs/stage1_v2_12plus12_review_design_v001.md`**; this log entry and the matching `docs/FLASHNH_CURRENT_STATE.md` entry record the freeze. Carried as one docs-only change (new design doc + these two entries); no tracked source/config/test change.

**Material decisions frozen.**
- **Configuration-level near-incumbent anchor:** P2, official pre-P9 objective `0.4388098707961096` (best epoch 9), arm-neutral. Descriptive margins: `0.01` primary / `0.005` sensitivity — descriptive review margins only, **not** statistical-equivalence bounds, **not** promotion thresholds; threshold-free top-k summaries also required.
- **Basin-wise paired anchors:** both P2 and R2 (R2 official pre-P9 objective `0.43276757946464195`, best epoch 6) retained as frozen references so hydrological results are not narrated relative to the Bayesian champion alone.
- **Best-epoch-only core hydrological review:** the 12+12 basin / KGE / Q98 comparison is frozen at each configuration's official best eligible epoch — 24 best-epoch validation-result products; the already-retained scalar fixed-support epoch trajectories are sufficient for the core stability analysis; all-12-epochs × 24-configs is **not** required for the core review.
- **Compute basis:** GPU-hours / configuration-equivalent compute is primary (not wall-clock). Exact per-trial GPU-hours **where authoritative Slurm/sacct evidence allows**; otherwise mark unavailable, report coverage, and keep exact accounting distinct from qualified campaign-level approximations.
- **Natural-support trajectory:** strictly **secondary** — appendix / secondary diagnostic only; never the HPO objective, an alternative ranking, or a second optimization target. Primary figures use Common-120 fixed support.
- **Six-axis normalized distance geometry:** learning_rate `log10`, hidden_size `log2`, embedding_dropout linear, output_dropout linear, batch_size `log2`, seq_length linear/ordinal over the frozen 48–120 grid; `d(x,y) = sqrt(mean_k((x_k − y_k)^2))` over the six normalized axes. Equal axis weighting is a **transparent descriptive geometry, not** evidence of equal scientific importance. Bayesian novelty = min distance to any earlier valid Bayesian config; incumbent distance = distance to the best-known Bayesian config immediately before proposal `i`. Frozen domain values remain authoritative in `src/baseline/sweep_v2_six_axis_campaign.py` / `sweep_v1_campaign.py`; the design doc does not redefine them.
- **Whole-sequence search-process analysis:** privilege the complete ordered sequence; early-half/late-half segmentation is not a primary method; legacy `proposal_drift_evidence` and the legacy early/late figures remain low-level historical diagnostics only; rank-correlation / trend statistics stay descriptive and are displayed with the underlying observations.
- **Basin-level NSE core set:** for every valid config at its best epoch, `basin_id -> Common-120 fixed-support raw-space NSE` over the frozen 400 basins, summarised by Q1, Q5, Q25, Q50, Q75, Q95, Q99, IQR, and finite/eligible counts (P10/P90 etc. may additionally be retained but do not substitute).
- **Basin paired-difference classification:** `delta = NSE_candidate − NSE_reference`; improved `delta > tol`, worse `delta < −tol`, tied `|delta| <= tol`, where `tol` is a **tiny documented machine-precision-scale value for floating-point equality only** — explicitly **not** a scientific NSE margin, and the `0.01` / `0.005` margins never apply at basin level. Report the full paired-difference distribution, not just counts. The old scratch-analysis `±0.01` basin "tie" convention is not imported.
- **Full-support KGE:** KGE-2009 and components (`kge`, `r`, `alpha = sigma_sim/sigma_obs`, `beta = mu_sim/mu_obs`) on the **same Common-120 support** at the best epoch, using the already-qualified `nh_raw_space_evaluation.raw_space_metrics` implementation — **secondary** diagnostic; cannot redefine trial validity, objective, ranking, or promotion.
- **Frozen Q98 high-flow methodology:** per basin, empirical Q98 = `np.quantile(observed_values, 0.98, method="linear")` over that basin's frozen Common-120 admitted 2024 observed discharge; fixed candidate-independent mask `observed >= Q98` (ties retained); **same observed-derived mask for every configuration**. Prespecified package: (A) Q98-threshold-normalized RMSE = `RMSE(high_flow) / observed_Q98_threshold`; (B) relative volume bias = `sum(sim − obs) / sum(obs)` over the subset; (C) observed-peak-time magnitude error = `[sim(t_obs_peak) − obs_peak] / obs_peak` at the timestamp of maximum **observed** discharge in the subset (no independent simulated-peak search); (D) high-flow NSE as a secondary caveated diagnostic; (E) high-flow sample count always reported. Guards: ≥50 high-flow timestamps and finite nonzero denominators for high-flow NSE / conditional-KGE-like metrics, else NaN/undefined; a secondary high-flow diagnostic failure **never** invalidates an otherwise valid HPO trial; diagnostic coverage always reported. High-flow KGE and event-based peak/timing/hydrograph diagnostics remain **exploratory**.
- **Legacy high-flow code:** `src/baseline/high_flow_event_metrics.py` predates this methodology and is **not scientifically authoritative for the new Q98 package** (RD1-A established: suppressed conditional NSE; NRMSE by observed high-flow mean not Q98 threshold; different sample-count gate; independent simulated-peak selection; event-window semantics). Low-level primitives may be reused later; the RD1 Q98 implementation must not silently inherit these semantics.
- **`seq_length = 120` interpretation:** frozen as **"boundary pressure observed / measurable; expansion is not authorized; sufficiently strong combined evidence may motivate a separate targeted >120 experiment"** — **not** `"natural / N/A"`. Does not change the search domain, authorize >120 trials, establish causality, or imply longer context is better. The lower boundary is described per evidence and existing semantics; no symmetry requirement is manufactured.
- **Axis / interaction evidence matrix:** exposes transparent lower-level evidence columns (coverage, direction, robustness, random-control support, Bayesian concentration, boundary pressure, interaction, failure/late-instability, basin-level robustness, KGE robustness, Q98 robustness) plus a **human** reviewer-confidence field and a candidate action. Weak evidence normally means **keep the present search space unchanged**, not speculative narrowing.
- **No automatic final scientific-confidence classifier:** the final `strong / suggestive / weak` axis label is a **human scientific-review judgment by ChatGPT + user**, documented with its supporting evidence — never a mechanical mapping of the matrix columns. `boundary_pressure_tier` and similar threshold-driven labels may be shown only as low-level diagnostic evidence. **Bayesian concentration alone can never establish strong axis evidence.**
- **Training stability:** the official score remains the best eligible Common-120 objective within the frozen 12-epoch `mf12x50000` fidelity; best epoch, best score, final-epoch score, best-minus-final, late gain, fixed-support trajectory, natural-support trajectory (secondary), runtime, and GPU-hours-where-recoverable are retained separately. A valid run with poor late stability remains a valid scientific observation; configurations are **not** retrospectively invalidated for inconvenient behaviour.
- **Existing validation products must be preserved pending review:** at minimum every valid configuration's official best-epoch `validation_results.p`, preferably all currently retained per-epoch validation-result products until RD1 closes, plus model checkpoints as fallback, the frozen Common-120 contract, the frozen screening-400 identity, the per-trial completion/provenance/checksum bundles, the Slurm/sacct GPU-hour evidence, and the tracked frozen random-control manifest. **Deletion of non-best-epoch validation products is not authorized.** The execution lane — not this task — must verify actual remote retention; this task contacts no remote system.
- **Data-contract distinction (from RD1-A):** the design document distinguishes (1) retrieving / re-reading an already-produced `validation_results.p`; (2) deterministic re-scoring of that retained output (pure arithmetic, **not** training); (3) new NeuralHydrology evaluation/inference from a checkpoint (only if a required validation result is missing). Deterministic metric extraction from an already-produced validation result is **not** described as new training.

**Prospective isolation.** No substantive P9–P12 / R9–R12 outcome was inspected or used as the basis of any decision above; `.scratch_local/v2_p9_p12_wave3_5a09bce/` was not opened or enumerated. The freeze rests only on pre-P9 committed evidence (P2, R2 identities and scores).

**Planned implementation sequence (not implemented here):** RD1-C2 configuration-level analysis core → RD1-C3 basin-analysis primitives → RD1-C4 fixed-support hydrological consumer + Q98 (highest-risk interface milestone; strong Codex-review candidate) → RD1-C5 transparent axis/interaction evidence derivations (no automated verdict) → RD1-C6 rendering + final review packet. The next local implementation milestone is RD1-C2, after review/acceptance of this docs patch. The real 12+12 scientific review occurs only after the execution lane declares the valid 12+12 campaign complete and the required evidence is available.

> _(Note added 2026-09-14: RD1-C2 is now CLOSED — implemented, independently reviewed (Codex APPROVE), committed, and pushed at `e03e957`. See the 2026-09-14 "RD1-C2 configuration-level analysis core CLOSED" entry above. RD1-C3 (basin-analysis primitives) is next, separately planned/authorized; RD1-C4/C5/C6 remain not implemented and not pre-authorized. This does not change the RD1-C1 frozen methodology recorded in this entry.)_

**Not authorized by this freeze.** `seq_length > 120` or any search-range change; narrowed ranges; continued Bayesian search (P13+); additional random controls (R13+); full-development evaluation or SHARED-A5 consumption beyond standing conclusions; model/hyperparameter promotion, selection, or winner declaration; new W&B / controller / sweep activity; any new training or NeuralHydrology inference; any new compute beyond deterministic re-scoring; sealed / temporal-test / spatial-holdout / California access. Eight valid Bayesian (P1–P8) + eight valid random-control (R1–R8) v2 observations; SHARED-A5 remains binding and no 12+12 finding may bypass that downstream gate.

## 2026-09-08 — Unattended Moriah W&B-agent launcher hardening CLOSED: the maintained tracked unattended submission path is `bash scripts/submit_sweep_v2_six_axis_wandb_agent_moriah.sh <production-manifest-path>`; guarded Slurm-client / `SLURM_CONF` restore + a `wandb agent` credential-availability preflight (`$HOME/.netrc` fallback via stdlib `netrc`) added to the tracked sbatch; the strict `mode=production` manifest stays authoritative for the W&B sweep id / project / entity; independent Codex review APPROVE; NOT proposal authorization; no P9–P12, no random-control Wave 3, no W&B / controller / Slurm / network / training contact

**[CLOSED — OPERATIONAL LAUNCHER HARDENING ONLY; NO SCIENTIFIC SCOPE CHANGE; NOT PROPOSAL AUTHORIZATION; NO P9–P12; NO RANDOM-CONTROL WAVE 3; NO W&B / CONTROLLER / SLURM / NETWORK / TRAINING CONTACT; NO WINNER / PROMOTION / SELECTION; NO OBJECTIVE / FIDELITY / SEARCH-SPACE / SEED-POLICY CHANGE; NO SEALED / TEMPORAL-TEST / SPATIAL-HOLDOUT / CALIFORNIA ACCESS]** The classification-B launcher-hardening step flagged as the required next action in the 2026-09-08 post-P8 8-plus-8 campaign-health checkpoint entry below is now closed. It is a small tracked operational-reproducibility change to the existing unattended agent-launch path; it makes no scientific decision and launches no compute.

**Change — two commits on top of `56da32d71a9d5404803d7db93094cd53ee7781b2`.**
- Implementation (`Harden unattended Moriah W&B agent launch`) — six files: new `scripts/submit_sweep_v2_six_axis_wandb_agent_moriah.sh` (a normal tracked `100644` shell file, invoked through `bash`, not a `chmod +x` entry point); new `tests/test_sweep_v2_six_axis_moriah_unattended_hardening.py`; and modifications to `scripts/run_sweep_v2_six_axis_wandb_agent_moriah.sbatch`, `scripts/create_sweep_v2_six_axis_wandb_bridge_production_sweep.py`, `tests/test_sweep_v2_six_axis_wandb_bridge_foundation.py`, and `docs/remote_operations.md`.
- This docs-only closure entry (`docs/FLASHNH_CURRENT_STATE.md` + this log).

**Maintained unattended submission path.** `bash scripts/submit_sweep_v2_six_axis_wandb_agent_moriah.sh <production-manifest-path>` (the manifest path may also be supplied through `FLASHNH_SWEEP_V2_PRODUCTION_MANIFEST`). The wrapper forwards exactly one strict `mode=production` launch manifest to `sbatch --parsable --export="ALL,FLASHNH_SWEEP_V2_PRODUCTION_MANIFEST=<manifest>" scripts/run_sweep_v2_six_axis_wandb_agent_moriah.sbatch` and does nothing else — it sets no `WANDB_PROJECT` / `WANDB_ENTITY` and performs no controller reconciliation.

**Slurm behavior.** A usable inherited Slurm environment is left intact. Only when `sbatch` is not already resolvable does the wrapper guardedly prepend the documented Moriah Slurm client bin path; `SLURM_CONF` is guardedly restored only when it is absent and the documented config file exists. There is no `sbatch --version` / config-health probe — the real `sbatch --parsable` submission is the single authoritative consumer, so a broken client surfaces as a real submission failure rather than a spurious pre-check pass or a false green.

**Manifest / W&B identity (unchanged authority).** The strict `mode=production` manifest remains authoritative for the W&B sweep id, project, and optional entity. The production sbatch obtains them from the maintained pre-agent validator (`validate-launch --emit env` / `resolve_validated_production_launch`) and refuses a non-empty inherited `WANDB_PROJECT` / `WANDB_ENTITY` that contradicts the validated manifest before any W&B contact. The submission wrapper does not independently choose project or entity.

**Credential availability (observed-failure hardening).** Immediately before `wandb agent`, the tracked sbatch resolves the agent-login credential in this order: (1) a non-empty inherited `WANDB_API_KEY` in the job environment is used as-is; (2) otherwise the `api.wandb.ai` password is read from `$HOME/.netrc` via the Python standard-library `netrc` parser (the `wandb agent` CLI login path does not itself read `.netrc` — only `wandb.Api()` does — which was the documented cause of the P5 first-attempt "No API key configured" failure); (3) otherwise the job fails now, before any W&B contact. The value is never logged, never copied into the project tree, and not length- or charset-policed; W&B remains authoritative for whether it actually authenticates.

**Part-D / proposal authorization is NOT affected.** The tracked technical submission wrapper is not proposal authorization. Before every user-authorized Bayesian proposal the campaign workflow must still perform the separate read-only controller-state reconciliation (Part-D condition): the expected prior N−1 runs exist and are terminal/valid, controller and config identity are authoritative, prior objectives reconcile with the recorded observations, and there is no unexpected / pre-existing / nonterminal Nth proposal. Mutable run IDs and objective values remain per-proposal evidence, not durable launcher state. No generic tracked Part-D framework is implemented or claimed.

**Review / evidence status.** Independent Codex review, final verdict **APPROVE** (executable-capability test-gate blocker closed; focused Git-Bash suite `12 passed, 2 capability-skipped, 0 failed`; vertical fake-only integration closed and passing — tracked wrapper → fake `sbatch` → the real tracked `sbatch` invocation form → real strict `validate-launch` → fake `wandb`; Interface / Consumer Contract Gate closed; secret/auth and scientific/scope reviews PASS; no remaining blocking finding). No real Slurm, W&B, controller, network, or training activity was required or performed for this qualification. Final reviewed implementation-candidate SHA-256 `01eb87c1d73b719468008d196edf8127a289dc5761c1520a180438bae7085597`.

**Scientific status unchanged.** Eight valid Bayesian (P1–P8) + eight valid random-control (R1–R8) v2 observations; no winner, no promotion, no selection. The frozen six-axis search space, the Common-120 raw-space median per-basin NSE screening objective, the screening-400 population, Seed A `model_seed = 967139`, and the frozen `mf12x50000` contract (12 epochs × 50,000 updates, save every epoch, `performance_early_stopping_enabled = false`) are all unchanged. SHARED-A5 conclusions remain binding.

**Not authorized by this closure.** P9; Bayesian Proposals 9–12; random-control Wave 3; any W&B / controller contact or proposal draw; a new W&B sweep; any training or new scientific evaluation; any winner, promotion, or selection; sealed / temporal-test / spatial-holdout / California access. The next scientific/campaign step returns to the user / ChatGPT strategy layer for a separate decision.

## 2026-09-08 — V2 Stage-1 six-axis campaign: post-P8 8-plus-8 campaign-health checkpoint complete; user-authorized sequential Bayesian Proposals 5–8 + random-control Wave 2 ran at `c91a0ff` and each adjudicated VALID from checksum-valid local compact evidence; eight valid Bayesian + eight valid random-control v2 observations now exist; READY for a user decision on Proposals 9–12 + Wave 3 with one flagged item (P4/P7 high-lr/bs256 late divergence); launcher classification B; no winner, no promotion, no formal near-12 scientific review, no P9+, no Wave 3

**[CLOSED — CAMPAIGN-HEALTH CHECKPOINT ONLY; NO WINNER, NO PROMOTION, NO CAUSAL HYPERPARAMETER INFERENCE, NO FORMAL NEAR-12 SCIENTIFIC REVIEW, NO PROPOSAL 9–12, NO RANDOM-CONTROL WAVE 3, NO NEW W&B CONTACT / CONTROLLER DRAW / SWEEP MUTATION, NO NEW TRAINING, NO NEW FULL-DEVELOPMENT EVALUATION, NO OBJECTIVE / FIDELITY / SEARCH-SPACE / SEED-POLICY CHANGE, NO EARLY STOPPING, NO SEALED / TEMPORAL-TEST / SPATIAL-HOLDOUT / CALIFORNIA ACCESS, NO COMMIT OR PUSH BY THIS REVIEW]** A bounded sequential Bayesian Proposal 5–8 block (controller `wta85z3b`, one new draw per proposal) and random-control Wave 2 (frozen IID manifest rows 4–7, manifest SHA-256 `59be6726b60863aeed1c25e86782bd7a5e1623434ce23c956e66eb54c527c095`, zero W&B contact) were explicitly user-authorized and ran at repo commit `c91a0ff6f809272a746c8f4a85e1b7f4ffb05514`. Read-only retrieval of their compact completion evidence from Moriah into `.scratch_local/post_p8_8plus8_review_v001/` and checksum/provenance verification were separately user-authorized, as was resuming and completing the bounded 8-plus-8 health review without a further approval if all eight missing rows could be substantiated from checksum-valid local evidence. They could: 145/145 files OK across five internal `SHA256SUMS.txt` (0 failed); source-tarball, supplement-archive, and frozen-manifest SHA-256s recorded in `retrieval_manifest/`. This entry records the user-authorized health-checkpoint outcome; it is docs-only and launches no compute. No code, test, config, manifest, launcher, evidence bundle, W&B state, or remote run output was changed.

**Adjudication.** Each of P5–P8 and R5–R8 is independently **VALID** (`execution_status = VALID`, `provenance_stage = VALID`, `objective_eligible = true`, `review_records.workflow_status = pass`; fixed-support contract SHA-256 `cb4ebe86afa501ef3d5929ead5b455f8df06e7d38b58ebf4148f8545fe6851ef`, `support_contract_sha256` identical across all 16; `sealed_scope = false`, `evaluation_scope = development_validation_2024_only` for all 16 — R1–R8 via the nested `preparation_record`). All 16 objectives are the best-epoch `flashnh/common120_raw_space_nse_v001` over the frozen 400-basin screening population, frozen `mf12x50000` fidelity, Seed A `967139`, no early stopping. **Eight valid Bayesian (P1–P8) + eight valid random-control (R1–R8) v2 observations now exist.** Objective values — Bayesian: P1 `0.36622`, P2 `0.43881`, P3 `0.37531`, P4 `0.42120`, P5 `0.43137`, P6 `0.41835`, P7 `0.42781`, P8 `0.41278`; random: R1 `0.36990`, R2 `0.43277`, R3 `0.37176`, R4 `0.42770`, R5 `0.35789`, R6 `0.41665`, R7 `0.36857`, R8 `0.31855`. (P7's value is the best-epoch score at epoch 8, from a valid pre-divergence checkpoint; see the instability note below.)

**Campaign-health findings (descriptive, n = 8 per arm, arms NOT paired, no inferential test, no causal claim).**
- **Distribution.** Bayesian mean `0.4115` / median `0.4198` / sd `0.0248` / `[0.3662, 0.4388]`; random mean `0.3830` / median `0.3708` / sd `0.0369` / `[0.3185, 0.4328]`. Bayesian distribution shifted up as a whole; top-1 a near-tie (P2 `0.4388` vs R2 `0.4328`). The Bayesian advantage is in the typical draw, not the champion.
- **Cumulative best.** Both arms reached their final incumbent at trial 2 and never improved (trials 3–8 added no incumbent in either arm).
- **GPU-hours.** Loosely comparable (same L4, one GPU/trial, same fidelity); both arms reached the incumbent by ~4.8–6.1 GPU-h. Brief qualified note only.
- **Bayesian P1–P4 vs P5–P8 concentration.** Mean `0.4004` → `0.4226`; sd `0.0304` → `0.0074`; range `0.0726` → `0.0186` — descriptively clear, tight upward concentration. Freq ≥ `0.40` (descriptive reference): 2/4 → 4/4. P5–P8 locations: `seq_length` 108–120 (all), `hidden_size` 64–128 (never 256), `batch_size` 128 (3/4).
- **Random R1–R4 vs R5–R8.** Mean `0.4005` → `0.3654` (lower, wider) — the random arm's second half did not show the same upward concentration; this argues against a simple shared later-trial trend, but does not establish a causal Bayesian advantage.
- **Six axes over all 16.** `learning_rate` sampled `1.30e-4`–`8.89e-4` (unsampled `[1e-4,1.30e-4]`, `[8.9e-4,1e-3]`); `seq_length` levels 72 & 84 never sampled by either arm; Bayesian pinned at `seq_length` upper bound 120 in 4/8 (top-boundary pressure); `batch_size` 512 abandoned after P1; all `hidden_size` levels used; all 16 `configuration_id` distinct; no controller collapse (run counts 4→5→6→7→8, method stays `bayes`, all 8 W&B runs `finished`).
- **Sequence-length association (prior 4+4 finding → 8+8).** STRENGTHENED — objective by `seq_length` bucket increases monotonically (sl48 `0.319` → sl60 `0.368` → sl96 `0.417` → sl108 `0.424` → sl120 `0.429`), holding within each arm. Still descriptive, confounded, **no causal claim**, n = 8 per arm.

**Late-epoch instability.** Only two of 16 runs show a clearly large late-epoch decline best→epoch-12, both Bayesian: **P4** (`≈ 0.1201`; e12 `0.301`; functional; best epoch 5) and **P7** (`≈ 0.3948`; e12 `0.033`; model effectively destroyed by e12; best epoch 8). These are the only two clearly large late-epoch declines among the 16 runs; the largest random-control `best − final` is R2 at ≈ `0.032`. Both show substantial late degradation, with P7 far more severe (P4 stays functional; P7 is effectively destroyed). P4 and P7 share exactly one search-space corner: `{learning_rate ≥ 8e-4 AND batch_size = 256}`. Training losses for both are finite throughout but diverge after the best epoch. Both had a sustained good plateau before divergence (so the best-epoch objective is not chasing a transient spike); both carry 12/12 model + 12/12 optimizer checkpoints with `strict_load` `all_checks_pass: true` and a pre-divergence best-epoch checkpoint. `best_minus_final` is recorded for all 16.

**Is there enough repeated evidence to REQUIRE escalation before Proposal 9? — NO, with mandatory user disclosure.** The P4/P7 pair crosses from noise to a pattern and must be an explicit, documented input to P9–P12 scoping (no autopilot roll to P9), but it does not require escalation now because it is (a) confined to a describable sub-region of one axis pair, (b) fully captured by the existing recorded `best_minus_final` diagnostic, and (c) downstream-gated by the already-mandatory SHARED-A5 full-development evaluation / checkpoint-selection / promotion gate: the screening-400 objective alone cannot authorize final promotion, so these runs cannot be promoted on the screening objective alone, and valid pre-divergence best checkpoints do not bypass later promotion review (this does not predict which checkpoint that gate will ultimately select). **P4 → MONITOR; P7 → MONITOR** (P7 individually closer to escalation but its objective is from a valid saved pre-divergence checkpoint and it is one trial). **Objective and frozen fidelity unchanged; no early stopping** — two late-deteriorating runs of 16 do not warrant a fidelity-policy change and the best-epoch objective handles them correctly.

**Evidence-path policy.** The completion-report prose table alone was insufficient for this checkpoint (as anticipated). Continue requiring per-proposal compact-artifact retention; ADD explicit retention of the per-epoch training-loss slice (decisive for P4/P7, not in `review_records`) and of any failed-launch evidence directory (P5's was retained and was useful).

**Launcher-helper classification: B — small tracked operational hardening is the chosen next step before another unattended P9–P12 block; this is an operational reproducibility issue, not a scientific campaign redesign.** Narrow inspection of the retained helper snapshot: NO embedded credential (only 40-hex string is the git SHA; `WANDB_API_KEY` resolved from `~/.netrc` at runtime, never logged). Two recurring gaps the tracked path lacks: (1) `SLURM_CONF`/`PATH` export for detached non-interactive children (the `profile.d` Slurm hook is interactive-only); (2) `~/.netrc` fallback for the `wandb agent` CLI login path (the documented cause of the P5 first-attempt failure, job `46114607` "No API key configured", no W&B run created; retry `46114616` succeeded). Not A (gaps real, recurred); not C (bridge + sbatch + `validate-launch` + forbidden-sweep-id refusals sound). Prepared, non-applied hardening sketch and the files/tests to add are in `.scratch_local/post_p8_8plus8_review_v001/` — a separate tracked change with its own review. It is not a scientific reason to redesign the HPO campaign, and is not skippable merely because the campaign-health verdict is READY: the user has approved a workflow in which it is completed and reviewed before another unattended P9–P12 block.

**Readiness verdict: READY for a user decision on Bayesian Proposals 9–12 + random-control Wave 3**, with one flagged item to weigh in scoping: the P4/P7 `{learning_rate ≥ 8e-4, batch_size = 256}` late-divergence pattern — options (a) constrain/watch the `learning_rate` upper region, (b) require per-proposal `best_minus_final` reporting, (c) apply the classification-B launcher hardening. None blocks a decision. **Next project step: (1) launcher hardening + focused review; (2) then a separate user decision/authorization for Proposals 9–12 + random-control Wave 3; (3) followed by the planned formal 12-plus-12 scientific review.**

**Not authorized / not done by this checkpoint.** No winner; no promotion; no formal near-12 scientific review; no Proposal 9–12; no random-control Wave 3; no new W&B contact / controller draw / sweep mutation; no new training; no new full-development evaluation; no change to search space, objective, fidelity, screening population, or seed/sampling policy; no early stopping; no sealed / temporal-test / spatial-holdout / California access; no commit or push by this review. Compact evidence, the authoritative machine-readable 16-row table, comparability validation, figure package, and full analysis: `.scratch_local/post_p8_8plus8_review_v001/` (untracked). Carried as one docs-only change (`docs/FLASHNH_CURRENT_STATE.md` + this log); no tracked source/config/test change.

> _(Note added 2026-09-08: the classification-B launcher-hardening "next project step (1)" identified in this entry is now COMPLETE and CLOSED — see the "Unattended Moriah W&B-agent launcher hardening CLOSED" entry at the top of this log. The maintained unattended submission path is `bash scripts/submit_sweep_v2_six_axis_wandb_agent_moriah.sh <production-manifest-path>`. This does not authorize Bayesian Proposals 9–12 or random-control Wave 3, which remain a separate user decision, and changes none of this entry's campaign-health findings, the frozen fidelity/objective, or the SHARED-A5 conclusions.)_
>
> _(Note added 2026-09-10: "next project step" (2) is COMPLETE — the user subsequently authorized the Bayesian continuation P9–P12 and the frozen random-control Wave 3 R9–R12; both ran at `5a09bce`, each adjudicated VALID, and the campaign reached the planned 12+12 hard stop. Step (3), the formal 12-plus-12 scientific review, is not done: RD1-C1 froze its methodology (2026-09-09 entry) and execution/evidence closure is recorded in the "2026-09-10 — Stage-1 v2 six-axis 12+12 campaign EXECUTION/EVIDENCE CLOSURE" entry at the top of this log. The "eight valid Bayesian (P1–P8) + eight valid random-control (R1–R8)" counts here are superseded by 12+12; the campaign-health findings, frozen fidelity/objective, and SHARED-A5 conclusions are unchanged.)_

## 2026-09-07 — V2 Bayesian Proposal 4 adjudicated VALID (Bayesian observation 4); explicitly user-authorized as a single sequential proposal after SHARED-A5 closed; full frozen `mf12x50000` fidelity; objective `0.42119978965037297` at epoch 5; close-human-review-through-Proposal-4 phase complete; four valid Bayesian + four valid random-control v2 observations now exist; no winner, no promotion, no Proposal 5, no random-control Wave 2

**[CLOSED — DOCUMENTATION-ONLY CLOSURE; NO WINNER, NO MODEL/CONFIG PROMOTION, NO CAUSAL HYPERPARAMETER INFERENCE, NO PROPOSAL 5–8, NO RANDOM-CONTROL WAVE 2, NO NEW W&B CONTACT / CONTROLLER DRAW / SWEEP MUTATION, NO NEW TRAINING, NO NEW FULL-DEVELOPMENT EVALUATION, NO SEALED / TEMPORAL-TEST / SPATIAL-HOLDOUT / CALIFORNIA ACCESS]** Bayesian Proposal 4 from controller `wta85z3b` was explicitly user-authorized as a single sequential proposal after the SHARED-A5 screening-adequacy audit closed. It ran at exact commit `a0c5e7516944d5b2c26372408c5edc76b61ee0ae` and has been independently verified from its compact completion bundle (`.scratch_local/v2_proposal4_completion_a0c5e75_46113284_20260907T172619Z/`, local checksum verification 25/25 OK) and adjudicated **VALID (v2 Bayesian observation 4)** by read-only scientific + operational completion review. This entry records the result; it changes no scientific policy and launches no compute. No code, test, config, manifest, launcher, evidence bundle, W&B state, or remote run output was changed in this closure.

**Run / config identity.** Controller `omri-porat1-huji/flashnh-stage1/wta85z3b`; `search_arm=bayesian`, `proposal_order=4`, `execution_generation=1`, wave `six_axis_q12_48_120_v001_wave1`; `configuration_id=sweep_v2_cfg_5697a85648162f8ecbd8`; `trial_id=stage1_phase_b_sweep_v2_six_axis_common120_v001__bayesian__proposal004__sweep_v2_cfg_5697a85648162f8ecbd8__mf12x50000__seedA967139__attempt001`. W&B run `omri-porat1-huji/flashnh-stage1/ioo96j7p` (state `finished`, `flashnh/valid=true`; exactly one Proposal-4 run in sweep `wta85z3b`). Slurm job `46113284` (`catfish`/`catfish-02`, one NVIDIA L4, `COMPLETED` ExitCode `0:0`, elapsed `03:10:45`, `Restarts=0`, no requeue). Launch-manifest SHA-256 `7e82b0ffff998102fb65f27af7565043ba92d3733a2372e3e36c36a569a91a84`; fixed-support contract SHA-256 `cb4ebe86afa501ef3d5929ead5b455f8df06e7d38b58ebf4148f8545fe6851ef`; screening basin-ids SHA-256 `d4395d93ebc567cf09e149c0121463d75cf4f7ecc02c07a7c4a7999763baa372` (policy `stage1_provisional_operational_screening_subset_v001`, 400 basins). Seed A `model_seed=967139`; fidelity `mf12x50000`; `evaluation_scope=development_validation_2024_only`; `sealed_scope=false`.

**Six proposed axes (ONE Bayesian observation, not selected/promoted; optimizer input/telemetry only).** `learning_rate=0.00082420903810797349`, `hidden_size=256`, `embedding_dropout=0.28748777796959363`, `output_dropout=0.29998679047464538`, `batch_size=256`, `seq_length=108` (raw controller axes ≡ resolved overrides ≡ W&B run config ≡ applied NH config; `seq_length=108` on the `q_uniform(48,120,12)` grid).

**Coverage and outcome.** Full frozen fidelity completed: `target_epoch=12`, `max_updates_per_epoch=50000`, `save_weights_every=1`, `performance_early_stopping_enabled=false`; 12/12 model checkpoints + 12/12 optimizer states + 12/12 screening evaluations, all `evaluation_status=PASS`. Direct per-epoch evidence supplement from the large checkpoints: per-parameter optimizer `state['step']` = 50,000 × epoch every epoch (cumulative 50,000 → 600,000), and exactly the frozen 400 screening basins every epoch (per-epoch `validation_results.p` sorted-id SHA-256 `3d7203b134987de9ab9d2571b22808252a82e82edc2579169a575ada0b97d0ab` = frozen `validation_basins.txt`, byte-identical across all 12 epochs). `execution_provenance.json` `provenance_stage=execution_status=VALID`, `artifact_identity_status=PASS`, `prepare_status=PASS`, `objective_eligible=true`, `retry_history=[]`, `retry_of_trial_id=null`, `failure_category=null`; `review_records.json` `trial_summary.workflow_status=pass`; `blocked=false`, `stopped=false`, `final_status=monolithic_training_and_screening_complete`. **Objective `flashnh/common120_raw_space_nse_v001` = `0.42119978965037297` at `best_epoch=5`** (maximize); epoch-12 fixed-support score `0.3011203273126656`; **best-minus-final `0.12007946233770739`**; `best_score_10 = best_score_12 = 0.42119978965037297`; `late_gain_10_to_12=0.0`; `late_best=false`; runtime 11,389.8 s. W&B `run.summary` objective == `execution_provenance.objective_score` == review-record `objective_score` exactly; one finite objective-history row. `publication_state="not_published"` / `valid_result_order=null` in `review_records.json` is the downstream human-review ledger state — the execution-spine state captured before the in-run W&B bridge logs the objective, and a review-slice ordinal assigned only when a formal Bayesian review table is consolidated — not a defect or a W&B publication failure. The in-run bridge did publish the objective and `flashnh/valid=true` to run `ioo96j7p`, which is how the controller consumes the observation. No retry; no sealed access; Proposal 5 not requested or launched.

**P4's best-to-final decline is a diagnostic observation, not a fidelity-policy change.** P4's best−final `0.12008` is ~3× the next-largest (P2 `0.04042`) and coincides with the set-highest `learning_rate` (`8.2e-4` across all eight v2 configs) and an epoch-6 training-loss spike. It is treated as ONE observation. Because the screening objective is the best-epoch score across the 12 epochs, the controller receives P4 as a good observation regardless of the late decline. **The frozen 12 × 50,000 fidelity is unchanged and no early stopping is introduced.** Best epochs across P1–P4 are (1, 9, 9, 5) — no repeated early-peak pattern across the four Bayesian observations.

**Controller now holds exactly four valid completed Bayesian observations** (observations, not selected/promoted configurations; no causal hyperparameter effect is inferred from four adaptive observations): P1 `0.36621751839318467` (epoch 1); P2 `0.4388098707961096` (epoch 9); P3 `0.37530856722966427` (epoch 9); P4 `0.42119978965037297` (epoch 5). Together with the four independently-VALID random-control Wave-1 observations (R1 `0.36989789510584137`, R2 `0.43276757946464195`, R3 `0.3717623575075273`, R4 `0.42770033214795594`), **four valid Bayesian + four valid random-control v2 observations now exist.** The close-human-review-through-Proposal-4 phase is complete. Descriptive local review evidence (untracked, `.gitignore` `.scratch_local/`): `.scratch_local/post_p4_transition_review_v001/` — same-panel P1–P4 objective trajectories, per-arm screening-objective comparison (P1–P4 vs R1–R4), 6-D search-space coverage of the eight configs, and the exact source-value table.

**SHARED-A5 remains binding.** Screening-400 remains the economical Stage-1 v2 HPO / candidate-screening population; it is NOT authoritative for exact within-top-group ordering, absolute expected full-development NSE, checkpoint selection, or promotion, and the later full-development evaluation / checkpoint-selection / promotion gate remains mandatory. P4 was not part of the SHARED-A5 seven-configuration audit and does not reopen it; no new full-development evaluation was performed.

**Not authorized / not done by this closure.** No winner; no model/configuration promotion; no Proposal 5, 6, 7, or 8; no random-control Wave 2; no new W&B contact / controller draw / sweep mutation; no new training; no new full-development evaluation; no change to search space, objective, fidelity, screening population, or seed/sampling policy; no early stopping; no sealed / temporal-test / spatial-holdout / California access; no commit or push by this milestone. The next user decision is whether to authorize a bounded Bayesian Proposal 5–8 block (sequential) plus random-control Wave 2 (frozen manifest rows 4–7, runnable in parallel), followed by a paused eight-plus-eight campaign-health review. This factual closure is carried by the docs-only change that adds this entry and the matching `docs/FLASHNH_CURRENT_STATE.md` entry; no tracked source/config/test change.

> _(Note added 2026-09-08: the "no Proposal 5, 6, 7, or 8; no random-control Wave 2" clause of this closure is now superseded — a bounded sequential Bayesian Proposal 5–8 block and random-control Wave 2 (frozen manifest rows 4–7) were subsequently and explicitly user-authorized, ran at repo commit `c91a0ff`, and each completed VALID. See the "V2 Stage-1 six-axis campaign: post-P8 8-plus-8 campaign-health checkpoint" entry at the top of this log. That authorization changes none of this entry's Proposal-4 adjudication or the SHARED-A5 conclusions; a winner, promotion, and the formal near-12 scientific review remain not done and separately gated.)_

## 2026-09-07 — `[SHARED-A5]`: development-population Common-120 screening-adequacy audit CLOSED; screening-400 retained as the economical Stage-1 HPO screening population, NOT authoritative for exact top-group ordering or final promotion; Tier-C escalation NOT triggered; no winner, no promotion

**[CLOSED — SCREENING-ADEQUACY AUDIT ONLY; NO WINNER, NO MODEL/CONFIG PROMOTION, NO PROPOSAL 4, NO RANDOM-CONTROL WAVE 2, NO NEW W&B CONTACT, NO NEW TRAINING, NO NEW SEARCH SPACE, NO FULL-DEVELOPMENT MULTI-EPOCH CHECKPOINT SELECTION, NO SEALED / TEMPORAL-TEST / SPATIAL-HOLDOUT / CALIFORNIA ACCESS]** The SHARED-A3 seven-configuration population-transfer audit (screening-selected `best_epoch` fixed per configuration; only the evaluation population changes from screening-400 to the canonical 2,307-basin development population) has been executed for all seven configurations and the prospectively-frozen SHARED-A3 §7 rubric applied. This entry records the user-authorized scientific conclusion; it is docs-only and launches no compute.

**Execution.** P1 (`sweep_v2_cfg_26daf2a8f293007dd60b`) reused unchanged from the qualified canary (canonical row SHA-256 `3166aba529ef780c0a3ce35aaed26ac54286dd04a0eb3ae18db2fb57c0190097`). P2/P3/R1/R2/R3/R4 each completed one full-development NeuralHydrology evaluation plus one canonical scoring pass under Slurm (GPU jobs `46112127`/`46112145`/`46112155`/`46112157`/`46112159`/`46112161` on `catfish`; dependent CPU scoring `46112140`/`46112154`/`46112156`/`46112158`/`46112160`/`46112162` on `glacier`; generic preflight `46112006`, 6/6 PASS; all twelve `COMPLETED 0:0`). Every canonical row: exactly 2,307 basins, 0 missing / 0 extra / 0 excluded; `canonical_completeness` / `canonical_population_verified` / `provenance_verified` all true; shared `membership_ids_sha256 397ab432…`, shared `contract_checksum_sha256 1100c736…`, `population_role development_train`, identical across all seven. Source/staged checkpoint, config, scaler bytes unchanged pre vs post evaluation. No W&B; no epoch reselection; no training. The initial replacement-run GPU launch failure was a purely operational sbatch-header issue corrected with no scientific change; final rows come only from the valid replacement executions. Evidence: `reports/devpop_audit_common120_seven_config_v001/` and `.scratch_local/devpop_audit_remaining_six_v001/evidence/devpop_audit_six_bundle_20260907T123328Z/`; large result pickles left on Moriah.

**Frozen comparison (committed aggregator `aggregate_devpop_audit_seven_rows`, `allow_fixture_rows=False`; output SHA-256 `590000b2af62ed608d6999885b7c374e96fa54f26ddead6d2a4a69573d9a9f7a`).** Full-development ranking by median Common-120 raw-space NSE: 1 R2 `0.431472951033149`, 2 P2 `0.4274593325275011`, 3 R4 `0.4266591028699196`, 4 R3 `0.36679743945469`, 5 P1 `0.36470379840322187`, 6 P3 `0.36272353973025173`, 7 R1 `0.36257972359430857`. Screening top-3 `{P2, R2, R4}` = full-development top-3 `{R2, P2, R4}` — **top-3 group membership unchanged**; internal order changed (P2↔R2 at ranks 1–2). Spearman `0.7857142857142857`, Kendall τ `0.6190476190476191`, 4 pairwise rank reversals. All seven full-minus-screening deltas negative, absolute range ≈ `0.00104` (R4) to `0.01259` (P3); largest absolute drops are the Bayesian configs P2 (`-0.01135`) and P3 (`-0.01259`). `interpretation.tier = null`, `escalation_required = false`: the sole mechanically-crisp SHARED-A3 §7 rule (Tier C = top-3 membership change) is **not triggered**. No absolute NSE-delta threshold is defined or introduced.

**Decision (user-authorized 2026-09-07).** Screening-400 is **adequate as the economical working screening population for continued Stage-1 v2 HPO / candidate screening.** Basis: the same three configurations form the top group under both populations, that group stays clearly separated from the remaining four, and no result changes the substantive conclusion about which configurations occupy the strong-candidate group. The audit does **not** establish screening-400 as authoritative for exact within-top-group ranking, absolute expected full-development NSE, checkpoint selection, or final promotion (P2/R2 swap ranks 1–2; four pairwise reversals; per-configuration transfer shifts vary; all seven full-population medians below their screening medians). Therefore screening-400 continues as the economical screening population, its exact within-top-group ranking is treated cautiously, screening scores alone are not used for final promotion, and the already-required later **full-development evaluation / checkpoint-selection / promotion gate remains mandatory**. Screening and full-development evaluation are not claimed equivalent; no causal claim is made that Bayesian optimization causes poorer population transfer. Characterization: top-3 stable; some Tier-B-type internal movement, but not decision-changing for continued screening use — the durable formal conclusion is that Tier-C escalation is not triggered and screening-400 is sufficient for the next optimization-screening stage.

**Not authorized by this closure.** No winner; no model/configuration promotion; no Proposal 4; no random-control Wave 2; no new W&B contact; no new training; no new candidate/search space; no full-development multi-epoch checkpoint selection; no sealed / temporal-test / spatial-holdout / California access. Those remain later user decisions. The next phase is a return to optimization planning, determined separately by a fresh review conversation. Carried as one docs-only commit (`docs/FLASHNH_CURRENT_STATE.md` + this log); no tracked source/config/test change.

> _(Note added 2026-09-07, later the same day: the "no Proposal 4" clause of this closure is now superseded — Proposal 4 was subsequently and explicitly user-authorized as a single sequential Bayesian proposal and completed VALID as Bayesian observation 4. See the "V2 Bayesian Proposal 4 adjudicated VALID" entry at the top of this log. That authorization is separate from this audit closure and changes none of the SHARED-A5 screening-adequacy conclusions. Random-control Wave 2 remains unlaunched and separately gated.)_

## 2026-09-07 — `[SHARED-A5]` Phase A: remaining-six development-population Common-120 audit execution plumbing CLOSED (configuration-generic per-entry preflight + one-entry canonical scorer + seven-row SHARED-A3 comparison collector + execution-time scaler provenance); Codex found one seven-row-collector provenance-binding defect, repaired and re-reviewed PASS; the six P2/P3/R1–R4 evaluations are still not launched; no seven-row comparison, no screening-adequacy conclusion, no winner

**[CLOSED — EXECUTION PLUMBING ONLY; NO PRODUCTION RUN, NO SCIENTIFIC COMPARISON RESULT, NO WINNER / PROMOTION / PROPOSAL 4 / WAVE 2 / SEALED ACCESS]** SHARED-A5 Phase A adds the minimum tracked, configuration-generic execution plumbing needed to make the remaining six development-population evaluations (P2/P3 Bayesian, R1–R4 random-control) reproducible and fail-closed. It does not touch the frozen SHARED-A3 scientific design and launches no compute. The producer/consumer vertical is closed for the remaining-six execution path.

**Implemented and tested — commit `f6ad90bd33b6aa05b697fa35a18997e0c9e786c7` (parent `2222a8094360718fac00c75c4e4e61c9df143ed0`).**
- `src/baseline/devpop_audit_preflight.py` — manifest-driven, configuration-generic per-entry preflight; every architecture/identity/period/population fact is derived from the validated frozen selection-manifest entry and `FROZEN_FIXED_CONFIGURATION_V2`, never from P1 constants.
- `src/baseline/devpop_audit_scoring.py` — thin one-entry canonical scoring/orchestration path binding one frozen manifest entry to one canonical audit row via the unmodified SHARED-A2 evaluator; fails closed on any entry/epoch/checkpoint mismatch.
- `src/baseline/devpop_audit_seven_row_aggregation.py` — seven-row SHARED-A3 comparison consumer (deltas, ranks, Spearman/Kendall, pairwise reversals, top-3 stability). Only the mechanically-crisp SHARED-A3 §7 rule is encoded (a top-3 membership change triggers Tier C escalation); no tie rule and no NSE-delta threshold are invented. Screening score/epoch are joined from the frozen manifest.
- `src/baseline/devpop_audit_eval_run_producer.py` — execution-time scaler byte provenance (`audit_execution_time_scaler_provenance` plus an optional `expected_scaler_sha256` gate). No historical screening-time scaler identity exists; this path records execution-time provenance only and does not claim a historical one.
- Vertical producer/preflight → evaluator/scorer → seven-row collector test plus focused regression tests.

**Independent review.** Codex initially found one material provenance-binding defect in the seven-row collector: it validated each row's outer identity against the frozen manifest entry but never checked the row's embedded `checkpoint_identity` and verified evaluator provenance, so a valid row could be relabelled to another configuration's identity while retaining the original nested provenance. Repaired by binding the embedded `checkpoint_identity` and provenance receipt (trial/configuration identity, checkpoint epoch, checkpoint filename, checkpoint SHA-256, provenance schema, `provenance_verified`) to both the row's own authoritative identity and the corresponding frozen manifest entry; the exact P1→P2 relabel is now rejected, and the vertical test no longer passes by cloning one row and relabelling only its outer fields. Finding-specific Codex re-review returned **PASS**. Final focused Phase-A test result: 212 passed, 1 pre-existing warning.

**Explicitly not decided / not executed.** The six P2/P3/R1–R4 development-population evaluations have still not been launched. There is no seven-row real scientific comparison, no screening-vs-full ranking-stability result, no screening-adequacy conclusion, no winner, no promotion, no full-population checkpoint-selection study, no Proposal 4 or random-control Wave 2 authorization, and no sealed / temporal-test / spatial-holdout / California access. No objective, search-space, fidelity, sampling-policy, or interpretation-rule change. The immediate next separately authorized step is the remaining-six production execution. This closure is carried as two commits: the implementation/test commit `f6ad90b` above and the docs-only commit that adds this entry.

## 2026-09-06 — `[SHARED-A5]`: development-population Common-120 audit production execution opened; canonical 2,307-basin support artifact materialized + verified; P1 audit-path qualification canary COMPLETE (passed every gate); per-basin evaluator performance defect diagnosed, repaired, and committed (`34138c27`), then qualified byte-identical on real P1 data (Slurm `46103139`); seven-entry selection manifest materialized on Moriah; P1 canary + evaluator-hardening qualification CLOSED; remaining six configurations not run; no winner, no promotion

**[IN PROGRESS — ONE OF SEVEN CONFIGURATIONS SCORED AS AN AUDIT-PATH CANARY; NOT THE SCIENTIFIC COMPARISON RESULT; NO WINNER / PROMOTION / PROPOSAL 4 / WAVE 2 / SEALED ACCESS]** SHARED-A5 begins the real screening-400 vs full-2,307-basin development-population audit designed in the 2026-09-05 SHARED-A3 entry and made implementation-ready in the 2026-09-05 SHARED-A4 entry. The SHARED-A4 "explicitly not done" list is now historical (see the note appended to that entry below and to the SHARED-A4 current-state entry).

**Canonical Common-120 support artifact — materialized and verified.** `common120_devpop_audit_support_contract_v001.json`: file SHA-256 `6868a1b2cb11454b538deda225369993b7a70bf06876ee3311d895e9712dc339`, internal canonical checksum `1100c736693f2db3dd15f34cfdd89da6c06f59320b9afd0d3a78cfc2427ed46b`. Built once via `common120_support_builder.build_common120_support_for_development_population`; canonical development population membership-ids SHA-256 `397ab432564c18c3abc5158a47ada2b28840bbf6f0c213d2475444fded33858f` (2,307 basins, `development_train`); lead 6, target `qobs_mm_per_h_lead06`, 2024 validation window, seq-length floor 120, zero zero-support basins. Checksum-pinned and shared unchanged across all seven configurations.

**Seven-entry checkpoint-selection manifest — populated, frozen, and materialized on Moriah.** `.scratch_local/devpop_audit_a5/devpop_audit_selection_manifest_v001.json`, materialized byte-identically for the production audit at `/sci/labs/efratmorin/omripo/Flash-NH/runs/devpop_audit_common120_v001/devpop_audit_selection_manifest_v001.json`: file SHA-256 `4ce75f1e7fb74becf765ab59e17ffdaea44f2dd5f71cf827468e87388b9ac1a0`, internal order-independent `manifest_sha256` `3b1a471164d6d85dd0071f45c069f9c5619ad89e3f4fafc269a8fb23ae19b9f9`. The committed `load_devpop_audit_selection_manifest` validator passes: exactly seven entries — P1/P2/P3 (Bayesian {1,2,3}) + R1/R2/R3/R4 (random-control {1,2,3,4}); no duplicate trial/config identities; per-entry frozen screening score, `screening_best_epoch`, `checkpoint_filename`, and `checkpoint_sha256` present and grammar-cross-checked. The Moriah copy carries the same file SHA-256 and is shared and immutable across all seven configurations.

**P1 audit-path qualification canary — COMPLETE.** Configuration `sweep_v2_cfg_26daf2a8f293007dd60b` (Bayesian Proposal 1); screening-selected `best_epoch=1`; checkpoint `model_epoch001.pt` SHA-256 `71c920e2e8afa155c24f2083c226bd488fe1413709eddc502b09ea834f63a763`.
- Two producer defects were found and repaired, both committed before this milestone's patch: (1) frozen PT dynamic inputs were omitted from the prepared audit run dir — `012ffd6`; (2) a reused pilot setting `validate_n_random_basins=1000` silently truncated the intended 2,307-basin evaluation — `4bce760`. The corrected P1 NeuralHydrology full-development evaluation is Slurm job `46099909` (`COMPLETED`); its result object contains exactly the canonical 2,307 basins (set equality, membership-SHA match, 0 missing / 0 extra).
- Canonical scoring is Slurm job `46101232`, which passed Gates 0–7 (identity preflight, canonical expected-population spec, result-object membership, result shape, provenance receipt, canonical contract validation, `evaluate_devpop_common120_audit_row(require_canonical=True)`, paired comparison). Row: `canonical_completeness=true`, `canonical_population_verified=true`, `provenance_verified=true`, `n_basins_evaluated=2307`, `n_basins_excluded=0`, full-development P1 median Common-120 raw-space NSE `0.36470379840322187`.
- Paired observation (one data point): screening-400 P1 median `0.36621751839318467`; full-2,307 P1 median `0.36470379840322187`; signed delta (full − screening) `-0.0015137199899628029`. This is a technical audit-path qualification plus a single paired point; it does not on its own establish whether the screening-400 population is globally adequate, and it does not modify the SHARED-A3 §7 prospectively-frozen interpretation rubric (top-3 membership change remains the escalation criterion).

**Evaluator performance defect — diagnosed from code, repaired, and committed as `34138c27` ("Harden devpop audit evaluator").** `deserialize_contract_support` unconditionally re-ran `validate_devpop_audit_contract`, whose checksum step re-serializes (`json.dumps`, sorted keys) and SHA-256s the entire ~contract-sized payload; the SHARED-A2 evaluator calls `deserialize_contract_support` once per basin, so for the canonical 2,307-basin contract the whole payload was re-serialized and hashed ~2,307 times — an O(N_basins × contract_size) pathology (Gate 6 evaluator wall time 7,763.6 s ≈ 2.16 h for P1). Repair (additive, semantics-preserving): a new **private, internal** fast accessor `_deserialize_contract_support_prevalidated(contract, basin_id)` in `src/baseline/devpop_common120_audit_contract.py` (underscore-prefixed, deliberately excluded from `__all__`, not a public unvalidated entry point) does only the O(1) membership check + shared date-array decode, and is used by `_basin_support_metrics` in `src/baseline/devpop_common120_audit_evaluator.py` because `evaluate_devpop_common120_audit_row` already fully validates the immutable contract once at its boundary. That evaluator now creates exactly one local plain-dict snapshot (`validated_contract = dict(contract)`), validates that object, and consumes that same validated snapshot for every subsequent contract read (identity checks, target/lead/period, the per-basin support accessor, the completeness gate, the returned identity/checksum fields) — it never validates one mapping and later consumes another. `deserialize_contract_support` keeps its public scientific meaning as the safe, validating accessor (validate, then delegate to the private helper). The full contract validator is now invoked a small constant number of times per audit row (boundary + completeness gate), independent of basin count. Nothing scientific/evaluator-semantic changes: schema, checksum, canonical population identity, Common-120 support identity, provenance verification, timestamp matching, completeness, finite-NSE, zero-exclusion, and sealed-scope protections are all still enforced before any successful canonical row. New focused regression tests (semantic-identity of the fast path, tampered-contract still fails before a row, misuse raises `DevpopAuditContractError`, and a validator-invocation-count check that does not scale with basin count, the private helper's absence from `__all__`, and a check that the evaluator consumes its validated local snapshot rather than re-reading a caller-supplied Mapping) plus adjacent regression tests pass.

**Real-data equivalence qualification — COMPLETE and PASSED.** CPU-only Slurm job `46103139` (`COMPLETED 0:0`, elapsed `00:11:53`; no GPU, no NeuralHydrology inference, P1 run directory untouched) re-scored real P1 through the optimized evaluator at `34138c27` and reproduced the pre-hardening result exactly: `nse_median == 0.36470379840322187`, contract internal checksum `1100c736…`, membership-ids SHA-256 `397ab432…`, checkpoint SHA-256 `71c920e2…`, P1 result SHA-256 `c88a9b8d…`, `canonical_completeness` / `canonical_population_verified` / `provenance_verified` all true, 2,307 evaluated, 0 excluded, zero non-finite sim at admitted support. Full canonical audit-row structural equality against the pre-hardening reference row (Slurm `46101232`): differing paths `[]`, all 2,307 per-basin rows deep-equal, aggregate / provenance / completeness deep-equal, 28/28 explicit equivalence checks passed; the new canonical row is byte-identical to the reference (SHA-256 `3166aba529ef780c0a3ce35aaed26ac54286dd04a0eb3ae18db2fb57c0190097`). Optimized/pre-hardening evaluator call time `649.370466709137 s` vs `7763.612004041672 s` (speedup `11.955597616543768`, ~12×) — engineering evidence for durable provenance, not a new scientific acceptance threshold. The evaluator optimization is qualified on real P1 data as scientifically equivalent; the P1 audit-path canary and the evaluator-hardening qualification are CLOSED. Compact evidence: Moriah `/sci/labs/efratmorin/omripo/Flash-NH/evidence/devpop_audit_p1_equivalence_46103139/`, local `.scratch_local/devpop_audit_a5/evidence_devpop_audit_p1_equivalence_46103139/`.

**Explicitly not decided / not executed.** The campaign is ready for the remaining six P2/P3/R1–R4 development-population evaluations, but those six have not been launched; no seven-row aggregation; no screening-vs-full ranking-stability result; no screening-adequacy conclusion; no winner; no configuration promotion; no full-population checkpoint-selection study; no Proposal 4 or random-control Wave 2 authorization; no sealed / temporal-test / spatial-holdout / California access; no change to objective, search space, fidelity, sampling policy, or interpretation rules. The evaluator-hardening patch (`src/baseline/devpop_common120_audit_contract.py`, `src/baseline/devpop_common120_audit_evaluator.py`, `tests/test_devpop_common120_audit_contract.py`, `tests/test_devpop_common120_audit_evaluator.py`) was committed as `34138c27` on `master`; the real-data equivalence qualification recorded above was run against that commit, and this factual closure is carried by the docs-only commit that adds this update.

## 2026-09-05 — `[SHARED-A4]`: LOCAL/SYNTHETIC development-population Common-120 audit orchestration implementation closed and independently reviewed PASS (seven-entry selection-manifest contract + audit evaluation-run producer + vertical producer→A2 path); no real checkpoints, no real artifact, no execution

**[CLOSED — LOCAL/SYNTHETIC IMPLEMENTATION READINESS ONLY; NOT THE SCIENTIFIC AUDIT; NO REMOTE/W&B/SLURM/NH/SEALED ACCESS]** This closes SHARED-A4 implementation readiness for the seven-configuration screening-400 vs full-2,307-basin development-population audit designed and frozen in the 2026-09-05 SHARED-A3 entry below. It does not open, run, or conclude the scientific audit itself; that remains SHARED-A5+ gated work requiring separate authorization.

**What was implemented (four new files, local/synthetic tests only).** `src/baseline/devpop_audit_selection_manifest.py`, `src/baseline/devpop_audit_eval_run_producer.py`, `tests/test_devpop_audit_selection_manifest.py`, `tests/test_devpop_audit_eval_run_producer.py`.
- One atomic seven-entry checkpoint-selection manifest contract: Bayesian proposals 1–3 + frozen random-control Wave-1 rows 0–3 (`proposal_order` 1–4); exact, fail-closed composition.
- Manifest identity is hash-pinned (`manifest_sha256`, order-independent over the seven entries); recorded `screening_best_epoch` is carried through, never recomputed.
- Fail-closed checkpoint identity: `checkpoint_filename` must equal the existing `weight_stem(screening_best_epoch) + ".pt"` convention (reused, not duplicated); filename, SHA-256, and source bytes are verified before any copy/rename.
- Audit-specific evaluation-run producer consumes the validated full seven-entry manifest plus an entry selector, proves selected-entry membership, preserves the exact `selection_manifest_sha256` into its persisted run manifest, prepares the full-development validation configuration using existing lower-level machinery (freshly re-derived development population), and verifies canonical population, target, lead 6, the 2024 validation period, screening configuration identity against the loaded fixed-support contract, checkpoint identity, and copied scaler/checkpoint provenance.
- One vertical local/synthetic path demonstrated: validated seven-entry manifest → selected member → audit evaluation-run producer → persisted manifest/checkpoint/config provenance → A2 provenance receipt → existing unmodified A2 evaluator → one canonical synthetic audit row.

**Independent review.** First Codex review found two material provenance defects — (1) the producer was not tied to the exact atomic seven-entry manifest (it accepted an arbitrary individually-valid standalone entry); (2) `screening_best_epoch` was not bound to the source checkpoint filename/epoch. Both were repaired at a minimal, additive level: the producer now takes the validated full seven-entry manifest plus an entry selector, proves the selected entry's membership, and carries the exact `selection_manifest_sha256` into its persisted run manifest; the selection-manifest entry validator now fail-closed requires `checkpoint_filename == weight_stem(screening_best_epoch) + ".pt"` before any copy/rename. Focused Codex re-review returned **PASS** — exact seven-entry manifest binding closed, epoch/checkpoint mismatch closed, vertical producer→A2 contract closed, no new material defect.

**Test evidence (not rerun for this documentation closure).** 22 SHARED-A4 tests passed; 206 relevant regression tests passed.

**Explicitly not decided / not executed — SHARED-A5+ gated, separate authorization required.** No recovery of the seven real training-run/checkpoint locations; no real checkpoint hashes computed or frozen; no real populated seven-entry selection manifest; no canonical real 2,307-basin Common-120 support artifact materialized; no Moriah/h2o/W&B/Slurm contact; no NeuralHydrology execution; no canary; none of the seven real development-population evaluations; no real audit-row aggregation; no scientific comparison result; no winner selection or configuration promotion; no sealed-scope access; no Proposal 4 or random-control Wave 2 authorization; no change to objective, search space, fidelity, sampling policy, or scientific interpretation rules.

> **HISTORICAL / SUPERSEDED by the 2026-09-06 SHARED-A5 entry above.** This "not executed" list was accurate on 2026-09-05. Since then the canonical 2,307-basin Common-120 support artifact has been materialized and verified, the seven-entry selection manifest has been populated and passes the committed validator, and the P1 audit-path qualification canary has run end to end on real NeuralHydrology output (Slurm 46099909 → scoring 46101232). Still not done: the remaining six evaluations, the seven-row aggregation, any screening-vs-full conclusion, any winner/promotion, Proposal 4 / Wave 2, and any sealed-scope access. The SHARED-A4 design/review/scope content in this entry otherwise stands.

**Scope of this closure.** SHARED-A4 closes as six files together: the four reviewed implementation/test files (`src/baseline/devpop_audit_selection_manifest.py`, `src/baseline/devpop_audit_eval_run_producer.py`, `tests/test_devpop_audit_selection_manifest.py`, `tests/test_devpop_audit_eval_run_producer.py`) plus the two documentation updates (`docs/FLASHNH_CURRENT_STATE.md`, `docs/decision_log.md`). No other files.

## 2026-09-05 — `[SHARED-A3]`: development-population Common-120 audit scientific-entry and producer-contract design freeze closed (documentation only); SHARED-A4/A5 milestone boundary corrected

**[CLOSED — DOCUMENTATION-ONLY DESIGN FREEZE; NO PRODUCTION AUDIT, NO CHECKPOINT-HASH POPULATION, NO REMOTE/W&B/SEALED ACCESS]** This closure freezes the scientific question and the producer/consumer design for the seven-configuration screening-400 vs full-2,307-basin development-population audit, closing the gap the 2026-09-02/2026-09-03 entries below left explicitly open. Full detail lives in `docs/stage1_devpop_common120_audit_contract_v001.md` (new file); this entry records only the decision summary. No code, test, config, manifest, launcher, evidence bundle, W&B state, or remote run output was touched.

**Decisions recorded.**
- The audit isolates population transfer only: each of the seven valid v2 configurations' screening-selected `best_epoch` checkpoint is held fixed while only the evaluation population changes (screening-400 -> canonical 2,307). It does not select a winner, does not determine what full-population validation itself would select, and does not promote. The diagnostic `devpop_audit` objective scope stays separate from the `common120_raw_space_nse_v001` optimizer objective.
- Screening-selected `best_epoch` is explicitly **not** automatically authoritative for eventual promotion; a full-population, multi-epoch checkpoint-selection study is mandatory before any promotion decision and may be escalated earlier if this audit shows meaningful instability.
- A single seven-entry, hash-pinned checkpoint-selection manifest design is frozen (fields + authority per field); it is **not populated** with real checkpoint paths/hashes in this closure.
- `common120_support_builder.build_common120_support_for_development_population` is confirmed the sole canonical Common-120 support builder, built once and reused unchanged and checksum-pinned across all seven configurations; not materialized in this closure.
- The producer -> SHARED-A2-consumer vertical path is frozen. Neither `pilot_full_validation.build_pilot_full_validation_bundle` nor `nh_seed_evaluation.prepare_development_population_eval_run_dir` is frozen as the reused producer unchanged (wrong scope / non-authoritative marker semantics respectively); SHARED-A4 must build an additive producer atop lower-level shared primitives with explicit target/lead/period/population/checkpoint cross-checks.
- A thin seven-row aggregation contract is frozen (exact seven identities, shared contract checksum, shared population identity, per-row completeness and provenance, screening score/epoch joined from the manifest — never reconstructed from audit output).
- A pre-result interpretation rubric is frozen **prospectively**, before any full-population number exists: required reporting dimensions (median NSE both populations, signed/absolute delta, rank, Spearman, Kendall, pairwise reversals, top-3 stability, basin-level diagnostics) plus three escalation tiers (materially stable / investigate / materially misleading) keyed on **top-3 membership change**, not an ad hoc NSE-delta threshold decided after seeing results.
- Fail-closed failure/restart/overwrite principles are frozen (no partial row, no silent exclusion, no silent overwrite, verified-complete reuse permitted, mismatched identity requires a fresh output).
- **Milestone boundary corrected.** SHARED-A4 is now scoped as local/synthetic implementation only — no Moriah/h2o/W&B/Slurm contact, no real checkpoint hash/path population, no real Common-120 2,307-basin artifact, no real NH run. SHARED-A5 (new, separately gated) is the future milestone for real evidence recovery, real artifact materialization, and production execution, beginning only after SHARED-A4 is reviewed/closed and remote activity is explicitly authorized.

**Explicitly not decided / not executed.** No checkpoint paths or hashes populated; no Common-120 2,307-basin artifact materialized; no producer implemented; no real NH run; no Moriah/h2o/Slurm/W&B contact; no sealed-scope access; no winner, promotion, or causal hyperparameter conclusion; no change to search space, objective, fidelity, or sampling policy; Proposal 4 and random-control Wave 2 remain unlaunched and separately gated.

**Scope of this closure.** Adds `docs/stage1_devpop_common120_audit_contract_v001.md` and this documentation update (`docs/FLASHNH_CURRENT_STATE.md`, `docs/decision_log.md`). No other files. Not committed or pushed by this task.
...
**LANE B — Proposal 3 (VALID, v2 Bayesian observation 3).** Controller `omri-porat1-huji/flashnh-stage1/wta85z3b`; W&B run `voo2a4i1` (`finished`, `flashnh/valid=true`); Slurm job `46017631` (`catfish`/`catfish-02`, one NVIDIA L4, `COMPLETED 0:0`, runtime `02:52:51`). `search_arm=bayesian`, `proposal_order=3`, `execution_generation=1`, `configuration_id=sweep_v2_cfg_00be6b4182e764f24f15`, `trial_id=…__bayesian__proposal003__sweep_v2_cfg_00be6b4182e764f24f15__mf12x50000__seedA967139__attempt001`; Seed A `967139`; fidelity `mf12x50000`; `evaluation_scope=development_validation_2024_only`; `sealed_scope=false`. `execution_provenance.json` `provenance_stage=execution_status=VALID`, `artifact_identity_status=PASS`, `prepare_status=PASS`, `objective_eligible=true`, `retry_history=[]`, `retry_of_trial_id=null`; `review_records.json` `trial_summary.workflow_status=pass`, `failure_category=null`. Launch-manifest SHA-256 `7b96c3636a13b1c76a3c9c33ba49d2c2dd3648eccedb7017efeb2e239d8d95c8`; generated NH config SHA-256 `ef134d8b2dd9cd4b37e1caecabcd150554d45b78776ab410fafb94c4ba336d72`; fixed-support contract SHA-256 `cb4ebe86afa501ef3d5929ead5b455f8df06e7d38b58ebf4148f8545fe6851ef`; screening SHA-256 `d4395d93ebc567cf09e149c0121463d75cf4f7ecc02c07a7c4a7999763baa372` (400 basins). Six proposed axes (one Bayesian observation, not selected/promoted): `learning_rate=0.00049221944330504037`, `hidden_size=256`, `embedding_dropout=0.32902259001994533`, `output_dropout=0.37382255853365531`, `batch_size=256`, `seq_length=60`. Coverage: 12 checkpoints + 12 optimizer states; a direct-count evidence supplement confirms exactly 50,000 actual optimizer updates every epoch (cumulative 600,000, from per-parameter `state['step']`) and exactly the frozen 400 screening basins every epoch (per-epoch `validation_results.p` sorted-id SHA-256 `3d7203b134987de9ab9d2571b22808252a82e82edc2579169a575ada0b97d0ab` = frozen `validation_basins.txt`, identical across all 12 epochs, no duplicate/missing/extra); screening PASS epochs 1–12; fixed- and natural-support evaluations complete and finite; `blocked=false`, `stopped=false`, `final_status=monolithic_training_and_screening_complete`. Objective **`0.37530856722966427` at epoch 9**; epoch-12 **`0.36099580201812653`**; best-minus-final **`0.014312765211537737`**; `late_gain_10_to_12=0.0`; `late_best=false`; runtime 10,264.16 s; maximum fixed/natural-support epoch-wise divergence ≈ `0.005318` at epoch 4. W&B: one finite objective row; summary/history/provenance/review agree within float representation. `publication_state="not_published"` / `valid_result_order=null` is the downstream human-review ledger state, not a defect. No retry; no sealed access; Proposal 4 not requested or launched.

The controller now holds exactly three valid completed Bayesian observations (observations, not selected/promoted configurations; no causal hyperparameter effect is inferred from three adaptive observations): P1 `0.36621751839318467` (epoch 1); P2 `0.4388098707961096` (epoch 9); P3 `0.37530856722966427` (epoch 9).

**LANE R — random-control Wave 1 (frozen rows 0–3, all four independently VALID).** Frozen manifest `config/stage1_phase_b_sweep_v2_six_axis_random_control_v001_random_control_manifest.json`, byte SHA-256 `59be6726b60863aeed1c25e86782bd7a5e1623434ce23c956e66eb54c527c095`, frozen **after Bayesian observation 1** (scientifically independent of Proposal 1; **not** a fully pre-outcome-frozen prospective control). Wave 1 = frozen rows 0–3. All four trials: ran from `b4fe5eda…`; `search_arm=random_control`; `execution_generation=1`; Seed A `967139`; fidelity `mf12x50000`; 12 epochs; exactly 50,000 actual optimizer updates per epoch; exactly the frozen 400 screening basins per epoch; 12/12 screening evaluations; no retry; `wandb_sweep_id=None` and zero W&B contact; `sealed_scope=false`; independently adjudicated VALID (no row's classification depended on another's). Deployed output root `/sci/labs/efratmorin/omripo/Flash-NH/data/stage1_phase_b_sweep_v2_six_axis_random_control_v001/wave1_20260901T154518Z`.

- Row 0: `proposal_order=1`, job `46018446`, `COMPLETED 0:0`, elapsed `02:25:28`, `configuration_id=sweep_v2_cfg_a476ab2b0f9fb7b42863`; `learning_rate=0.00038637832918302144`, `hidden_size=256`, `embedding_dropout=0.16324049036760591`, `output_dropout=0.32315075660579679`, `batch_size=256`, `seq_length=60`; best `0.36989789510584137` at epoch 10, epoch-12 `0.36001047867320574`, best-minus-final `0.009887416432635632`, max fixed/natural divergence ≈ 0.00633 (epoch 4).
- Row 1: `proposal_order=2`, job `46018447`, `COMPLETED 0:0`, elapsed `02:22:54`, `configuration_id=sweep_v2_cfg_5f9f8c5442fb1200d68e`; `learning_rate=0.00013016182151779704`, `hidden_size=64`, `embedding_dropout=0.19310104981894816`, `output_dropout=0.31371031433387786`, `batch_size=512`, `seq_length=108`; best `0.43276757946464195` at epoch 6, epoch-12 `0.4003737377555962`, best-minus-final `0.032393841709045745`, max divergence ≈ 0.00286 (epoch 11).
- Row 2: `proposal_order=3`, job `46018448`, `COMPLETED 0:0`, elapsed `02:16:35`, `configuration_id=sweep_v2_cfg_54476936ed984e3a4d64`; `learning_rate=0.00027138903592439174`, `hidden_size=64`, `embedding_dropout=0.27494538782218808`, `output_dropout=0.19665000313541278`, `batch_size=512`, `seq_length=60`; best `0.3717623575075273` at epoch 5, epoch-12 `0.35945905704764963`, best-minus-final `0.012303300459877653`, max divergence ≈ 0.01348 (epoch 3).
- Row 3: `proposal_order=4`, job `46018449`, `COMPLETED 0:0`, elapsed `02:14:49`, `configuration_id=sweep_v2_cfg_54d75cc3479d174bfa7e`; `learning_rate=0.00025723895769944868`, `hidden_size=128`, `embedding_dropout=0.21073170668108956`, `output_dropout=0.29293576343030359`, `batch_size=128`, `seq_length=108`; best `0.42770033214795594` at epoch 5, epoch-12 `0.4217216973619983`, best-minus-final `0.005978634785957615`, max divergence ≈ 0.00307 (epoch 5).

All four `late_gain_10_to_12=0.0`, `late_best=false`. Descriptive best-score ordering within Wave 1: row 1 > row 3 > row 2 > row 0 (descriptive only — not a selection, no winner, no manifest redraw, no single-axis causal effect).

**Bounded shared scientific conclusions (recorded).**
1. There are now seven valid, materially varied v2 configurations: three sequential Bayesian observations and four frozen IID random-control observations.
2. The seven best Common-120 objectives span approximately 0.3662 to 0.4388.
3. Provisional descriptive best-score order across all seven: Bayesian Proposal 2 `0.4388098707961096` > random row 1 `0.43276757946464195` > random row 3 `0.42770033214795594` > Bayesian Proposal 3 `0.37530856722966427` > random row 2 `0.3717623575075273` > random row 0 `0.36989789510584137` > Bayesian Proposal 1 `0.36621751839318467`.
4. This ranking is provisional and restricted to the 400-basin screening subset.
5. The four random draws are materially varied across multiple axes, including learning rate, hidden size, batch size, dropout, and sequence length.
6. These results do not justify a winner declaration, model promotion, a causal hyperparameter-effect claim, or any change to the search space, objective, fidelity, or sampling design.
7. The approved trigger for comparing screening-400 with the full development-validation population was approximately 3–5 materially different valid configurations.
8. The campaign now has seven valid configurations because Proposal 3 and Wave 1 executed concurrently. This is a bounded operational overshoot, not a policy failure.
9. The screening/full-population audit is now due and should occur before authorizing further Proposal-4 or Wave-2 execution.
10. Proposal 4 and random Wave 2 remain unlaunched and separately gated.

The audit's implementation, checkpoint-selection rule, compute plan, and acceptance threshold are deliberately not predefined in this closure; they require a new inspected planning milestone.

**Evidence (untracked per `docs/repo_policy.md`; local + Moriah).** Proposal 3: `.scratch_local/v2_proposal3_completion_b4fe5ed_46017631_20260901T201745Z/` (22-file bundle, `SHA256SUMS.txt` verified 22/22 OK) plus immutable sibling `supplement_direct_counts_20260901T204205Z/` (`SUPPLEMENT_SHA256SUMS.txt` verified OK; the original `SHA256SUMS.txt` was not modified). Wave 1: `.scratch_local/v2_random_control_wave1_completion_b4fe5ed_20260901T203307Z/` (`CHECKSUMS.txt` verified 12/12 byte-identical against the Moriah copies; per-row `execution_provenance.json` / `review_records.json` / `generation_manifest.json` and `cross_row_summary.md`).

**Not done / not authorized in this closure.** No winner, no model selection or promotion, no causal hyperparameter inference, no manifest redraw, no change to search domain / fidelity / objective / sampling design / promotion policy. No W&B contact or mutation, no Slurm submission, no Moriah synchronization, no training or evaluation, no sealed temporal-test / spatial-holdout / California access, no Common-120 regeneration. Proposal 4 and random Wave 2 remain unlaunched and separately gated. This closure edits only the two documentation files (`docs/FLASHNH_CURRENT_STATE.md`, `docs/decision_log.md`); no commit or push is performed by the closure task.

## 2026-09-01 — V2 production Proposal 2 adjudicated VALID and recorded as Bayesian observation 2; a separate execution step from Proposal 1; proposed axes are one observation, not a selected/promoted configuration; no winner, no hyperparameter-effect inference; Proposal 3 and random Wave 1 remain separately gated

**[CLOSED — TWO VALID v2 PRODUCTION TRIALS]** The second real Bayesian proposal from the accepted v2 production controller `wta85z3b` (`omri-porat1-huji/flashnh-stage1/wta85z3b`) executed to completion at exact commit `0090e94606383209ec9cb58e5dec6295e9bc5179` (Slurm job `46013192`, `catfish`/`catfish-04`, one NVIDIA L4, 8 CPU / 128 GB, `COMPLETED 0:0`, `02:37:07`, MaxRSS.batch `83,463,732K` ≈ 79.6 GiB, TotalCPU `02:36:43`; W&B run `y4przhjs` `swept-sweep-2`, terminal state `finished`; controller run count exactly 2). A read-only scientific + operational completion review adjudicates it **VALID**.

**Decisions recorded.** (1) **Proposal 2 is VALID and counts as v2 Bayesian observation 2.** `execution_provenance.json` `provenance_stage=execution_status=VALID`, `artifact_identity_status=PASS`, `prepare_status=PASS`, `objective_eligible=true`; `review_records.json` `trial_summary.workflow_status=pass`, `failure_category=null`. (2) **The six proposed axes are one Bayesian observation — not selected, promoted, or winning hyperparameters:** `learning_rate=0.0007223610500908302`, `hidden_size=128`, `embedding_dropout=0.27237327958287133`, `output_dropout=0.3494365141157241`, `batch_size=128`, `seq_length=120` (raw controller axes ≡ normalized ≡ W&B run config; `seq_length` on the `q_uniform(48,120,12)` grid, upper endpoint). (3) Identities: `proposal_order=2`, `execution_generation=1`, `configuration_id=sweep_v2_cfg_dd9f7b847381f637439d`, `proposal_id=…__bayesian__proposal002`, `trial_id=…__proposal002__sweep_v2_cfg_dd9f7b847381f637439d__mf12x50000__seedA967139__attempt001`; Seed A `967139`; fidelity `mf12x50000`; `evaluation_scope=development_validation_2024_only`; `sealed_scope=false`. Launch-manifest canonical SHA-256 `f48d5ef57d9c0eb3188a77354f9c79c512b8068ba3ba306c6fe200751b064397`; generated NH config SHA-256 `775e1ee67eb4bd352716bb361cc99414cf7d8f408c45c0acd5e41f968c7ffd2d`; fixed-support contract SHA-256 `cb4ebe86afa501ef3d5929ead5b455f8df06e7d38b58ebf4148f8545fe6851ef`; screening SHA-256 `d4395d93ebc567cf09e149c0121463d75cf4f7ecc02c07a7c4a7999763baa372` (400 basins). Only two v2 production agent jobs have ever existed (`46009495`, `46013192`) — no retry, no duplicate, no replacement. (4) **Execution coverage is complete under the frozen fidelity:** exactly 12 checkpoints and 12 optimizer states (epochs 1–12); exactly 50,000 optimizer updates per epoch (cumulative 600,000; tfevents last global step 600,000), all cap-compliant; screening PASS with 400/400 basins for epochs 1–12; fixed-support Common-120 and natural-support raw-space evaluations complete and finite for epochs 1–12; `blocked=false`, `stopped=false`, `final_status=monolithic_training_and_screening_complete`. (5) **Objective `0.4388098707961096` at epoch 9** (`best_epoch=9`); **epoch-12 fixed-support score `0.3983907128864448`**; **best-minus-final `0.04041915790966483`**; `best_score_10 = best_score_12 = 0.4388098707961096`; `late_gain_10_to_12=0.0`; `late_best=false`. (6) **Fixed-support and natural-support 12-epoch trajectories are identical at every epoch (maximum divergence `0.0`).** This is expected: `seq_length=120` equals the frozen Common-120 120-hour common-support floor, so each screening basin's naturally-available window coincides with its frozen support window. It is not a defect and does not alter the objective, which is the fixed-support Common-120 median NSE. (7) **W&B publication equality:** the published metric `flashnh/common120_raw_space_nse_v001 = 0.4388098707961096` is exactly equal across `run.summary`, `run.history()`, `execution_provenance.objective_score`, and the review record — no float-repr difference; `flashnh/valid=true`; `flashnh/trial_id` matches; `scan_history` 1 row. (8) **Descriptive Proposal-1 comparison only:** P1 best `0.36621751839318467` at epoch 1 vs P2 best `0.4388098707961096` at epoch 9, difference ≈ `+0.0725923524` in favour of P2 (epoch-12 difference ≈ `+0.0512528762`); P2 is materially higher on the optimizer objective and improves over training whereas P1 never beats epoch 1. **With only two observations, no causal hyperparameter effect is inferred and no winner is declared.** (9) No sealed-scope access, no retry (`retry_history=[]`, `retry_of_trial_id=null`), no second run, no Proposal 3, and no change to search domain, fidelity, objective, or promotion policy.

**Evidence.** Compact untracked local bundle `.scratch_local/v2_proposal2_completion_0090e94_46013192_20260901T140200Z/` (37 files incl. consolidated `SHA256SUMS.txt`; `adjudication_summary.{json,md}`, `proposal1_vs_proposal2.md`, `trajectory_table.txt`, `BOUNDARY_AND_COUNTERS.txt`, `attempt_execution_provenance.json`, `attempt_review_records.json`, `attempt_generation_manifest.json`, `optimizer_update_counts.json`, checkpoint/optimizer/eval inventories, `wandb_readonly.json`, `slurm_sacct.txt`, NH `output.log`/`config.yml`, agent log head/tail/keylines). Local checksum verification 36/36 OK, 0 failures. Read-only Moriah collection scratch left under `$HOME/tmp/` (home, not repo). Untracked per `docs/repo_policy.md`. One accepted non-blocking limitation (identical to Proposal 1): `gpu_hours` / `operations.slurm_job_id` / `operations.slurm_state` are `null` in `review_records.json`; job identity is independently fixed by `sacct` and the agent `.err`/`.out` logs (~2.62 L4-GPU-h from Slurm elapsed).

**Roadmap unchanged (approved operating policy, restated).** Detailed human review through approximately Proposal 4; bounded sequential Bayesian operation thereafter; formal Bayesian scientific reviews near 12 and 24 valid Bayesian observations; 12 IID random-control trials in three waves of four; Common-120 (`flashnh/common120_raw_space_nse_v001`) remains the sole optimizer objective with secondary metrics diagnostic; screening-400 vs full-population development-validation comparison after approximately 3–5 materially different valid configurations. Bayesian optimization remains sequential — each valid result reaches `wta85z3b` before the next proposal is requested.

**Not done in this closure.** Proposal 2 is not a winner and its configuration is not promoted or selected. This documentation commit records only the two documentation files. **Proposal 3 is not launched** — the next sequential Bayesian proposal requires separate explicit user authorization (controller healthy, exactly 2 completed valid observations). The v2 six-axis IID random-control readiness milestone remains committed and pushed at `45b7a830a0c2d6122d8879d231d57a716297ffc9` ("Add v2 six-axis random-control readiness"); **random Wave 1 remains unlaunched and separately gated**. No W&B contact, no Slurm submission, no Moriah synchronization, no training or evaluation, no sealed-set access, no Common-120 regeneration, and no code/config/test/manifest/launcher change occurred in this closure.

## 2026-09-01 — v2 random-control authority audit, THEN authorized freeze of the v2 six-axis IID random-control manifest (frozen after Bayesian observation 1); campaign operating roadmap recorded as approved policy

**[AUDIT → AUTHORIZED SCIENTIFIC DECISION → OFFLINE READINESS IMPLEMENTED; NOT COMMITTED, NOT EXECUTED]** Part 1 was a read-only repository audit at exact commit `0090e94606383209ec9cb58e5dec6295e9bc5179` (branch `master`, in agreement with `origin/master`) establishing that no compatible six-axis random-control manifest existed. Part 2 (same day, after the audit conclusion and the stop at the scientific boundary were accepted) is the user-authorized creation and freeze of exactly one such manifest, plus the lean offline execution-readiness path for it. Nothing was committed or pushed; no W&B contact, no Slurm submission, no training.

**Audit findings (authoritative, as of the audit — Part 1).**

1. **The only random-control manifest in the repository is the historical Sweep-v1 one:** `config/stage1_phase_b_sweep_v1_original_domain_v001_random_control_manifest.json` (byte SHA-256 `180d94feefa2cfc9686a29609b91711c7586ddfe903ea9c4ef2fcaef002346e3`, pinned in `src/baseline/sweep_v1_campaign.py` `RANDOM_CONTROL_MANIFEST_SHA256`). It has 12 rows, `manifest_rng_seed=20260822`, `model_seed_a=967139`, `domain_version="original_domain_v001"`, generator `python_random_mt19937_iid_log_uniform_and_uniform_categorical_v001`, draw order LR(log10)→embedding_dropout→output_dropout→hidden_size→batch_size.
2. **It is FIVE-axis, not six-axis.** Swept axes: `learning_rate` (log-uniform 1e-4–1e-3), `hidden_size` `{64,128,256}`, `embedding_dropout` (uniform 0.0–0.4), `output_dropout` (uniform 0.0–0.4), `batch_size` `{128,256,512}`. `seq_length` is **fixed at 72** in `fixed_configuration` and is not an axis.
3. **It was frozen before any Bayesian outcome** — committed once on 2026-08-22 (`c03fdb3`), never modified, and never executed; Proposal 1 executed 2026-09-01. But it was frozen for the v1 five-axis domain, which predates the 2026-08-26 six-axis scientific design mandate.
4. **It is NOT compatible with the v2 six-axis domain.** The v2 domain (`src/baseline/sweep_v2_six_axis_campaign.py` `SEARCH_DOMAIN_V2`, `DOMAIN_VERSION_V2="six_axis_q12_48_120_v001"`) adds a sixth axis `seq_length` on `q_uniform(min=48, max=120, q=12)` → grid `(48, 60, 72, 84, 96, 108, 120)`. A five-axis manifest with fixed `seq_length=72` cannot be reinterpreted as a six-axis realization without inventing the seq_length draw.
5. **No v2 six-axis random-control manifest, generator, arm, or trial exists.** `SEARCH_ARMS_V2 = frozenset({"bayesian"})`; `sweep_v2_six_axis_production_adapter.prepare_bayesian_proposal_v2` is the only legal arm; `sweep_v2_six_axis_review_analysis` carries the `"random_control"` string only for forward-compatible display and states explicitly that "v2 never runs a `random_control` arm". There is no `prepare_phase_b_sweep_v2_*_random_control_manifest.py`, no v2 random-control sbatch/trial script, and no v2 random-control test.

**Compatibility conclusion.** The repository contains only the historical Sweep-v1 five-axis random-control contract. There is **no** authoritative compatible six-axis 12-row random-control manifest. A missing compatible manifest is a scientific decision boundary, not an ordinary technical defect — so no scientific rows were generated or frozen, and the lean v2 random-control readiness implementation was **not** performed (its precondition, "all scientific choices already authoritative", is not met).

**Reusable components identified by the audit (subsequently adapted in Part 2).** Generator/manifest pattern: `src/baseline/sweep_v1_campaign.py` (`generate_random_control_rows`, `render_manifest`, `validate_manifest_rows`, canonical-JSON + SHA-256 helpers) and `scripts/prepare_phase_b_sweep_v1_random_control_manifest.py` (offline, no wandb/NH import). v2 identity/normalization: `sweep_v2_six_axis_campaign` (`canonical_hyperparameters_v2`, `configuration_id_v2`, `proposal_id_v2`, `trial_id_v2`, `normalize_seq_length_axis`, `SEQ_LENGTH_DOMAIN_V2`, `FROZEN_FIXED_CONFIGURATION_V2`). Config generation / execution spine / provenance / Slurm accounting: `sweep_v2_six_axis_execution` (`write_proposal_intake_provenance_v2`, `select_executor_mode_v2`, `execute_prepared_trial_v2`, `_review_records_v2`, `enrich_operations_slurm_accounting_v2`). Retry/recovery provenance: `sweep_v2_six_axis_retry` (`derive_exact_retry_identity_v2`, `build_bounded_wandb_tags_v2`). Validity adjudication: reuses `sweep_v1_execution._derive_validity` via the v2 module. Objective/Common-120: `src/baseline/fixed_support_contract_v2.py`, `config/stage1_v2_common120_fixed_support_artifact_identity_v001.json`, contract id `common120_raw_space_nse_v001`. Objective recovery: `sweep_v2_six_axis_objective_recovery`. Legacy non-W&B launch template: `scripts/run_sweep_v1_random_control_trial.py` + `scripts/run_sweep_v1_random_control_moriah.sbatch` (a random arm needs no W&B agent / controller join). The v2 random arm must NOT import or contact controller `wta85z3b` or any W&B optimizer.

**Approved campaign operating roadmap (recorded as approved operating POLICY — does not authorize execution).**

1. The random-control arm is **12 independent ordinary IID trials** — no optimizer-feedback dependency among them.
2. Once its scientific manifest and implementation are ready, the random arm is intended to execute as **three waves of four concurrent independent jobs**. Four-way random-trial concurrency is allowed.
3. **Bayesian optimization remains sequential:** each valid result must reach controller `wta85z3b` before the next proposal is requested.
4. **Detailed human review** is appropriate for the initial Bayesian proposals, currently through approximately **Proposal 4**. Human approval is not required before every ordinary later Bayesian trial.
5. After initial operational stability, use **bounded sequential Bayesian execution** toward the agreed scientific review boundaries.
6. **Formal Bayesian scientific reviews near 12 and 24 valid Bayesian observations.** New proposal issuance pauses near those boundaries, allowing only bounded overshoot from work already in flight.
7. **Common-120 (`common120_raw_space_nse_v001`, W&B metric `flashnh/common120_raw_space_nse_v001`) remains the sole optimizer objective.** Secondary metrics stay diagnostic and must not silently become optimizer inputs during the active campaign.
8. After approximately **3–5 materially different valid configurations**, compare screening-400 conclusions with full-population development-validation conclusions before permanently relying on or freezing screening-400 for final selection.

These roadmap decisions do not authorize random execution, Proposal 3, another W&B agent, training, or active-run changes.

**The scientific decision (RESOLVED — user authorized, Part 2).** The user authorized exactly one 12-row ordinary IID random-control manifest drawn only from the exact frozen v2 six-axis priors, under a one-shot realization rule. It is labelled **frozen after Bayesian observation 1**: it is scientifically independent of Proposal 1 (Proposal 1's configuration, trajectory, and objective play no part in its generation) but is *not* a fully pre-outcome-frozen prospective control, because the first Bayesian observation was already recorded when it was authorized.

**Deterministic seed derivation (recorded).** Namespace `stage1_phase_b_sweep_v2_six_axis_random_control_v001` → SHA-256 over its exact UTF-8 bytes = `0979f5d60aa60db35d6f0b5c248bfdf73ac24b734b1b5fcb9753db8517299ea2` → first 8 lowercase hex chars `0979f5d6` → unsigned base-16 integer **`158987734`**. Verified distinct from model Seed A (`967139`) and from the v1 manifest seed (`20260822`); an unexpected equality is a hard stop with no fallback (`derive_manifest_rng_seed` raises `SweepV2RandomControlError`). Recorded in `src/baseline/sweep_v2_six_axis_random_control.py` and guarded at import.

**One-shot IID realization rule (recorded).** `generate_random_control_rows_v2()` draws the 12 rows exactly once from `random.Random(158987734)` (Python MT19937). Per-row draw order is pinned to v2's authoritative committed axis tuple `_AXES_V2`: `learning_rate` (10 ** U(log10 1e-4, log10 1e-3)), `hidden_size` (choice {64,128,256}), `embedding_dropout` (U(0,0.4)), `output_dropout` (U(0,0.4)), `batch_size` (choice {128,256,512}), `seq_length` (`int(round(U(48,120)/12)*12)` — the committed W&B `q_uniform(48,120,12)` wire semantics, **not** a categorical prior; endpoints 48/120 carry half the mass of an interior grid value). No filtering, stratification, balancing, space-filling, Sobol/LHS, duplicate rejection, or coverage optimization. Natural duplicates would be structurally legal (the realized 12 rows happen to be all-distinct: 12 distinct `configuration_id`s).

**Frozen manifest (recorded).** `config/stage1_phase_b_sweep_v2_six_axis_random_control_v001_random_control_manifest.json`, canonical JSON (`json.dumps(..., sort_keys=True, separators=(",",":")) + "\n"`), byte SHA-256 **`59be6726b60863aeed1c25e86782bd7a5e1623434ce23c956e66eb54c527c095`** (sidecar `.sha256`, pinned as `RANDOM_CONTROL_MANIFEST_SHA256_V2`). 12 rows, `proposal_order` 1..12, `search_arm="random_control"`, `campaign_id="stage1_phase_b_sweep_v2_six_axis_common120_v001"`, `domain_version="six_axis_q12_48_120_v001"`, `objective_id="common120_raw_space_nse_v001"`, `fidelity_id="mf12x50000"` (Seed A `967139`, 12 epochs, `max_updates_per_epoch=50000`, `save_weights_every=1`), `support_contract_sha256="cb4ebe86afa501ef3d5929ead5b455f8df06e7d38b58ebf4148f8545fe6851ef"`. Realized `seq_length` distribution `{48:1, 60:4, 84:1, 96:3, 108:2, 120:1}`; `hidden_size` `{64:5, 128:4, 256:3}`; `batch_size` `{128:5, 256:3, 512:4}`.

**Offline implementation (Part 2, not committed).** New: `src/baseline/sweep_v2_six_axis_random_control.py` (static contract/generator/validator — no launcher, NH, or W&B; never contacts `wta85z3b`); `scripts/prepare_phase_b_sweep_v2_six_axis_random_control_manifest.py` (offline write/verify/audit); `scripts/run_sweep_v2_six_axis_random_control_trial.py` + `scripts/run_sweep_v2_six_axis_random_control_moriah.sbatch` (one Slurm allocation = one committed row; W&B-free; runtime provenance pinned before execution via the shared `sweep_v1_runtime_contract.verify_commit_and_interpreter` — canonical Moriah workdir/interpreter, explicit full-40-hex expected commit, exact HEAD match, tracked-tree-clean refusal that preserves untracked artifacts; explicit retry lineage resolved through `sweep_v2_six_axis_retry.derive_exact_retry_identity_v2` — `execution_generation=1` is attempt001 with no predecessor, later generations retain `configuration_id`/`proposal_id` and carry `retry_of_trial_id` into execution provenance and review records; reuse the identical prepare→write→execute→interpret spine as the Bayesian bridge); `tests/test_sweep_v2_six_axis_random_control.py` (13 generator/identity tests) + `tests/test_sweep_v2_six_axis_random_control_consumer.py` (25 focused tests exercising the production consumer vertically through the synthetic execution seam, the adapter refusal cases, Bayesian-front-door isolation, retry-lineage resolution, and the static runner/launcher contract). Additive edits: `sweep_v2_six_axis_campaign.SEARCH_ARMS_V2` broadened to `{"bayesian","random_control"}` (the production Bayesian front door stays pinned via `expected_arm="bayesian"`; the W&B sweep config is untouched); `prepare_random_control_proposal_v2` added to `sweep_v2_six_axis_production_adapter.py`; `run_prepared_trial_in_production_v2` added to `sweep_v2_six_axis_execution.py`; stale "v2 has no random_control arm" comments corrected in `sweep_v2_six_axis_review_analysis.py` and its test. Intended execution shape: **three waves of four concurrent independent jobs** (row indices 0–3, 4–7, 8–11), each writing to `output_root/<trial_id>` so concurrent roots never collide. Infrastructure retries retain `configuration_id`/`proposal_id` and advance only the trial attempt suffix (`execution_generation`).

**Status separation (explicit).**
- *Approved operating policy:* the 8 roadmap points above.
- *Frozen scientific manifest:* DONE — seed `158987734`, manifest SHA-256 `59be6726…`, frozen after Bayesian observation 1.
- *Offline implementation readiness:* DONE — generator + validator + non-W&B runner + one-row sbatch, with the production consumer focused-tested vertically (frozen row → prepare → persisted provenance → synthetic no-training execution seam → interpreted result) and the Moriah launcher runtime-provenance contract focused-tested (38 focused tests across the two random-control test files); full offline `prepare` still requires the external ~130 MB fixed-support contract + package root + splits and therefore completes only at Moriah launch time.
- *Live execution authorization:* NOT granted for the random arm, nor for any Bayesian proposal beyond Proposal 2 — which has since run to completion and been independently adjudicated VALID as Bayesian observation 2 (not a winner or promotion). Proposal 3 remains not launched and separately gated.

**Explicit non-authorization.** This entry does not authorize: regeneration or redrawing of the frozen random rows; W&B sweep creation or mutation; W&B agent execution; Slurm submission; random or Bayesian training; Proposal 3 or any further Bayesian proposal; sealed temporal-test / spatial-holdout / California access; Common-120 regeneration or requalification; search-domain, fidelity, metric, or promotion-policy changes; commit or push. Proposal 2 (`proposal_order=2`, job `46013192`, W&B run `y4przhjs`) was left completely undisturbed; controller `wta85z3b` was not contacted. Focused tests: `tests/test_sweep_v2_six_axis_random_control.py` (13 passed) + `tests/test_sweep_v2_six_axis_random_control_consumer.py` (25 passed) plus regression on `test_sweep_v2_six_axis_review_analysis.py`, `test_sweep_v2_six_axis_execution.py`, `test_sweep_v2_six_axis_production_adapter.py`, `test_sweep_v2_six_axis_config.py`, `test_sweep_v2_six_axis_retry.py`, `test_sweep_v2_six_axis_objective_recovery.py`, `test_sweep_v2_six_axis_wandb_bridge_foundation.py`, `test_sweep_v2_six_axis_wandb_bridge_production_seam.py`, `test_sweep_v1_static_campaign_foundation.py`, `test_policy_v2_six_axis.py` (256 passed for that combined set), and on `test_sweep_v1_exact_retry_production_launcher.py`, `test_sweep_v1_runtime_contract.py`, `test_sweep_v1_retry.py` (68 passed) for the reused runtime-provenance / retry contract; `git diff --check` clean. The Codex REQUEST-CHANGES follow-up repair (2026-09-01, same commit base) then added the vertical producer/consumer + Moriah launcher runtime-provenance contract + explicit retry-lineage plumbing and their focused tests; the manifest bytes, SHA-256, seed, and 12 rows are unchanged.

## 2026-09-01 — V2 production Proposal 1 adjudicated VALID and recorded as Bayesian observation 1; proposed axes are one observation, not a selected/promoted configuration; frozen fidelity not reopened

**[CLOSED — ONE VALID v2 PRODUCTION TRIAL]** The first real Bayesian proposal from the accepted v2 production controller `wta85z3b` (`omri-porat1-huji/flashnh-stage1/wta85z3b`) executed to completion at exact commit `6eb301e8206f8c4b6d76a33be5ed888966ef5afd` (Slurm job `46009495`, `catfish`/`catfish-03`, one NVIDIA L4, `COMPLETED 0:0`, `03:32:42`; W&B run `txd8mer9` `revived-sweep-1`, terminal state `finished`; controller run count exactly 1). A read-only scientific + operational completion review adjudicates it **VALID**.

**Decisions recorded.** (1) **Proposal 1 is VALID and counts as v2 Bayesian observation 1.** `execution_provenance.json` `provenance_stage=execution_status=VALID`, `artifact_identity_status=PASS`, `prepare_status=PASS`, `objective_eligible=true`; `review_records.json` `trial_summary.workflow_status=pass`, `failure_category=null`. (2) **The six proposed axes are one Bayesian observation — not selected, promoted, or winning hyperparameters:** `learning_rate=0.00026727483167567154`, `hidden_size=256`, `embedding_dropout=0.1203941658997464`, `output_dropout=0.3947011313328828`, `batch_size=512`, `seq_length=60` (raw controller axes ≡ normalized ≡ W&B run config; `seq_length` on the `q_uniform(48,120,12)` grid). (3) Identities: `proposal_order=1`, `execution_generation=1`, `configuration_id=sweep_v2_cfg_26daf2a8f293007dd60b`, `proposal_id=…__bayesian__proposal001`, `trial_id=…__proposal001__sweep_v2_cfg_26daf2a8f293007dd60b__mf12x50000__seedA967139__attempt001`; Seed A `967139`; fidelity `mf12x50000`; `evaluation_scope=development_validation_2024_only`; `sealed_scope=false`. Launch-manifest canonical SHA-256 `09a324f670575f62618479b6a7270344064a9b3fe994780c283965b05838688a`; generated NH config SHA-256 `dc8711c17f473439284fab9bec0877dba42a865fafeaf765ab96597ac8e14607`; fixed-support contract SHA-256 `cb4ebe86afa501ef3d5929ead5b455f8df06e7d38b58ebf4148f8545fe6851ef`; screening SHA-256 `d4395d93ebc567cf09e149c0121463d75cf4f7ecc02c07a7c4a7999763baa372` (400 basins). (4) **Execution coverage is complete:** exactly 12 checkpoints and 12 optimizer states (epochs 1–12); exactly 50,000 optimizer updates per epoch (cumulative 600,000), all cap-compliant; screening PASS with 400/400 basins for epochs 1–12; fixed-support Common-120 and natural-support raw-space evaluations complete and finite for epochs 1–12; `blocked=false`, `stopped=false`, `final_status=monolithic_training_and_screening_complete`. (5) **Objective `0.36621751839318467` at epoch 1** (`best_epoch=1`); **epoch-12 fixed-support score `0.3471378366389816`**; **best-minus-final `0.01907968175420305`**; `late_gain_10_to_12=0.0`. The fixed- and natural-support 12-epoch trajectories were complete and closely aligned (identical epoch-1 peak; max epoch-wise divergence ≈ 0.0047). (6) **W&B publication equality:** the published metric-history value `flashnh/common120_raw_space_nse_v001 = 0.36621751839318467` exactly equals the Flash-NH authoritative objective; `flashnh/valid=true`; `flashnh/trial_id` matches; `scan_history` 1 row (`run.summary` float repr `0.3662175183931846` is display only). (7) **The epoch-1 peak / declining trajectory is a scientific observation about this one configuration and is NOT grounds to change the frozen Seed-A 12-epoch × 50,000-update fidelity after a single configuration.** The frozen fidelity remains as designed. (8) No sealed-scope access, no retry (`retry_history=[]`), no second run, and no Proposal 2 occurred within this trial.

**Evidence.** Compact untracked local bundle `.scratch_local/v2_proposal1_completion_6eb301e_46009495_20260901T072828Z/` (33 files incl. `SHA256SUMS.txt`; `adjudication_summary.{json,md}`, `attempt_execution_provenance.json`, `attempt_review_records.json`, `optimizer_update_counts.json`, checkpoint/optimizer/eval inventories, `trajectory_table.txt` + `proposal1_screening_nse_trajectory.png`, `phase3_wandb_readonly.json`, `BOUNDARY_AND_COUNTERS.txt`). Local checksum verification 32/32 OK, 0 failures; Moriah mirror re-verified ALL OK. Untracked per `docs/repo_policy.md`. One accepted non-blocking limitation: `gpu_hours` / `operations.slurm_job_id` / `operations.slurm_state` are `null` in `review_records.json` (post-run Slurm-accounting enrichment not attached); job identity is independently fixed by W&B `wandb-metadata.json` and `sacct`.

**Not done in this closure.** Proposal 1 is not a winner and its configuration is not promoted or selected. This documentation commit records only the two documentation files. Proposal 2 is executed as a separate step immediately following this commit (`proposal_order=2`, `execution_generation=1`, one new strict `mode=production` manifest, one `wandb agent --count 1`, one L4 allocation, unchanged frozen fidelity, detached after startup verification); its startup evidence is untracked and not committed.

## 2026-08-31 — V2 production W&B controller `wta85z3b` registered once and accepted; Proposal 1 remains a separate execution step

**[CLOSED — ONE REAL PRODUCTION CONTROLLER REGISTRATION]** Under explicit user authorization, exactly one real `wandb.sweep(...)` call registered the reusable v2 six-axis production controller `wta85z3b` (`omri-porat1-huji/flashnh-stage1/wta85z3b`) at exact commit `ddd39646599b610ebb850f6b7207ec7ef6356ba3`. CPU Slurm job `46009198` (`glacier`/`glacier-25`, 2 CPU / 8 GB, `COMPLETED 0:0`, `00:01:24`) ran the single `register-controller` invocation (exit 0; `Create sweep with ID: wta85z3b`; `status=success` receipt); no retry, no resubmission.

**Decisions recorded.** (1) `wta85z3b` is **accepted as the v2 production controller**. A live read-only `wandb.Api()` verification (no mutation) confirmed method `bayes`, metric `flashnh/common120_raw_space_nse_v001`/`maximize`, program `scripts/run_sweep_v2_six_axis_wandb_bridge.py`, static command `["${interpreter}", "${program}"]`, the authoritative committed v2 six-axis domain with no extra parameters, controller state `PENDING`, and run count exactly zero (16 identity/config + 7 per-parameter checks, all pass). (2) Registration is **not** proposal creation: no manifest, proposal, `proposal_order`/`execution_generation` reservation, `wandb agent`, W&B run, GPU, training, evaluation, or objective value exists or occurred. Proposal 1 is a distinct execution step. (3) No automatic registration retry is permissible — the external controller now exists and all future v2 production work must use this exact sweep and its committed static command. (4) The frozen v1 sweep `4x3btz2s` and disposable rehearsal sweep `oz5p4csb` were not contacted or queried and remain forbidden for every production path.

**Evidence.** Slurm job `46009198`. Compact untracked local bundle `.scratch_local/v2_production_controller_registration_ddd3964_20260831T131151Z/moriah_evidence/` (29 files; `SHA256SUMS.txt`; independently re-verified 29/29, 0 failures), untracked per `docs/repo_policy.md`. Key records: `controller_registration_receipt.json`, `phase3_registration_stdout.json` (+ exit code `0`), `phase4_readonly_verification.json` (`all_checks_pass: true`, `2026-08-31T13:17:06Z`, wandb 0.28.1), `phase5_boundary_statement.json` (`wandb_sweep_calls_attempted = 1`, all other counters `0`/`null`), `sacct.txt`.

**Not done in this registration/closure.** No production manifest, proposal, `proposal_order=1` reservation, `wandb agent`, W&B run, GPU allocation, training, inference, scientific evaluation, objective publication, Common-120 regeneration, sealed-set access, controller cancellation/retirement, or contact with `4x3btz2s`/`oz5p4csb`. This closure's commit records only the two documentation files.

## 2026-08-30 — V2 disposable CPU W&B one-agent rehearsal SUCCEEDED and CLOSED; proposal 900001 / run `8aj5ldxq` are operational-only; production planning and disposable-sweep retirement remain separate user gates

**[CLOSED — OPERATIONAL REHEARSAL, NO SCIENTIFIC RESULT]** Building on the registration entry below (same date), exactly one authorized `wandb agent --count 1` was run against the disposable controller `omri-porat1-huji/flashnh-stage1/oz5p4csb` (registered manifest semantic SHA-256 `329125a56e29c22e1ccfe12b58d88400ba1be8cb1d694bffd8af44e3ffae2797`, execution commit `1dc27478ff9b9e49edaecfd8da689b8d1ca324bd`, clean detached HEAD on Moriah, CPU-only launcher). It consumed exactly one disposable proposal (`proposal_order = execution_generation = 900001`), created exactly one W&B run (`8aj5ldxq`, `usual-sweep-1`, terminal state `finished`), passed the manifest-driven proposal-intake and preparation path (`prepare_only=true`, `prepare_status=PASS`, final provenance stage `executor_mode_selected`, executor mode `monolithic`, `objective_score=null`), and stopped before training (`flashnh/rehearsal_stopped_before_training=true`; run summary also carries `flashnh/trial_id`; zero history rows).

**Decisions recorded.** (1) The one-agent disposable rehearsal succeeded and is **closed**. Exact-commit pinning, the pre-agent read-only identity/configuration gate, `--count 1`, absolute no-retry after first agent contact, and stop-before-training all worked as intended. (2) The disposable proposed axes (`batch_size=256`, `embedding_dropout=0.24247769820599452`, `hidden_size=128`, `learning_rate=0.00028381477465092816`, `output_dropout=0.13121329813483537`, `seq_length=84`) and run `8aj5ldxq` are **operational evidence only** — they show a well-formed six-axis proposal crossed the controller/bridge interface, and cannot be treated as a scientific result, selected hyperparameters, a candidate, or an optimization observation, and are not reusable. (3) Passing this rehearsal proves the operational controller/manifest/bridge/stop path only; it does **not** authorize production. The next scientific/operational phase concerns a separately designed v2 production controller and execution path; no v2 production sweep has been created or registered, and the frozen v1 sweep `4x3btz2s` is not the next production target and must not be contacted. (4) Retirement of the disposable sweep `oz5p4csb` remains a separate user gate; it is not performed or authorized here.

**Evidence.** Pre-agent gate: Slurm job `45986888` (`glacier`/`glacier-04`, 2 CPU / 8 GB / no GPU, `COMPLETED 0:0`, `00:00:44`) — fresh read-only sweep state `PENDING`, run count 0, exact controller identity, qualified helper SHA-256 `0140bf60156652a2d67b6f7a2e9e350a4d9acdf662063abdf7e454b011a2b9f0`. Mutation-boundary rehearsal: Slurm job `45986910` (`glacier`/`glacier-37`, same resources, `COMPLETED 0:0`, `00:01:19`). Authoritative post-run read-only verification: Slurm job `45986928` (`glacier`/`glacier-33`, `COMPLETED 0:0`, `00:00:36`) — `POSTRUN_VERIFIED`, 24/24 checks pass, sweep run count exactly 1, run `8aj5ldxq` `finished`, W&B trial ID equals local intake (`…__proposal900001__…__attempt900001`), no placeholder metric (`qualification/rehearsal_placeholder_metric_v2`) or production objective (`flashnh/common120_raw_space_nse_v001`) value in summary or history. Strict rehearsal manifest byte SHA-256 unchanged at `78abc0483b314843d85d772d3be0338270fc0f55c1c9b7d5643a5cfa411f735c` throughout. Portable, checksum-verified local bundle `.scratch_local/v2_real_disposable_rehearsal_1dc2747_20260830T121257Z/` (authoritative manifest `PORTABLE_SHA256SUMS.txt`, 88 entries; portability note `EVIDENCE_PORTABILITY.md`); portable archive `.scratch_local/v2_real_disposable_rehearsal_1dc2747_20260830T121257Z_portable.tar` (SHA-256 `df2deffadc073d1f96df36a0becf133e1e7b5e0e4c470be8ec3bc8caab7d6460`, 378,880 bytes), fresh-extraction validation 88/88 OK / 0 missing / 0 mismatch / no unexpected files / safe single top-level directory. Untracked per `docs/repo_policy.md`.

**Non-blocking evidence-hygiene incidents.** (a) Verifier-only repair: the first post-run verifier (job `45986919`) established the major result but skipped the local/W&B trial-ID equality check because its local-discovery predicate expected the earlier `proposal_intake` stage after the single `execution_provenance.json` had already advanced in place to `executor_mode_selected`. Only the read-only verifier was repaired (`v2_real_rehearsal_postrun.sbatch` `4692989e…` → `c65f498c95211de2e86c41bf4c1c9899a3bace2683b3ed755a25c274884f38e1`) and rerun as job `45986928`; the `wandb agent` was not retried. (b) Portable-evidence repair: the original Moriah post-run evidence used a deeply nested trial path not portable to the local Windows transfer; the six authoritative output-root payloads were preserved byte-identically under `evidence_postrun_45986928/output_root_snapshot_flat/` (six hashes match both the original long-path entries and `output_root_SHA256SUMS.txt`), and a complete authoritative `PORTABLE_SHA256SUMS.txt` plus `EVIDENCE_PORTABILITY.md` were added beside the bundle. Original historical checksum manifests and all evidence payload bytes were unchanged by either repair.

**Not done / not authorized.** No second agent, proposal, or run; no training, model inference, or scientific evaluation; no sealed-scope access (`sealed_scope=false`, `evaluation_scope=development_validation_2024_only`); no production-objective or placeholder-metric value published; no frozen-v1 or production-sweep contact; no new sweep registration; no manifest mutation; Common-120 was loaded and validated through the intended read-only code path only, with no regeneration, copying, rewriting, requalification, training, or scientific evaluation (filesystem non-mutation is inferred from the inspected read-only code path, not independently syscall-audited during this rehearsal); no disposable-sweep retirement. This documentation closure made no commit, push, or code/test/config/manifest/evidence change.

## 2026-08-30 — V2 disposable rehearsal sweep registration performed once and closed; disposable sweep `oz5p4csb` is the durable rehearsal controller; CPU agent rehearsal remains a separate gate

**[CLOSED — ONE REAL DISPOSABLE REGISTRATION]** Registration was intentionally separated from agent execution. Exactly one real, disposable v2 W&B sweep registration was authorized and performed: a single `wandb.sweep(...)` call created the non-production disposable sweep `oz5p4csb` (`omri-porat1-huji/flashnh-stage1/oz5p4csb`) over the frozen v2 six-axis domain, with the optimizer metric fixed to the rehearsal placeholder `qualification/rehearsal_placeholder_metric_v2` rather than the production objective `flashnh/common120_raw_space_nse_v001`. The strict rehearsal manifest `.scratch_local/sweep_v2_six_axis_wandb_bridge_launch_manifests/rehearsal_v001.json` (semantic checksum `329125a56e29c22e1ccfe12b58d88400ba1be8cb1d694bffd8af44e3ffae2797`, loader-validated) together with sweep id `oz5p4csb` identify this disposable controller. `proposal_order=execution_generation=900001` reserve a non-scientific rehearsal namespace bound to this manifest — not Proposal 1, not a production execution generation, not countable toward the HPO budget.

**Decisions recorded.** (1) Post-registration verification used read-only W&B access only; the final preserved read-only record shows `oz5p4csb` `PENDING` with zero runs and an exact method/metric/program/command/parameter match against the manifest, with the frozen v1 production sweep `4x3btz2s` not targeted. (2) Registration success is **not** CPU-rehearsal success; real CPU agent execution against this sweep remains a separate user gate, contingent on first inspecting this registration evidence. (3) No automatic registration retry is permissible: the external sweep already exists, so any future v2 rehearsal work must use this exact sweep and manifest rather than registering another disposable sweep. (4) The initial in-job read-only verifier exited non-zero on a JSON-parsing defect (it parsed the entire receipt file, which carries two W&B stdout banner lines before its JSON object); a separate read-only job with corrected parsing then completed verification successfully. This was a non-blocking verification-harness defect after an already-successful registration, with no W&B mutation by either verifier; the orchestrator recorded `phase3_rc=1 phase2_rc=0` and did not retry.

**Evidence.** Slurm jobs `45983252` (registration + failed initial verify; `glacier`/`glacier-34`, `COMPLETED 0:0`, `00:00:46`, 2 CPU / 8 GB) and `45983254` (successful read-only re-verification; `glacier`/`glacier-34`, `COMPLETED 0:0`, `00:00:08`, 2 CPU / 8 GB). Local compact evidence, independently checksum-verified and untracked per `docs/repo_policy.md`: `.scratch_local/v2_real_disposable_registration_1dc2747_20260830T074704Z/` (22/22 files) and the follow-up bundle `.scratch_local/v2_real_disposable_registration_followup_1dc2747_20260830T082056Z/` (14/14 files, adding final authoritative `sacct` and the preserved raw first-verifier failure material). One accepted limitation: the pre-repair verifier body was not byte-snapshotted before it was edited in place; its SHA-256, the failing source line quoted in the preserved traceback, the triggering receipt, and the exit-code records are preserved. The follow-up collection used a transient helper script under `$HOME` on Moriah rather than a project-local location; it was removed after use and did not affect Git, credentials, evidence integrity, or user material — a non-blocking process deviation. Corrective rule: future Flash-NH temporary helpers must be created only in project-local `tmp/` or `.scratch_local/`.

**Not done / not authorized.** No `wandb agent`; no W&B run; no proposal consumed; no manifest `output_root` created; no training, evaluation, or objective publication; no production sweep or production training. Common-120 runtime non-access is inferred from the recorded descriptor-only registration, preflight, and read-only code paths; it was not syscall-audited. The documentation task that records this closure made no commit, push, or code/test/config/manifest/evidence change.

## 2026-08-29 — V2 Rehearsal Preparation A/B CLOSED; registration seam Linux/offline-qualified

**[CLOSED — IMPLEMENTATION + LINUX/OFFLINE QUALIFICATION ONLY]** The disposable CPU W&B rehearsal preparation milestone named by the 2026-08-28 Common-120 entry below was split into two increments for reviewability: **Preparation A** (`ba1f130274f82c6b76506f5b5c99d9155b4c4c1d`, offline v2 six-axis rehearsal sweep config serialization) and **Preparation B** (`aa0a2bd93c1163a348893a20b72acfc85a737456`, the registration seam). Both reuse the authoritative v2 rehearsal config builder, `build_wandb_bridge_rehearsal_sweep_config_v2` (`src/baseline/sweep_v2_six_axis_config.py`), and the strict manifest writer (`src/baseline/sweep_v2_six_axis_wandb_bridge_manifest.py`) rather than introducing parallel implementations — Preparation A exposes/serializes the authoritative builder through the maintained `scripts/build_sweep_v2_six_axis_wandb_bridge_rehearsal_sweep_config.py` serializer script, and Preparation B calls the authoritative builder directly as part of the registration seam; both take committed-descriptor authority for the Common-120 fixed-support and screening identities from the one authoritative record, `config/stage1_v2_common120_fixed_support_artifact_identity_v001.json`. The manifest's `fixed_support_contract_sha256` binding deliberately uses the descriptor's *internal canonical* contract checksum (`artifact["internal_canonical_contract_sha256"]`), not the external serialized-file checksum — the two remain distinct identities per the Common-120 entry.

Preparation B's registration seam (`register_v2_rehearsal_sweep`) prospectively assembles and fully validates a strict rehearsal manifest, and runs the runtime/Git/interpreter contract, before making any external call. The manifest itself publishes via a genuinely atomic no-clobber writer (`os.link`, never `os.replace`, same-directory temp file). On a post-registration partial failure — an unsafe returned sweep id, or a manifest-write failure after a real sweep was already created — the seam raises `RegistrationPartialFailure` carrying a `partial_failure` receipt that preserves the returned W&B sweep identity and explicitly instructs against automatic retry; it never retries registration itself.

**Linux/offline qualification (evidence and verdict).** A focused, fully offline qualification of Preparation B ran on Moriah at commit `aa0a2bd…` (Slurm job `45972985`, `v2_prepb_linux_qual`, `glacier` partition, host `glacier-31`, `COMPLETED`, exit `0:0`, elapsed `00:00:41`, canonical Moriah Python 3.11.15): the exact six-file focused pytest suite (123 passed, 0 skipped/failed/errors, 29.61s); a direct filesystem `os.link` probe on the same Moriah device proving both the no-clobber `FileExistsError` case (destination and source bytes unchanged) and the successful independent-link case (survives source-pathname removal); and a `--preflight-only` run with a poison `wandb.py` first on `PYTHONPATH` (raises `ImportError` immediately), exit 0, empty stderr, confirming `run_preflight()` never imports or contacts W&B. The preflight receipt confirmed `stop_before_training=true`, `max_agents=1`, the rehearsal placeholder metric, the real six-axis identity, the intended manifest absent both before and after, and `proposal_order=execution_generation=2147483647` used strictly as an in-memory `--preflight-only` sentinel, never a reservation. Independent evidence review returned verdict `APPROVE LINUX QUALIFICATION FOR DOCUMENTATION`.

Local evidence (project-relative, untracked per `docs/repo_policy.md`, independently checksum-verified): `.scratch_local/v2_rehearsal_preparation_b_linux_qual_aa0a2bd_20260829T111017Z/` (23/23 files) and a narrow follow-up bundle `.scratch_local/v2_rehearsal_preparation_b_linux_qual_followup_aa0a2bd_20260829T112702Z/` (9/9 files) adding raw `sacct` scheduler accounting for job `45972985` and current Git identity/reflog/ancestry evidence, collected after an independent reviewer flagged that the original bundle lacked a raw `sacct` record. Accepted, non-blocking evidence-scope limitations — the original bundle's earliest snapshot begins after Moriah's Git fast-forward to `aa0a2bd…` rather than before it; reflog/ancestry evidence collected in the follow-up is retrospective corroboration, not independent proof of the historical sync command; external `output_root` non-creation is inferred from the inspected `run_preflight()` code path rather than a direct before/after filesystem probe; Common-120 artifact non-access is inferred from the recorded command and descriptor-only preflight path rather than syscall auditing — are recorded in the follow-up bundle's `evidence_limitations.md` and do not contradict the qualification.

**Authorization boundary.** PREPARATION A — CLOSED. PREPARATION B — CLOSED. LINUX/OFFLINE QUALIFICATION — CLOSED. No real W&B sweep, run, manifest, proposal, or agent exists for v2; the poisoned import guard proved that no W&B import or contact occurred. REAL DISPOSABLE REGISTRATION — NOT PERFORMED / SEPARATELY GATED: the next milestone is one real, disposable v2 W&B sweep registration only, requiring separate explicit authorization. Real registration does not itself authorize agent execution. REAL CPU AGENT REHEARSAL — NOT PERFORMED / SEPARATELY GATED, a distinct later gate contingent on inspecting real registration evidence first. PRODUCTION SWEEP/TRAINING — NOT AUTHORIZED. No agent, Common-120 runtime access, training, evaluation, or objective publication occurred.

## 2026-08-28 — V2 Common-120 artifact identity recording CLOSED; authoritative external fixed-support contract independently qualified

**[CLOSED — ARTIFACT IDENTITY RECORDING ONLY]** The authoritative v2 fixed-support artifact is the external, untracked 130,593,808-byte file `data/fixed_support_contracts/stage1_v2_common120_fixed_support_contract_v001.json`. Its tracked, machine-readable identity record is `config/stage1_v2_common120_fixed_support_artifact_identity_v001.json`; no loader, registry, bridge behavior, or training behavior is introduced by this entry. The external serialized-file SHA-256 is `99fdc4f44768779661a735d25974e25b64a5d7f502ec8ffa4c5d2bab42bc34d6`; the distinct internal canonical fixed-support-contract SHA-256 is `cb4ebe86afa501ef3d5929ead5b455f8df06e7d38b58ebf4148f8545fe6851ef`.

The record binds schema `flashnh_stage1_v2_fixed_support_contract` v2 / contract `common120_raw_space_nse_v001` / optimizer metric `flashnh/common120_raw_space_nse_v001` to all five package/split digests, the exact 400-basin screening digest, and the complete packaged MRMS+RTMA gap artifact digest. Membership is `history_valid_120(t)` plus validation/lead boundaries and finite `qobs_mm_per_h_lead06[b, t]`; the lead-06 target is already aligned to `t + 6h`, with no second shift. MRMS-only support is invalid. Independent qualification job `45968946` reconstructed every membership from raw package inputs: 0 basin mismatches, 0 timestamp mismatches, matched package/split/screening/gap bindings and both artifact checksums, and unchanged inode/size/mtime/checksum before versus after; verdict `QUALIFIED`.

Support accounting is 8,537 already-history-valid Common-120 issue times, 3,347,893 retained finite-qobs basin–issue-time memberships, range 6,983–8,537, median 8,489.5, mean 8,369.7325, and zero zero-support basins. Thus approximately 98.04% means `3347893 / (400 * 8537)`: mean retained finite-qobs membership within the already-history-valid Common-120 grid, **not** a 72h-to-120h sequence-length retention ratio. Regression (`45959191`: 503 passed, 0 failed, 0 errors), full 400-basin no-write dry run (`45959214`), and one controlled generation (`45959430`) are recorded in the descriptor alongside local evidence-bundle references.

No v2 disposable W&B sweep exists; no real v2 rehearsal, production sweep, production proposal request, proposal-order ledger, training, or objective publication has occurred. The next milestone is **V2 DISPOSABLE CPU W&B REHEARSAL PREPARATION**, separately gated. This documentation/metadata closure does not authorize W&B contact.

## 2026-08-26 — Sweep-v1 attempt005 (Proposal 1, generation 5) adjudicated VALID and closed as the countable trial; STARTED-stage provenance-clobbering root cause repaired; objective-recovery mechanism operationally qualified via a real, disposable, online exercise

**[CLOSED — TECHNICAL AUDIT + REPAIR, ONE SCIENTIFIC RESULT ACCEPTED]** Following attempt005's real Sweep-v1 production execution (Slurm job `45948021`, `trial_id` ending `__attempt005`, `execution_generation=5`, `retry_of_trial_id=attempt001`, W&B run `ardib08c` under production sweep `4x3btz2s`, `proposal_order=1`), this task performed a completion audit, root-caused and repaired a provenance-completeness defect discovered by that audit, and operationally qualified the objective-recovery mechanism the repair's design depends on. Canonical starting commit for this task: `929a4028ad8060ed0a5a7740febf9e2d71190f22`.

**Section A — completion audit; corrected log-scan method; independent full recomputation; no contradiction found.** The 58,325,800-byte `slurm_45948021.out` is tqdm-carriage-return-heavy (643,478 CR vs. 124 real newlines); a naive line-based `grep -c` over it silently scans almost none of the file. Corrected by CR→newline normalization into a 643,602-line file, verified against positive controls (`Epoch`=634,251, `Validation`=6,431) matching an independently obtained prior count. Re-scanned for `Traceback`/CUDA errors/OOM/NaN/`Inf`/exception/failed/killed/cancelled/timeout: 0 matches for every category on the full-coverage file, corroborated by the clean 23,302-byte NH bridge `output.log` (12 clean epoch/validation cycles). All 12 epochs' checkpoints and `validation_results.p` pickles were independently re-evaluated via the repository's own qualified `raw_space_metrics_for_run_period` helper (no parallel reimplementation): every epoch's recomputed median raw-space NSE matches the persisted value exactly, 400/400 basins requested and evaluated with 0 area-excluded and 0 non-finite simulations at every epoch, and the evaluated basin-id set is identical across all 12 epochs (a stronger empirical outcome than the frozen per-epoch population-accounting policy requires, not a claim that the policy itself mandates cross-epoch identity). `best_epoch=1`, `objective_score=0.391678449944578`, exactly reproducing the original determination. **Gate result: NO CONTRADICTION FOUND — VALID stands.** attempt005 is the sole countable Proposal-1 result (1/36 valid Bayesian trials); attempts 1–4 remain non-countable per their existing adjudications (attempt001 INVALID; attempt002/003 non-countable operational failures; attempt004 a non-scientific rehearsal).

**Section B/C — evidence preservation and closure supplement.** attempt005's original evidence (`config.yaml`, `execution_provenance.json`, `generation_manifest.json`, `review_records.json`, `pilot_orchestration_state.json`, both Slurm log streams, the NH bridge log, and the basin-list files) was never modified; its checksums were recorded from direct byte-verified `scp -O` transfers against the remote originals. A post-hoc, explicitly-labeled closure supplement (`attempt005_closure_provenance_supplement_v001`, project-local/untracked per repo policy) records the audit above; it is explicit that it is not original evidence and never backfills or edits any original record.

**Section D/E — provenance-completeness defect found and repaired (no scientific impact).** attempt005's terminal `execution_provenance.json` is missing top-level `retry_history` and `executor_mode` keys, despite the durable intake/preparation envelope having already accumulated them earlier in the trial's lifecycle. Root cause: `execute_prepared_trial` (`src/baseline/sweep_v1_execution.py`) wrote a brand-new provenance dict literal at both the STARTED execution stage and the terminal stage, instead of merging onto the pre-existing durable envelope — silently dropping any field the envelope already held that the literal didn't re-list. Confirmed **no scientific impact**: neither field is read by `_derive_validity`, `_population_accounted`, or any other correctness/eligibility check; every field that does matter for validity (`objective_score`, `preparation_record`, `result`, trial/campaign/configuration identity, checksums) was present and correct throughout. **Repair:** `enrich_layer_b_provenance` gained an `_LAYER_B_IDENTITY_KEYS` tuple and now merges onto any existing on-disk record (hard-failing on an identity-key conflict) instead of doing a bare trial_id-only check; `execute_prepared_trial` gained an `executor_mode` parameter and now routes both its STARTED-stage and terminal writes through this merge helper, including `retry_of_trial_id` in the STARTED merge only when non-`None` (so a default `None` can never clobber an already-recorded value); `run_prepared_trial_in_production` threads `executor_mode` through. This is a **prospective fix for future trials**; it does not and cannot retroactively add the two missing keys to attempt005's already-terminal, immutable `execution_provenance.json`, which remains untouched. New `tests/test_sweep_v1_provenance_lifecycle_repair.py` (20 tests) walks the real production lifecycle (intake → prepared → prepared_with_config → STARTED → terminal), reproduces attempt005's exact shape, and checks 14 automatable properties of the repair.

**Section F — objective-recovery mechanism operationally qualified via a real, disposable, online exercise (never touching attempt005 or production identities).** The recovery module (`src/baseline/sweep_v1_objective_recovery.py`) initially enforced only 3 of its 8 required rejection properties. At the user's explicit direction ("author the missing checks, then qualify"), the 5 missing checks were implemented: `assert_recovery_eligible` now rejects non-VALID records, incomplete records, records missing `generated_nh_config_sha256`, and non-finite `objective_score`; a new changed-objective idempotency guard refuses to silently republish a different payload for an already-published trial identity; `sweep_v1_retry.assert_matches_pinned_identity` (shared by the exact-retry bridge and the recovery module) now also checks `wandb_run_id`. A disposable qualification script and Moriah CPU-only `.sbatch` launcher (`glacier` partition, no GPU, no NeuralHydrology training, never `wandb.agent()`) were built to exercise the real `recover_and_publish_objective` path against one disposable W&B sweep/run created solely for this purpose, hard-refusing at the top of the script if either identity ever matched the production sweep (`4x3btz2s`) or run (`ardib08c`).

The first two qualification attempts on Moriah found and fixed two real bugs, precisely validating the standing "fake-backend ≠ qualified" concern: (1) job `45950131` hit `AttributeError: 'Run' object has no attribute 'sweepId'` — real `wandb.apis.public.Run` objects expose sweep association as `run.sweep` (a `Sweep` object with `.id`, or `None`), not `run.sweepId`; the local unit-test double had the identical bug baked in (`self.sweepId = sweep_id`), so it never caught this — only the real online qualification did. Fixed in production code and in the test double (now mirroring the verified real shape via a `_FakeSweep` object), committed `aaa630c`. (2) job `45950133` reported `overall_pass=false` despite every individual check genuinely passing, because the qualification harness's post-publish readback reused the same `wandb.Api()` instance as an earlier pre-publish check; `wandb.Api()` caches `Run` objects per instance by path, so the reused call silently returned stale pre-publish data rather than a fresh server round-trip (the actual write, made via `recover_and_publish_objective`'s own internal, separate `wandb.Api()`, was independently verified to have persisted correctly). This was a harness-only bug, not a production defect; fixed by constructing a fresh `wandb.Api()` for the readback, committed `7fe4a2b`.

**Qualification result: PASS (Moriah Slurm job `45950255`, `glacier-33`, commit `7fe4a2b`, 16s elapsed).** `qualification_evidence.json` records `overall_pass: true` and all 7 of Section F's numbered exercise requirements satisfied: (1) synthetic immutable VALID record, finite objective; (2) correct run/sweep identity matching against disposable ids `wandb_run_id=0q8tet2b`/`wandb_sweep_id=zff7w0x3`; (3) one successful publication, independently confirmed via a genuine post-publish summary readback; (4) idempotent repeated reconciliation, proven with `wandb` poisoned out of `sys.modules` (`status: "already_published"`, no wandb import needed); (5) all 8 negative cases correctly rejected with the expected error types/messages (`invalid_record`, `incomplete_record`, `non_finite_objective`, `mismatched_sweep`, `mismatched_wandb_run_id`, `changed_objective`, `missing_source_hashes`, `contradictory_identity`); (6) durable evidence (10 files) written to Moriah; (7) clean W&B finish (`run_state_after_finish: "finished"`). Production identities `ardib08c`/`4x3btz2s` appear only in the evidence's `never_used_production_*` fields, confirming they were never touched. Evidence was subsequently transferred to and independently verified locally (SHA-256, 10/10 files match byte-for-byte against the Moriah-computed checksums) — the recursive `scp -O -r` transfer initially failed/stalled on 3 of 10 files due to a local Windows `MAX_PATH` (260-character) limit under the deep scratchpad destination path, unrelated to Moriah, network, or SSH; resolved by transferring into a short local path.

**attempt005's original objective was never republished or changed by Section F.** The qualification exercised only the disposable sweep/run created for that purpose; `objective_score=0.391678449944578` (epoch 1) remains attempt005's accepted, unmodified, un-republished objective.

**Commits.** `f8fc878` ("Repair STARTED-stage provenance clobbering; harden objective recovery"; 7 files, 1282 insertions/57 deletions: `sweep_v1_execution.py`, `sweep_v1_objective_recovery.py`, `sweep_v1_retry.py`, `test_sweep_v1_objective_recovery.py`, new `test_sweep_v1_provenance_lifecycle_repair.py`, new `scripts/sweep_v1_objective_recovery_qualification.py` + `.sbatch` launcher; 98 targeted tests passing at commit time). `aaa630c` ("Fix wandb.Api() Run.sweepId AttributeError in objective recovery"; 2 files; 118 targeted tests passing). `7fe4a2b` ("Fix stale wandb.Api() cache in qualification harness readback"; 1 file, harness-only). All three pushed to `origin/master`; Moriah's canonical clone fast-forwarded to `7fe4a2b` before the passing qualification run.

**Not done in this entry.** attempt005 was not retrained; its original `execution_provenance.json`/checkpoints/screening outputs/config/generation manifest/review records were not modified (evidence checksums recorded and unchanged); attempt005's accepted objective was not changed or republished; proposal 2 was not launched; `wandb agent` was never invoked; no controller proposal was requested; no GPU job was submitted; attempt006 was not launched; the production sweep (`4x3btz2s`) and run (`ardib08c`) were never used by the qualification exercise. Full 17-item closure report and Section H (final commit/push/sync/verify) recorded separately in the session's final report.

## 2026-08-25 — Production exact-retry launcher migrated to the manifest-driven bridge entry point; legacy CLI bypass closed; attempt004 rehearsal durably reserved

**[CLOSED — TECHNICAL, NOT SCIENTIFIC]** A read-only production-retry launch review found two gaps before attempt005 (the next permissible exact retry of attempt001/proposal 1) could be prepared: (1) `scripts/run_sweep_v1_exact_retry_moriah.sbatch` still invoked `scripts/run_sweep_v1_exact_retry_bridge.py`'s legacy multi-flag `main()` CLI route, which never runs the commit/interpreter/HOME/`.netrc` runtime contract (`src.baseline.sweep_v1_runtime_contract.run_full_runtime_contract`) that the disposable rehearsal launcher already exercises; (2) the durable prior-attempts record only reserved generations 2–3, not generation 4 (the disposable, non-scientific startup rehearsal: Slurm job `45942761`, W&B sweep `bgx4yovw`, run `6ikc3xgh`, completed successfully through wandb association/executor selection, stopped before training, no objective).

**Repair, both gaps closed together, no scientific-design change.** (A) `scripts/run_sweep_v1_exact_retry_moriah.sbatch` rewritten to the same manifest-driven, single-positional-argument shape as the rehearsal launcher: strict `"$#" -ne 1` argument-count guard, `exec`s `main_from_manifest` via the canonical Python from a `src.baseline.sweep_v1_launch_manifest` JSON file, no `FLASHNH_RETRY_*` environment variables, no long flag list, no `--export=ALL`. Production and rehearsal now share the identical Python entry, manifest loader, runtime contract, identity resolution, W&B initialization helper, config preparation, and executor selector; they differ only in Slurm resource directives and in the manifest's own `mode` field. (B) `_execute_retry` (the shared core both `main()` and `main_from_manifest` call) gained a `runtime_contract_verified: bool = False` parameter and a hard-fail check, inserted immediately after `target_sweep_id` is resolved and before any durable-intake write or `wandb` import: if the resolved target is the real production sweep (`PRODUCTION_WANDB_SWEEP_ID`, `4x3btz2s`) and the runtime contract was not verified, `_execute_retry` raises `SystemExit` rather than proceeding. `main_from_manifest` passes `runtime_contract_verified=True` (it always runs the contract first); the legacy `main()` passes the default `False`. There is exactly one path that can reach production — the legacy CLI route remains usable only for non-production sweeps and hard-fails the instant it would resolve to production. (C) A new durable prior-attempts artifact, `.scratch_local/sweep_v1_incident_records/prior_attempts_after_attempt004.json` (project-local, untracked per repo policy), adds a generation-4 entry (`status=disposable_rehearsal_stopped_before_training`, `non_scientific=true`, `countable_toward_36=false`, `objective_score=null`) alongside the pre-existing, byte-unchanged generation-2/3 entries; no code change was needed in `assert_generation_not_previously_attempted`, which is already generic over any prior-attempts list.

**attempt004 is not a scientific result.** It is a disposable, zero-training startup rehearsal against a disposable, non-production W&B sweep/run; it is permanently reserved and non-countable toward the 36-valid-trial Bayesian budget, exactly like generations 2–3. **attempt005 remains the next permissible production retry**: `execution_generation=5`, `trial_id` ending `__attempt005`, `retry_of_trial_id=attempt001`, same frozen proposal 1 configuration/fidelity/hyperparameters/Seed A, complete operational history spanning attempts 2–4, proposal 2 untouched.

**Tests.** New/updated coverage across `tests/test_sweep_v1_exact_retry_production_launcher.py` (rewritten for the manifest-driven shape), `tests/test_sweep_v1_exact_retry_bridge.py` (new regression test proving the legacy CLI route cannot reach production without the runtime contract, plus a citation of two pre-existing tests proving the legacy route still works end-to-end for non-production sweeps), and `tests/test_sweep_v1_retry.py` (new attempt005-derivation section: generation 5 derivation, mechanical rejection of generations 2–4, retry-history/provenance/W&B-tag content for attempt005). Pre-existing coverage in `tests/test_sweep_v1_launch_manifest.py` and `tests/test_sweep_v1_exact_retry_bridge_manifest.py` already proves production/rehearsal mode-vs-sweep-id cross-validation and that the runtime contract runs before `wandb.init()`, so no duplicate tests were added there. Full local sweep_v1-scoped suite (excluding the pre-existing, unrelated `neuralhydrology`-import collection failures in six `test_nh_*.py`/`test_run_stage1_nh_entrypoint.py` files, and the separately pre-existing missing-`scipy` failures in `test_sweep_v1_review_analysis.py`/`test_sweep_v1_review_rendering.py`, neither touched by this change) passes clean.

**Not done in this entry.** No Slurm job submitted, no W&B sweep/run/proposal created, no training run, no GPU used, attempt005 not launched, attempts 1–4 and their existing evidence untouched, proposal 2 not consumed. An offline attempt005 pre-submission manifest (resolve-only validated, never submitted, no output directory created) was separately constructed under the established project-local untracked launch-manifest location; exact commit hash, manifest path/checksum, and push/Moriah-sync status recorded in the session's final report, not duplicated here.

## 2026-08-24 — Sweep-v1 trial-1 (attempt001) adjudicated INVALID; monolithic-training + post-hoc-screening repair implemented, committed, and non-authoritatively qualified against attempt001's own checkpoints

**[CLOSED — TECHNICAL REPAIR, NOT A SCIENTIFIC RESULT]** Following the first real Sweep-v1 Bayesian production launch (proposal_order=1, `configuration_id=sweep_v1_cfg_5731e180d1bf9d582afc`, `trial_id=...__attempt001`, Slurm job `45935704`, commit `d8098ee`), the trial terminated `execution_status=INVALID` with `failure_category=blocked_continuation_conflict`: the old bounded-chunk `execute_prepared_pilot_run` trains its first chunk, then a second `continue_run` call finds epoch 2 already sitting flat in the base run directory (because Sweep-v1's generated config bakes the full 12-epoch budget into one `start_run` call) and refuses to proceed. **Adjudication: attempt001 is INVALID — it does not count as a valid scientific trial, consumed no proposal budget, and carries no objective.**

**Repair.** New `pilot_orchestration.execute_prepared_pilot_run_monolithic` (`src/baseline/pilot_orchestration.py`): one continuous `start_run`/discovery call trains straight through `target_epoch`, requires all `1..target_epoch` checkpoints present (never nested under a continuation dir), then performs authoritative raw-space post-hoc screening for every not-yet-logged required epoch via the existing `ensure_validation_results`/`evaluate_screening_checkpoint` path, saving restart-safe `orchestration_state` after each epoch. Returns `blocked_incomplete_monolithic_training`/`blocked_incomplete_post_hoc_screening` rather than a false pass if any epoch is missing. Wired into `src/baseline/sweep_v1_execution.py`'s `run_prepared_trial_in_production` in place of the bounded-chunk executor. Separately, `review_records.json`'s `operations.slurm_job_id` (previously always `null` even though the allocation identity was available) is now populated from the live `SLURM_JOB_ID` forwarded through `execute_prepared_trial`; `scripts/run_sweep_v1_wandb_bridge.py` now sources and passes it. `slurm_state`/`gpu_hours` remain a separate post-hoc `enrich_operations_slurm_accounting` patch (unknowable until the job terminates), unchanged in design. No twelve-progressive-continuation workflow was introduced; the continuation-conflict safety gate itself is untouched (it still protects the legitimate bounded-chunk workflow); no frozen science/fidelity/search-domain/objective changed. **Tests:** new `tests/test_pilot_orchestration_monolithic.py` (12-item required suite) plus updated `tests/test_sweep_v1_execution.py`/`tests/test_sweep_v1_wandb_bridge_provenance.py`; 6 files changed, 762 insertions/37 deletions; full repo regression suite green. Committed `2d750db`, pushed to `origin/master`, Moriah's canonical clone (`/sci/labs/efratmorin/omripo/Flash-NH/repos/flash-nh`) fast-forwarded to the same commit.

**Non-authoritative post-hoc qualification of attempt001's existing checkpoints.** attempt001's real trial directory (120M: 12 model + 12 optimizer-state checkpoints, epoch-1 validation results) was never modified. A driver script copied the entire directory to a distinctly-named, project-local, untracked path (`evidence/sweep_v1_attempt001_posthoc_qualification_NONAUTHORITATIVE_v001`), redirected only the copy's output-location fields in a deep copy of the real `preparation_record`, and invoked the exact real, now-repaired `run_prepared_trial_in_production` against the copy only (Slurm job `45938733`, catfish-02, ~34 min). Result: `execution_status=VALID`, `final_status=monolithic_training_and_screening_complete`, all 12 checkpoint/screening epochs present and PASS, `objective_score=0.391678449944578` (best_epoch=1; final_epoch_score(E12)=0.367228696815717; best_minus_final=0.024449753128860663; late_gain_10_to_12=0.0). Optimizer-state inspection confirms exactly 50,000 updates/epoch for all 12 epochs (cumulative step counters 50000→600000), at but never over the `max_updates_per_epoch` cap. Population accounting: 400/400 screening basins present in every epoch's validation results. This qualification is **not eligible for the 36-valid-trial Bayesian budget, was never published to W&B (the driver contains no W&B code), and does not relabel attempt001** — attempt001's real `execution_provenance.json`/`review_records.json` were verified byte-for-byte unchanged (still `execution_status=INVALID`, `failure_category=blocked_continuation_conflict`, empty trajectory, `objective_score=null`, `git_commit=d8098ee` i.e. pre-repair) after the qualification run; a transient mtime/permission-metadata artifact observed on the shared HPC filesystem during inspection was not accompanied by any content change and is attributed to the storage layer, not to any write path (no code in the qualification driver or the committed repair ever targets attempt001's real path).

**attempt002 retry contract (specified, not executed).** Same `proposal_id=stage1_phase_b_sweep_v1_original_domain_v001__bayesian__proposal001` / `configuration_id=sweep_v1_cfg_5731e180d1bf9d582afc` / hyperparameters / Seed-A `model_seed=967139`; fresh `execution_generation=2` → fresh `trial_id`/output directory (never reusing or nesting inside attempt001's paths); `retry_of_trial_id` set to attempt001's full `trial_id`; a fresh Slurm allocation. **No proposal 2 is consumed** — attempt002 is a manual exact-repeat of proposal 1's already-assigned point, launched outside the Bayesian controller's normal next-proposal flow (a `wandb agent` step would instead fetch a new point). For W&B: do not resume `wandb_run_id=7nxaim79` (attempt001's crashed run stays in W&B history untouched, contributing no objective); attempt002 gets its own new W&B run under the same `wandb_sweep_id=4x3btz2s`, configured with the identical swept-hyperparameter values so the dashboard groups it with proposal 1, and reports `objective_score` to W&B only if/when attempt002 is VALID. Flash-NH's own bookkeeping remains the sole scientific authority per §4: the existing `valid_result_order` field (already present in the schema, `null` on attempt001 since it has no valid result) is populated on attempt002's `proposal` record once VALID, decoupled from `proposal_order` (the immutable W&B assignment identity, unchanged at 1) — this is the existing mechanism, not a new one, for letting a later attempt's objective represent a proposal without rewriting the earlier attempt's row. **attempt002 was not launched in this task.**

**Not done in this entry.** No W&B objective published anywhere. No new sweep/run/proposal created. attempt002 not launched; proposal 2 not consumed. Exact commit hash, test results, and full 14-item report recorded in the session's final report, not duplicated here.

## 2026-08-24 — Sweep-v1 launch-command seam repaired: explicit `command` contract, environment-based operational-input resolution, no-overwrite safety (implementation + tests only, no launch)

A bounded launch-readiness inspection found the production W&B sweep config had no explicit `command`, so a real `wandb agent` would have used W&B's default (`${env} ${interpreter} ${program} ${args}`), appending the five swept hyperparameters as `--key=value` CLI flags to `scripts/run_sweep_v1_wandb_bridge.py` — flags its argparse does not accept, failing before `wandb.init()` and before proposal-intake provenance. Separately, the existing online-qualification evidence used `wandb.agent(function=...)` (in-process), which never constructs an OS-level `command`/argv at all, so that evidence never actually exercised the CLI-subprocess agent path the real production launcher depends on.

**Repair (one consolidated seam, not just the reported symptom).** `build_production_sweep_config` (`src/baseline/sweep_v1_execution.py`) now declares an explicit `"command": ["${interpreter}", "${program}"]` — no `${args}`, no `${env}`, no hardcoded paths, keeping the W&B config portable. Because W&B — not the sbatch launcher — now constructs the bridge's argv directly from that template, the launcher can no longer inject `--flag value` arguments the way the non-W&B random-control launcher does; instead `scripts/run_sweep_v1_wandb_agent_moriah.sbatch` requires and exports four `FLASHNH_SWEEP_V1_*` environment variables (`PACKAGE_ROOT`, `SCREENING_BASIN_IDS`, `OUTPUT_ROOT`, `PROPOSAL_ORDER`), inherited by `wandb agent` and its children via standard OS subprocess semantics, plus a shell-level positive-integer check on proposal order. `scripts/run_sweep_v1_wandb_bridge.py`'s four CLI flags became optional (kept for direct/manual/test invocation) and are resolved against their environment counterparts by two strict-precedence helpers (`_resolve_path_operational_input`, `_resolve_proposal_order`): agreement is accepted, exactly one supplied is accepted, a contradiction or the absence of both is a hard error — resolved before `PreparationPaths` construction and before `import wandb`, so a missing/malformed operational input never reaches proposal intake. A `FLASHNH_SWEEP_V1_BRIDGE_SELFTEST=resolve_only` hook resolves and prints the four values as JSON and returns before touching wandb, making a real OS-level subprocess test of the exact W&B-constructed argv possible without any network/training. `scripts/build_sweep_v1_production_sweep_config.py --output` now refuses to overwrite an existing file unless `--force` is passed.

**Not reopened.** `PreparedPilotExecutionResult`/`run_prepared_trial_in_production`/`_derive_validity`, the Layer-B provenance design, `write_prepared_proposal(..., allow_layer_b_provenance=True)`'s existing no-overwrite guard, the frozen 5-axis search domain, and the 12-epoch/50,000-update/Seed-A/raw-space scientific contract are all unchanged.

**Tests.** New `tests/test_sweep_v1_launch_command_contract.py` (20 tests): production config shape; real builder serialization and no-overwrite/`--force` behavior; CLI/environment resolver precedence and contradiction/absence/malformed-value hard failures; the real OS-level agent-to-bridge argv contract (argv derived from the real serialized sweep config, run as an actual subprocess with `resolve_only`, proving no swept-hyperparameter flags reach argv and all four operational inputs resolve exactly); and a static sbatch resource/env/concurrency/no-credential-exposure contract check. Full focused Sweep-v1 suite (`test_sweep_v1_execution.py`, `test_sweep_v1_output_dropout_batch_size_axes.py`, `test_sweep_v1_production_adapter.py`, `test_sweep_v1_static_campaign_foundation.py`, `test_sweep_v1_wandb_bridge_provenance.py`, plus the new file): 97 passed, 14 skipped (pre-existing torch-import guards; local `.venv` has no torch), 0 failed — the existing golden CLI-only bridge/provenance tests pass unchanged. `test_sweep_v1_review_analysis.py`/`test_sweep_v1_review_rendering.py` fail in this local `.venv` due to a pre-existing missing `scipy` dependency, unrelated to this repair (those files were not touched).

**Not done in this entry.** No W&B sweep/run/proposal created. No Slurm job submitted. No training run. No real Sweep-v1 production trial consumed — the first eventual launch remains exactly one fresh Bayesian sweep → one proposal → one Slurm allocation → one agent → `--count 1` → one scientific attempt. Pre-existing untracked Moriah `wandb/` directory not touched. Exact commit hash and push/Moriah-sync status recorded in the session's final report, not duplicated here.
