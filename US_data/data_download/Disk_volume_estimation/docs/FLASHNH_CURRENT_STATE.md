...
**Explicitly NOT done.** The six P2/P3/R1–R4 development-population evaluations have still not been launched; there is no seven-row real scientific comparison, no screening-vs-full ranking-stability result, no screening-adequacy conclusion, no winner, no promotion, no full-population checkpoint-selection study, no Proposal 4 / random-control Wave 2 authorization, and no sealed / temporal-test / spatial-holdout / California access. No objective, search-space, fidelity, sampling-policy, or interpretation-rule change. The immediate next separately authorized step is the remaining-six production execution.

## `[SHARED-A5]` development-population Common-120 audit production execution — canonical 2,307-basin artifact materialized + verified; the P1 audit-path qualification canary is COMPLETE and passed every gate; the per-basin evaluator performance defect is repaired and committed (`34138c27`) and now qualified byte-identical on real P1 data (Slurm `46103139`); the seven-entry selection manifest is materialized on Moriah; P1 canary + evaluator-hardening qualification CLOSED; remaining six configs NOT run; no winner/promotion (2026-09-06)

**[IN PROGRESS — ONE OF SEVEN CONFIGURATIONS SCORED AS AN AUDIT-PATH CANARY; NOT THE SCIENTIFIC COMPARISON RESULT; NO WINNER, NO PROMOTION, NO PROPOSAL 4 / WAVE 2, NO SEALED ACCESS]** SHARED-A5 opens the real screening-400 vs full-2,307-basin development-population audit designed in SHARED-A3 and made implementation-ready in SHARED-A4. The `[SHARED-A4]` entry below is now **historical** for the "explicitly NOT done" list — the real artifact and the first real evaluation exist; its design/review content stands.
