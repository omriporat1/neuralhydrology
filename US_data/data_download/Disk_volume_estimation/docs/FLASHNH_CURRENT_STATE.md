...
**Hydrograph extraction.** New module `src/baseline/rd1_c4_f_hydrograph_runner.py` reuses the qualified `evaluate_fixed_support_raw_space_metrics`/`derive_canonical_package_observed_series`/`authenticate_trial_roster` paths (never a parallel scientific computation) to render one same-panel, multi-candidate overlay figure per canonically selected basin: p10=`06600100`, p50=`01464000`, p90=`02303205` (frozen deterministic percentile-of-difference selection rule, `hydrograph_basin_selection_manifest.json`). Candidates are the 6 unique best-so-far incumbent trials across `{bayesian, random_control} x proposal_order {1,3,6,9,12}`, resolved via `build_incumbent_order_mapping` against the real 24-trial roster/configuration table used for RD1-C4-E. Each panel uses one frozen, symmetric ±36h peak-centered window (clipped, not shifted, to series bounds) and one shared axis scale.

**Remote run.** Same real inputs as RD1-C4-E (trial roster sha256 `01569743a7...`, contract sha256 `99fdc4f447...`, package root `stage1_scientific_package_v002`), plus the real selection manifest and configuration table. Moriah job `46185509`: `COMPLETED`, `ExitCode=0:0`, `Elapsed=00:00:35`, canonical `flash-nh` clone confirmed untouched. A first attempt (job `46185466`) completed successfully but its figures had a legend/title text-overflow rendering defect (full raw `trial_id` strings used as legend labels), found via mandatory visual review; fixed in `candidate_labels_from_manifest` (compact `arm@order` labels) and re-run.

**Evidence.** Retrieved and hash-verified into `.scratch_local/rd1_c4_f_hydrograph_46185509_evidence/`: `manifest.json` (provenance, peak windows, incumbent-order mapping) and 3 figures (`hydrograph_p10_06600100.png`, `hydrograph_p50_01464000.png`, `hydrograph_p90_02303205.png`), all visually confirmed legible. All 3 figures show observed peak discharge substantially exceeding every candidate's reconstruction, consistent with the previously documented shared high-flow/Q98 weakness across both search arms; no candidate or arm visually stands out as a systematic winner in these 3 examples.

**Scope.** This is descriptive run-progress evidence only. No training, inference, rescoring, W&B/controller contact, or sealed-scope access occurred. No classifier, winner, promotion, or tolerance decision was made. RD1-C4 is not closed by this entry. Full detail: `docs/decision_log.md` (2026-09-17 entry).

## RD1-C4-E provenance-envelope exceedance made report-only at `11e79faead3aaf3ab7abce8f07feeeece388ae32`; formal 24-trial results-production COMPLETED (Moriah job `46183269`) — evidence-only operational/measurement result; final audit policy, scientific classification, and C4 closure remain OPEN (2026-09-17)

**[FORMAL RESULTS BUNDLE PRODUCED — EVIDENCE-ONLY OPERATIONAL/MEASUREMENT RESULT; NOT A SCIENTIFIC CLASSIFIER, WINNER, PROMOTION, OR AUDIT-POLICY DECISION; NOT A C4 OR C5/C6 CLOSURE]** The PROVISIONAL provenance-envelope exceedance identified in RD1-C4-D1 (below) was still wired as a fatal check in the formal results-production consumer, so the first formal-results attempt (job `46178103`) aborted after 1:51:26 at basin `06911000`/Bayesian proposal002. That check now returns a structured, report-only `ProvenanceAuditRecord` per trial×basin cell instead of raising; timestamp and observed-array-shape identity checks remain fatal, unchanged. No change to canonical Q98 facts, the official objective, or NSE/KGE/Q98 computation. Committed at `11e79faead3aaf3ab7abce8f07feeeece388ae32` (4 files: consumer, results runner, 2 test modules), pushed to `origin/master`, synced into the isolated Moriah clone. Focused suite: 85/85 passed.

**Rerun.** Same 24-trial roster/contract/package root as job `46178103`, Moriah job `46183269`, fresh output `results_11e79fa_20260917T074711Z`: `COMPLETED`, `ExitCode=0:0`, `Elapsed=02:01:55`. Full metric coverage: 9,600/9,600 cells finite for every NSE/KGE/Q98 field. `provenance_audit_summary`: `n_cells=9,600`, `n_cells_with_exceedance=152`, `n_elements_compared=80,349,432`, `n_elements_exceeding=11,352` — matching the frozen RD1-C4-D1 diagnostic exactly.

**Evidence.** Retrieved and hash-verified (against `manifest.json` and independent remote `sha256sum`) into `.scratch_local/rd1_c4_e_rerun_46183269_evidence/`: `per_basin_metrics.csv`, `q98_diagnostics.csv`, `basin_distribution_summary.csv`, `canonical_q98_facts.csv`, `provenance_audit.csv`, `review_identity.json`, plus logs/`sacct`. Four evidence-backed figures (NSE ECDF by arm, KGE ECDF by arm, Q98 diagnostics by arm, provenance-envelope diagnostic explicitly labeled provenance metadata, not skill failure) at `.scratch_local/rd1_c4_e_rerun_46183269_evidence/figures/`.

**Scope.** This is an operational/measurement result only. No tolerance, classifier, winner, promotion, or final NSE/KGE/Q98 publication decision was made; RD1-C4 is not closed by this entry. No sealed-scope access, training, inference, W&B, or controller contact occurred. Full detail: `docs/decision_log.md` (2026-09-17 entry).

## RD1-C4-D1 full 24-trial array + reduction COMPLETED at `a81873c751eb8f13d4209030882e146a57d80ac9` (Moriah array job `46176236`, reducer job `46176640`) — evidence-only operational/measurement result; final audit policy, scientific classification, and C4 closure remain OPEN (2026-09-16)

**[FULL ARRAY + REDUCTION COMPLETED — EVIDENCE-ONLY OPERATIONAL/MEASUREMENT RESULT; NOT A SCIENTIFIC CLASSIFIER, WINNER, PROMOTION, OR AUDIT-POLICY DECISION; NOT A C4 OR D1 CLOSURE]** Following the accepted disposable benchmark (below), the full 24-trial D1 array and its reduction ran to completion using the same authoritative inputs (trial roster sha256 `01569743a7fe69f180f2ca4a2be2f518e45ed70b2b2822642894b8e22cc52e4e`, contract sha256 `99fdc4f44768779661a735d25974e25b64a5d7f502ec8ffa4c5d2bab42bc34d6`, package root `stage1_scientific_package_v002`) at isolated-clone HEAD `a81873c751eb8f13d4209030882e146a57d80ac9` (== `origin/master`).

**Array.** `sbatch --array=0-23%4 scripts/rd1_c4_d1/rd1_c4_d1_array.sbatch`, job `46176236`, fresh store root `full_a81873c_20260916T144219Z/store` (never reused from the benchmark). All 24 tasks `COMPLETED`, `ExitCode=0:0`; exactly 24 receipts confirmed on the filesystem; zero failures; no retry pass needed; only one 0-23 array was ever submitted.

**Reduction.** First submission (job `46176609`) FAILED in 2s — the ad hoc submission script omitted the required `D1_PACKAGE_ROOT` variable (a submission-script omission, not a repository code defect; no repo file changed; `D1_OUT_DIR` was never created by the failed attempt). Second submission (job `46176640`), same fresh `D1_OUT_DIR`, COMPLETED in `00:01:36`, exit `0:0`, publishing atomically.

**Result summary (from the reducer's `summary.json`, hash-verified after transfer).** Exactly 24/24 trials, 400 basins/trial, 9,600-cell cross-product with `fraction_cells_compared = 1.0`; **all 9,600 cells status `ok`**, every other status category (`support_mismatch`, `package_missing`, `package_checksum_mismatch`, `nonfinite_canonical`, `pickle_load_error`, `area_derivation_inconsistent`, `basin_missing_from_trial`, `other_error`) `= 0`; exact 12 Bayesian / 12 random-control trial split (4,800 cells per arm ÷ 400 basins); single contract identity across all cells; Q98 consequence aggregates `n_basins_with_changed_frozen_decision = n_basins_with_changed_membership = n_basins_with_changed_peak_timestamp = 0` (n=9,600 records). Package-vs-pickle provenance comparison (diagnostic only, per frozen-scope rules): 80,349,432 elements compared, 11,352 exceeding the explicitly report-only provisional envelope; `final_rd1_c4_audit_policy: "unresolved"`; `scientific_classification: "not performed"`.

**Evidence.** Full reduced-output layer (~20.2 MB, 10 files) transferred via `scp -O` to `.scratch_local/rd1_c4_d1_full_46176236_reduce_46176640/` and hash-verified locally against the reducer's own `manifest.csv`/`checksums.sha256` (8/8 files OK). Compact evidence-only packet at `.scratch_local/rd1_c4_d1_full_46176236_reduce_46176640/EVIDENCE_PACKET.md`. Per-shard/per-element detail parquet remains on Moriah only (referenced by hash/size in `receipt_index.csv`).

**Scope.** This is an operational/measurement result only. No tolerance, classifier, winner, promotion, or final NSE/KGE/Q98 publication decision was made; RD1-C4 is not closed by this entry. No sealed-scope access, training, inference, W&B, or controller contact occurred. Full detail: `docs/decision_log.md` (2026-09-16, "full 24-trial array + reduction COMPLETED" entry).

## RD1-C4-D1 disposable single-trial benchmark ACCEPTED at `8330c0d8748e858bd6704c84e9dcefa9e3b65893` (Moriah job `46176003`) — operational/resource/evidence result only; the 24-trial array, reducer, final audit policy, and C4 closure remain OPEN (2026-09-16)

**[DISPOSABLE BENCHMARK ACCEPTED — OPERATIONAL/RESOURCE/EVIDENCE RESULT ONLY; NOT A SCIENTIFIC CONCLUSION; NOT ARRAY/REDUCER AUTHORIZATION; NOT A C4 OR D1 CLOSURE]** Following benchmark-gate qualification (below), the one authorized disposable, isolated, one-trial resource benchmark ran and was accepted. Repair history: `af7f92eb7d522ed5a1f6966fbdfbc10f262958e8` fixed Slurm spool-copy resolution; `8330c0d8748e858bd6704c84e9dcefa9e3b65893` — the commit this benchmark ran at — narrowly permits a real validation-result variable shaped `('date','time_step')` when `time_step` is singleton, while package-canonical observations remain strict one-dimensional. Neither is general production hardening.

**Result summary.** Job `46176003` (partition `glacier`, node `glacier-17`) ran the exact authenticated P2 frozen-roster trial and COMPLETED, exit `0:0`. Scheduler elapsed `00:10:22`; process-internal (GNU time) wall clock `10:07.38`; `TotalCPU ≈ 07:40.8` (≈75% of 1 CPU per GNU time); `MaxRSS ≈ 605,700 KB` (GNU-time peak ≈ 778 MB) — both well inside the `1 CPU / 8 GB / 4h` request, which is retained as operationally appropriate for this scale but does **not** set array concurrency. Outcome: `ok: 400`, all other cell statuses `0`; `detail_rows = 3,279,806`; `n_q98_records = 400`; progress publication ordered end-to-end to `task_end(outcome="published")`; fresh store, no shard reuse; all seven receipt-declared component hashes independently reproduced on Moriah; compact evidence transferred to `.scratch_local/rd1_c4_d1_benchmark_p2_46176003/` and hash-verified locally (the ~76 MB `elements.parquet` remained remote-only, with its size/compression/hash recorded), plus a hash-verified postrun supplemental record (`sacct` accounting and the raw fixed-support-contract-file SHA-256) at `.scratch_local/rd1_c4_d1_benchmark_p2_46176003/postrun_supplemental/`. Full detail, exact hashes, and provenance boundaries: `docs/decision_log.md` (2026-09-16, "disposable single-trial benchmark ACCEPTED" entry).

**Scope.** Exactly one accepted benchmark exists. Jobs `46175271` and `46175818` are not accepted evidence and yield no scientific conclusion. No array, reducer, scientific classifier, tolerance, final NSE/KGE/Q98 publication, C5/C6 work, or sealed-scope access is authorized here. Open full-array blockers: atomic shard/reducer publication and integrity, durability/race handling, final ULP/storage correction, final Slurm hardening, array-concurrency sizing, and final scientific audit policy.

> _(Note added 2026-09-16: the full 24-trial array and reducer have now run and COMPLETED — Moriah array job `46176236` and reducer job `46176640` at commit `a81873c`. See the "full 24-trial array + reduction COMPLETED" entry above and the matching `docs/decision_log.md` entry. This remains an operational/measurement result only; it does not set the final scientific audit policy and does not close RD1-C4.)_

## RD1-C4-D1 benchmark-store guard QUALIFIED for exactly one disposable one-trial benchmark — the benchmark itself has NOT run; the 24-trial array, reducer, final audit policy, and C4 closure remain BLOCKED (2026-09-16)

D1/Correction A's authoritative-input interfaces and the benchmark-store isolation guard (including the dangling-symlink correction, Moriah-qualified in job `46173923`, `1 passed in 0.16s`) are locally implemented, reviewed, and qualified to permit **exactly one disposable, isolated, one-trial resource benchmark**. **The benchmark has not yet run** — no benchmark Slurm job has been submitted and no real-trial resource measurement exists. The full 24-trial array, the reducer, the final audit tolerance/policy, and RD1-C4 closure all remain blocked and unauthorized. See `docs/decision_log.md` (2026-09-16 entry) for full detail.

> _(Note added 2026-09-16: the one authorized disposable benchmark has now run and was ACCEPTED — Moriah job `46176003` at commit `8330c0d`. See the "disposable single-trial benchmark ACCEPTED" entry above and the matching `docs/decision_log.md` entry. This is an operational/resource/evidence result only; it does not close RD1-C4, does not authorize the array/reducer, and does not change the guard qualification recorded in this entry.)_

## RD1-C2 configuration-level analysis core CLOSED — implemented, independently reviewed, committed, and pushed at `e03e957109769ca7e34f034668da4ff5e463c268` (parent `34c49873be57c01d331bab0472dd2bbb19f2c606`); Codex final resolution review APPROVE after four earlier MAJOR interface defects were corrected pre-commit; focused RD1-C2 suite 67 passed, relevant v1/v2 regression suite 124 passed; MACHINERY MILESTONE ONLY — NO real 12+12 scientific review performed, NO Bayesian-vs-random verdict, NO winner / promotion / selection, NO search-space / range change / >120 seq_length, NO P13+ / R13+, NO SHARED-A5 consumption, NO new training / NH-evaluation / W&B / controller / Slurm / Moriah contact, NO sealed / temporal-test / spatial-holdout / California access; RD1-C3 (basin-analysis primitives) is next, separately planned/authorized (2026-09-14)

**[CLOSED — MACHINERY MILESTONE ONLY; DOCS CLOSURE FOR AN ALREADY-PUSHED IMPLEMENTATION COMMIT; NO REAL 12+12 SCIENTIFIC REVIEW; NO BAYESIAN-VS-RANDOM VERDICT; NO WINNER / PROMOTION / SELECTION; NO EXACT TOP-GROUP ORDERING CONCLUSION; NO SEARCH-SPACE / RANGE CHANGE; NO SEQ_LENGTH > 120; NO P13+ / R13+ / FURTHER SEARCH; NO FULL-DEVELOPMENT EVALUATION; NO SHARED-A5 CONSUMPTION; NO NEW TRAINING / NH-EVALUATION / W&B / CONTROLLER / SLURM / MORIAH / REMOTE-RETRIEVAL CONTACT; NO SEALED / TEMPORAL-TEST / SPATIAL-HOLDOUT / CALIFORNIA ACCESS; NO COMMIT OR PUSH BY THIS DOCS TASK]**

**What closed.** RD1-C2 — the configuration-level analysis core for the frozen Stage-1 v2 12+12 scientific review (RD1-C1, `docs/stage1_v2_12plus12_review_design_v001.md`, untouched by this entry) — is implemented, independently reviewed, committed, and pushed to `origin/master` at commit `e03e957109769ca7e34f034668da4ff5e463c268` ("Implement RD1-C2 configuration analysis core"), parent `34c49873be57c01d331bab0472dd2bbb19f2c606`. The implementation commit contains exactly two files, `src/baseline/stage1_v2_12plus12_review_analysis.py` and `tests/test_stage1_v2_12plus12_review_analysis.py` (2,067 insertions). No design document, campaign config, execution script, or campaign evidence file changed in that commit.
...
## Stage-1 v2 six-axis 12+12 campaign EXECUTION/EVIDENCE CLOSURE — the user-authorized Bayesian continuation P9–P12 (controller `wta85z3b`) and the frozen random-control Wave 3 R9–R12 ran at `5a09bce` and each adjudicated VALID from checksum-valid local compact evidence; the planned 12 Bayesian + 12 random-control HARD STOP is reached and verified; RD1-C1 frozen methodology untouched; the evidence required for the RD1 scientific review is complete and preserved; docs-only; NO scientific 12+12 interpretation, NO Bayesian-vs-random comparison, NO winner / promotion / selection / axis-narrowing / sequence-extension / >120 seq_length, NO P13+ / R13+, NO new compute / W&B / controller / Slurm / Moriah / training / NH-evaluation / remote-retrieval contact (2026-09-10)

**[CLOSED — EXECUTION/EVIDENCE CLOSURE ONLY; DOCS-ONLY; NO SCIENTIFIC 12+12 INTERPRETATION; NO BAYESIAN-VS-RANDOM COMPARISON; NO WINNER / PROMOTION / SELECTION; NO AXIS-NARROWING / SEQUENCE-EXTENSION / >120 SEQ_LENGTH; NO P13+ / R13+ / FURTHER PROPOSALS; NO OBJECTIVE / FIDELITY / SEARCH-SPACE / SEED-POLICY CHANGE; NO SHARED-A5 CONSUMPTION; NO NEW COMPUTE / W&B / CONTROLLER / SLURM / MORIAH / TRAINING / NH-EVALUATION / REMOTE-RETRIEVAL CONTACT; NO SEALED / TEMPORAL-TEST / SPATIAL-HOLDOUT / CALIFORNIA ACCESS; NO COMMIT OR PUSH BY THIS TASK]**

**What closed.** The user-authorized Bayesian continuation (Proposals 9–12, controller `wta85z3b`) and the frozen random-control Wave 3 (IID manifest rows 8–11, R9–R12) ran at repo commit `5a09bcee034748d85ed13fea8d0b098326e8988c` and completed. This entry records **execution/evidence closure only**: the campaign has reached the planned 12+12 hard stop, and the evidence for the separate RD1 scientific review is complete and preserved. It performs no Bayesian-vs-random comparison and makes no scientific decision — those belong exclusively to the RD1 review lane.

**RD1-C1 methodology untouched.** `docs/stage1_v2_12plus12_review_design_v001.md`, frozen in parent commit `1e32d577032fcdb33fd1d52ffd77f57fdd9cf8cf`, remains the authoritative frozen 12+12 review contract and is not modified or weakened here.

**Full valid 12+12 — exact counts.** Bayesian: exactly **12 valid proposals P1–P12**, no P13. Random-control: exactly **12 valid runs R1–R12**, no R13. P9–P12 and R9–R12 are each independently VALID from checksum-valid local compact evidence (`.scratch_local/v2_p9_p12_wave3_5a09bce/`): `execution_status`/`provenance_stage` VALID, `objective_eligible = true`, `workflow_status = pass`, strict-load `all_checks_pass` at `expected_commit = 5a09bce`, Seed A `967139`, fidelity `mf12x50000`, fixed-support contract SHA-256 `cb4ebe86…`, screening basin-ids SHA-256 `d4395d93…`, 12/12 model + 12/12 optimizer + 12/12 per-epoch `validation_results.p`, trajectory 1..12, `sealed_scope = false`. Bayesian W&B runs `ugt88sfz`/`pq6bc8a2`/`d2qftrg1`/`g4ueybn4` all `finished` / `flashnh/valid = true`; controller run-count 8→9→10→11→12, one new run per proposal, each behind the per-proposal read-only Part-D reconciliation; forbidden run-ids never queried. R9–R12 made zero W&B contact (`wandb_sweep_id`/`wandb_run_id` None). All bundle checksums re-verified (176 files, 0 failures).

**Objective is unchanged.** All 24 objectives are the best eligible epoch's `flashnh/common120_raw_space_nse_v001` — the median per-basin raw-space NSE over the frozen **400-basin screening population** on the frozen **Common-120** fixed candidate-independent temporal-evaluation support (120-hour floor; Common-120 is a support contract, not a basin population) — within the frozen `mf12x50000` fidelity, `evaluation_scope = development_validation_2024_only`. Per-trial objective values are recorded in `docs/decision_log.md` and the local adjudication evidence; **no ranking, winner, or Bayesian-vs-random conclusion is drawn** (screening-400 is not authoritative for ordering — SHARED-A5).

**Random-control manifest stayed frozen.** `config/stage1_phase_b_sweep_v2_six_axis_random_control_v001_random_control_manifest.json`, SHA-256 `59be6726b60863aeed1c25e86782bd7a5e1623434ce23c956e66eb54c527c095`, git-tracked and unmodified; exactly 12 rows; Wave 3 consumed rows 8–11 in frozen order with no reorder/replace/add and no replacement search.

**Hard stop honored — no silent-extension path.** No Bayesian proposal beyond P12 (P12 `wandb_run_final.json`: `sweep_run_count = 12`, 12 distinct run-ids; `bayes_run_registry_v3.json` 11 rows, no P12 append by design). No random-control run beyond R12 (12-row frozen manifest; no Wave 4). The tracked launcher is a manual one-shot `sbatch` wrapper and the agent sbatch ends in `wandb agent --count 1` (one proposal per job, then exit) behind a `max_agents = 1` gate; no cron/at/systemd/daemon drives either. Completion pollers are bounded read-only `squeue`/`sacct` waiters that write `POLLER_DONE.sentinel` and exit — they submit nothing; P10/P11/P12 pollers and the re-armed Wave-3 collector ended on genuine `slurm_state = COMPLETED` and exited 0. The W&B bayes controller is pull-based and cannot issue a run with no agent alive and no queued job.

**Evidence complete and preserved for RD1.** All 24 configs retain 12/12 per-epoch `validation_results.p`, 12/12 model + 12/12 optimizer checkpoints, and provenance JSONs (`PRESERVATION_RD1.md` §2). The best-epoch validation product is retained for every config whichever epoch the review fixes as official best. The Moriah run directories remain the **sole** copy of the full per-epoch `validation_results.p` pickles; the RD1 preservation hold (`_RD1_PRESERVE_DO_NOT_DELETE.txt` markers at both protected roots) is binding until the 12+12 scientific-review lane closes. This task took no remote action; the execution lane, not this task, verifies live remote retention.

**Exact GPU-hour source available — no gap.** Authoritative `sacct` accounting retains every `omripo` record back to 2026-08-09; per-trial `ElapsedRaw` is available for all 24 (`PRESERVATION_RD1.md` §4; per-row `sacct.txt` in each bundle). No gap blocks exact per-trial reconstruction. Indicative totals (1× L4 per trial): Bayesian ≈ 33.7 GPU-h, random-control ≈ 26.6 GPU-h, campaign ≈ 60.4 GPU-h. Wall-clock elapsed time is not substituted for GPU-hours.

**Stability diagnostics — factual, not retroactive invalidity.** Authoritative best-minus-final: **P7 ≈ 0.395** (best epoch 8), **P12 ≈ 0.325** (best epoch 5), **P4 ≈ 0.120** (best epoch 5); all other 21 runs ≤ ≈ 0.040. **P12 is another severe late-training-deterioration case** and strengthens the existing high-learning-rate instability MONITOR concern — it occurred at approximately `learning_rate = 8.30e-4`, `batch_size = 128`, extending the earlier P4/P7 pattern (both `{learning_rate ≥ 8e-4 AND batch_size = 256}`) beyond `batch_size = 256`. This is an observed three-case pattern, **not** a causal single-axis claim and **not** a proven `learning_rate ≥ 8e-4` universal failure threshold. P4/P7/P12 each remain **VALID**: the official objective is the best eligible epoch within the complete frozen 12-epoch fidelity, and that best epoch precedes the divergence in every case (no evidence is asserted that the best-epoch objective was itself depressed by the instability). `best_minus_final` and the retained scalar trajectories capture the late behaviour; its scientific meaning is the RD1 review's to determine. No early-stopping rule, fidelity, search-space bound, or scoring rule is changed.

**No scientific decision made here.** No winner, promotion, selection, or causal hyperparameter inference; no search-space / range / `seq_length > 120` change; no narrowed ranges; no P13+ / R13+; no full-development evaluation; no SHARED-A5 consumption beyond standing conclusions. SHARED-A5 remains binding — the full-development evaluation / checkpoint-selection / promotion gate is mandatory downstream and no 12+12 finding may bypass it. The `seq_length` boundary-pressure tally (7/12 Bayesian at the 120 ceiling; P9–P11 = 120, P12 = 108) is a factual diagnostic for the RD1 review and authorizes nothing.

**Intended next state: `READY FOR 12+12 SCIENTIFIC REVIEW`.** The next phase is implementation and use of the prospectively frozen RD1 machinery — next local milestone **RD1-C2** (configuration-level analysis core) per the frozen design sequence — then the formal ChatGPT + user 12+12 scientific review. Compact adjudication/closure evidence (untracked, `.gitignore` `.scratch_local/`): `.scratch_local/v2_p9_p12_wave3_5a09bce/` — `ADJUDICATION_LOG.md`, `PRESERVATION_RD1.md`, per-trial compact bundles + `SHA256SUMS.txt`. This factual closure is carried by the docs-only change that adds this entry and the matching `docs/decision_log.md` entry; no tracked source/config/test/manifest/scientific-design change.

> _(Note added 2026-09-14: RD1-C2 is now CLOSED — implemented, independently reviewed (Codex APPROVE), committed, and pushed at `e03e957`. See the "RD1-C2 configuration-level analysis core CLOSED" entry at the top of this file. RD1-C3 (basin-analysis primitives) is next, separately planned/authorized. This does not change any 12+12 execution/evidence finding recorded above.)_

## Prospective 12+12 scientific-review methodology FROZEN (RD1-C1) — the read-only RD1-A interface archaeology is complete, and the user has now frozen the prospective Stage-1 v2 12+12 scientific-review design as a durable docs contract at `docs/stage1_v2_12plus12_review_design_v001.md`, before any substantive Bayesian Proposal 9–12 / random-control Wave-3 outcome was examined in this scientific-design lane; docs-only; no analysis code implemented; no compute; no W&B / controller / Moriah / h2o / Slurm / training / NeuralHydrology-evaluation contact; NOT proposal authorization; NO winner / promotion / selection; NO objective / fidelity / search-space / seed-policy change; NO >120 seq_length, range change, continued Bayesian search, additional random controls, full-development evaluation, or SHARED-A5 consumption; NO sealed / temporal-test / spatial-holdout / California access (2026-09-09)

**[CLOSED — PROSPECTIVE DESIGN FREEZE ONLY (RD1-C1); DOCS-ONLY; NO ANALYSIS CODE; NO COMPUTE; NO W&B / CONTROLLER / MORIAH / H2O / SLURM / TRAINING / NH-EVALUATION / REMOTE-RETRIEVAL CONTACT; NOT PROPOSAL AUTHORIZATION; NO WINNER / PROMOTION / SELECTION; NO OBJECTIVE / FIDELITY / SEARCH-SPACE / SEED-POLICY CHANGE; NO >120 SEQ_LENGTH / RANGE CHANGE / P13+ / R13+ / FULL-DEVELOPMENT EVALUATION / SHARED-A5 CONSUMPTION; NO SEALED / TEMPORAL-TEST / SPATIAL-HOLDOUT / CALIFORNIA ACCESS; NO COMMIT OR PUSH BY THIS TASK]**

**RD1-A archaeology complete.** A read-only interface-archaeology / review pass established how the already-approved RD1 review methodology maps onto the repository's actual producers, artifacts, schemas, and reusable analysis code. It changed no file and inspected no substantive P9–P12 / R9–R12 outcome. Its central data-contract finding: the compact local campaign receipts retain identities, the six hyperparameters, scalar objective results, best/final/stability diagnostics, fixed/natural **median** epoch trajectories, provenance, support identity, and some Slurm evidence — but **not** the full per-basin / per-timestamp scientific inputs (400-basin NSE distributions, P2/R2 paired basin comparison, KGE/components, Q98 diagnostics), which require the authoritative NeuralHydrology `validation/model_epochNNN/validation_results.p` products plus the frozen Common-120 support contract.

**RD1-C1 — prospective 12+12 review design FROZEN.** The user has frozen the prospective 12+12 scientific-review methodology as a durable design contract: **`docs/stage1_v2_12plus12_review_design_v001.md`**. It is the authoritative detailed specification for the planned formal 12-plus-12 review. Material frozen decisions: Common-120 is a temporal-support contract (screening-400 population, 2024 validation, fixed-support objective = median of 400 per-basin raw-space NSE at the best eligible epoch; natural support diagnostic only); 12 Bayesian + 12 random-control configurations are the unit of evidence and an ordinary t-test/Mann–Whitney is not the primary Bayesian-vs-random evidence; only Seed A is represented (no seed sensitivity); **P2 `0.4388098707961096`** is the frozen arm-neutral configuration-level near-incumbent anchor with descriptive margins 0.01 primary / 0.005 sensitivity (not equivalence bounds, not promotion thresholds; threshold-free top-k also required); **P2 and R2 `0.43276757946464195`** are both frozen basin-wise paired anchors; the core hydrological review is at each configuration's official best epoch only (24 best-epoch validation-result products), with the retained scalar epoch trajectories sufficient for the stability analysis; exact per-trial GPU-hours where authoritative Slurm/sacct evidence allows, otherwise marked unavailable with coverage reported; a six-axis normalized-distance geometry (lr log10, hidden_size log2, embedding/output dropout linear, batch_size log2, seq_length linear/ordinal; `d = sqrt(mean_k((x_k−y_k)^2))`; equal weighting is descriptive, not equal importance) with Bayesian novelty and incumbent-distance series; whole-sequence search-process analysis (legacy early/late `proposal_drift_evidence` demoted to a historical diagnostic); the frozen basin-level NSE core set (Q1/Q5/Q25/Q50/Q75/Q95/Q99/IQR + counts); basin paired-difference classification with a **machine-precision-only** numerical tie tolerance (not a scientific NSE margin); natural-support trajectory strictly secondary; full-support KGE-2009 + components (reusing the qualified repo implementation) as a secondary diagnostic; the frozen Q98 high-flow methodology (`np.quantile(obs, 0.98, method="linear")` per basin, candidate-independent `obs >= Q98` mask retaining ties; package = Q98-normalized RMSE, relative volume bias, observed-peak-time magnitude error, caveated high-flow NSE, always-reported sample count; ≥50-sample and finite-nonzero-denominator guards; a secondary diagnostic failure never invalidates a trial; high-flow KGE and event diagnostics exploratory); `src/baseline/high_flow_event_metrics.py` explicitly not authoritative for the new Q98 package; the RD1 interpretation of `seq_length = 120` is **"boundary pressure observed / measurable; expansion not authorized; strong combined evidence may motivate a separate targeted >120 experiment"** (not `"natural / N/A"`), with no symmetry requirement on the lower bound; an axis / interaction evidence matrix that exposes transparent lower-level evidence columns only — the final strong/suggestive/weak axis-confidence label remains a **human ChatGPT + user scientific-review judgment**, never a mechanical classifier, and Bayesian concentration alone can never establish strong axis evidence; a valid run with poor late stability remains a valid observation.

**P9–P12 scientific outcomes remain deliberately unreviewed in this scientific-design lane.** No P9–P12 / R9–R12 outcome was inspected or used to form any RD1-C1 decision; `.scratch_local/v2_p9_p12_wave3_5a09bce/` was not opened or enumerated. Pre-P9 evidence (P2, R2 identities and scores from committed docs) was used to freeze the references, as the design requires.

**Implementation not yet complete.** RD1-C1 implements no analysis code. The intended bounded implementation sequence is recorded in the design document: **RD1-C2** (configuration-level analysis core), **RD1-C3** (basin-analysis primitives), **RD1-C4** (fixed-support hydrological consumer + Q98 — highest-risk interface milestone, strong Codex-review candidate), **RD1-C5** (transparent axis/interaction evidence derivations, no automated verdict), **RD1-C6** (rendering + final review packet). The next local scientific-analysis implementation milestone is **RD1-C2**, after review/acceptance of this docs patch.

> _(Note added 2026-09-14: RD1-C2 is now CLOSED — implemented, independently reviewed (Codex APPROVE), committed, and pushed at `e03e957`. See the "RD1-C2 configuration-level analysis core CLOSED" entry at the top of this file. RD1-C3 (basin-analysis primitives) is next, separately planned/authorized; RD1-C4/C5/C6 remain not implemented and not pre-authorized. This does not change the RD1-C1 frozen methodology.)_

**Remote validation-product preservation is an execution-lane dependency.** The final 12+12 review depends on preservation of already-produced NeuralHydrology validation outputs — at minimum every valid configuration's official best-epoch `validation_results.p` (preferably all currently retained per-epoch products until RD1 closes), model checkpoints as fallback, the frozen Common-120 contract, the frozen screening-400 identity, the per-trial completion/provenance/checksum bundles, the Slurm/sacct evidence for exact GPU-hour accounting, and the tracked frozen random-control manifest. **Deletion of non-best-epoch validation products is not authorized.** This task takes no remote action and does not contact the execution lane; that lane — not this task — must verify actual remote retention.

**Scientific status unchanged.** Eight valid Bayesian (P1–P8) + eight valid random-control (R1–R8) v2 observations; no winner, no promotion, no selection; frozen six-axis search space, the Common-120 raw-space median per-basin NSE screening objective, the screening-400 population, Seed A `model_seed = 967139`, and the frozen `mf12x50000` fidelity all unchanged; SHARED-A5 conclusions remain binding and no 12+12 finding may bypass that downstream gate. This factual closure is carried by the docs-only change that adds this entry, the matching `docs/decision_log.md` entry, and the new design document (no tracked source/config/test change).

## Unattended Moriah W&B-agent launcher hardening CLOSED — the maintained tracked unattended submission path is `bash scripts/submit_sweep_v2_six_axis_wandb_agent_moriah.sh <production-manifest-path>`; guarded Slurm-client / `SLURM_CONF` restore + a `wandb agent` credential-availability preflight (`$HOME/.netrc` fallback via stdlib `netrc`) added to the tracked sbatch; the strict `mode=production` manifest stays authoritative for the W&B sweep id / project / entity; independent Codex review APPROVE; NOT proposal authorization; NO P9, NO Proposals 9–12, NO random-control Wave 3, NO W&B / controller / Slurm / network / training contact (2026-09-08)

**[CLOSED — OPERATIONAL LAUNCHER HARDENING ONLY; NO SCIENTIFIC SCOPE CHANGE; NOT PROPOSAL AUTHORIZATION; NO P9–P12; NO RANDOM-CONTROL WAVE 3; NO W&B / CONTROLLER / SLURM / NETWORK / TRAINING CONTACT; NO WINNER / PROMOTION / SELECTION; NO OBJECTIVE / FIDELITY / SEARCH-SPACE / SEED-POLICY CHANGE; NO SEALED / TEMPORAL-TEST / SPATIAL-HOLDOUT / CALIFORNIA ACCESS]** The post-P8 classification-B launcher-hardening operational milestone — flagged as the required next step in the post-P8 8-plus-8 campaign-health checkpoint entry below — is now closed. It is a small tracked operational-reproducibility change to the existing unattended agent-launch path; it makes no scientific decision and launches no compute. Independently reviewed by Codex, final verdict **APPROVE** (executable-capability test-gate blocker closed; focused Git-Bash suite `12 passed, 2 capability-skipped, 0 failed`; vertical fake-only integration closed and passing; Interface / Consumer Contract Gate closed; secret/auth review PASS; scientific/scope review PASS; no remaining blocking finding). Carried by two commits: the six-file implementation (`Harden unattended Moriah W&B agent launch`) and the docs-only closure that adds this entry and the matching `docs/decision_log.md` entry.

**Maintained unattended submission path.** `bash scripts/submit_sweep_v2_six_axis_wandb_agent_moriah.sh <production-manifest-path>` (the manifest path may also be supplied through `FLASHNH_SWEEP_V2_PRODUCTION_MANIFEST`). The wrapper is intentionally a normal, non-executable, tracked `100644` shell file invoked through `bash` — not a `chmod +x` entry point. It forwards exactly one strict `mode=production` launch manifest to `sbatch --parsable --export="ALL,FLASHNH_SWEEP_V2_PRODUCTION_MANIFEST=<manifest>" scripts/run_sweep_v2_six_axis_wandb_agent_moriah.sbatch` and does nothing else — it sets no `WANDB_PROJECT` / `WANDB_ENTITY` and performs no controller reconciliation.
...
**Manifest / W&B identity (unchanged authority).** The strict `mode=production` manifest remains authoritative for the W&B sweep id, project, and optional entity. The production sbatch obtains them from the maintained pre-agent validator (`create_sweep_v2_six_axis_wandb_bridge_production_sweep.py validate-launch --manifest-path <m> --emit env` / `resolve_validated_production_launch`) and refuses a non-empty inherited `WANDB_PROJECT` / `WANDB_ENTITY` that contradicts the validated manifest before any W&B contact. The submission wrapper does not independently choose project or entity.

**Credential availability (observed-failure hardening).** Immediately before `wandb agent`, the tracked sbatch resolves the agent-login credential in this order: (1) a non-empty inherited `WANDB_API_KEY` in the job environment is used as-is; (2) otherwise the `api.wandb.ai` password is read from `$HOME/.netrc` via the Python standard-library `netrc` parser (the `wandb agent` CLI login path does not itself read `.netrc` — only `wandb.Api()` does — which was the documented cause of the P5 first-attempt "No API key configured" failure, job `46114607`, no W&B run created; retry job `46114616` succeeded); (3) otherwise the job fails now, before any W&B contact. The value is never logged, never copied into the project tree, and not length- or charset-policed; W&B remains authoritative for whether it actually authenticates.
...
**Review / evidence status.** Independent Codex review, final verdict APPROVE; Interface / Consumer Contract Gate closed. The vertical integration test is fake-only: tracked wrapper → fake `sbatch` → the actual tracked `sbatch` invocation form → real strict `validate-launch` → fake `wandb`. No real Slurm, W&B, controller, network, or training activity was required or performed for this qualification. Final reviewed implementation-candidate SHA-256 `01eb87c1d73b719468008d196edf8127a289dc5761c1520a180438bae7085597`.
...
**Explicitly NOT authorized by this closure.** P9; Bayesian Proposals 9–12; random-control Wave 3; any W&B / controller contact or proposal draw; a new W&B sweep; any training or new scientific evaluation; any winner, promotion, or selection; sealed / temporal-test / spatial-holdout / California access. The next scientific/campaign step returns to the user / ChatGPT strategy layer for a separate decision.

## V2 Stage-1 six-axis campaign — post-P8 8-plus-8 health checkpoint complete; sequential Bayesian Proposals 5–8 and random-control Wave 2 ran at `c91a0ff` and each adjudicated VALID from checksum-valid local compact evidence; eight valid Bayesian + eight valid random-control v2 observations now exist; bounded campaign-health review done (READY for a user decision on P9–P12 + Wave 3, one item flagged); NO winner, NO promotion, NO formal near-12 scientific review, NO P9+, NO Wave 3 (2026-09-08)

**[CLOSED — CAMPAIGN-HEALTH CHECKPOINT ONLY; NO WINNER, NO MODEL/CONFIG PROMOTION, NO CAUSAL HYPERPARAMETER INFERENCE, NO FORMAL NEAR-12 SCIENTIFIC REVIEW, NO PROPOSAL 9–12, NO RANDOM-CONTROL WAVE 3, NO NEW W&B CONTACT / CONTROLLER DRAW / SWEEP MUTATION, NO NEW TRAINING, NO NEW FULL-DEVELOPMENT EVALUATION, NO OBJECTIVE / FIDELITY / SEARCH-SPACE / SEED-POLICY CHANGE, NO EARLY STOPPING, NO SEALED / TEMPORAL-TEST / SPATIAL-HOLDOUT / CALIFORNIA ACCESS, NO COMMIT OR PUSH BY THIS REVIEW]** After the Proposal-4 closure, a bounded sequential Bayesian Proposal 5–8 block (controller `wta85z3b`, one new draw per proposal) and random-control Wave 2 (frozen IID manifest rows 4–7, SHA-256 `59be6726b60863aeed1c25e86782bd7a5e1623434ce23c956e66eb54c527c095`, zero W&B contact) were explicitly user-authorized and executed at repo commit `c91a0ff6f809272a746c8f4a85e1b7f4ffb05514`. Their compact completion evidence was retrieved read-only from Moriah into the canonical retained-evidence location `.scratch_local/post_p8_8plus8_review_v001/` and checksum-verified (145/145 files OK across five internal `SHA256SUMS.txt`, 0 failed; source-tarball and frozen-manifest SHA-256s recorded in `retrieval_manifest/`). Each of P5–P8 and R5–R8 is independently adjudicated **VALID** (`execution_status = VALID`, `provenance_stage = VALID`, `objective_eligible = true`, `review_records.workflow_status = pass`, fixed-support contract SHA-256 `cb4ebe86afa501ef3d5929ead5b455f8df06e7d38b58ebf4148f8545fe6851ef` identical to P1–P4/R1–R4). **Eight valid Bayesian (P1–P8) + eight valid random-control (R1–R8) v2 observations now exist.** This entry records the health checkpoint only; it changes no scientific policy and launches no compute. No code, test, config, manifest, launcher, evidence bundle, W&B state, or remote run output was changed.

**Objective unchanged.** All 16 objectives are the best-epoch `flashnh/common120_raw_space_nse_v001` (fixed-support Common-120 raw-space median per-basin NSE over the frozen 400-basin screening population) across the frozen `mf12x50000` budget (12 epochs × 50,000 updates, save every epoch, `performance_early_stopping_enabled = false`), Seed A `model_seed = 967139`, `evaluation_scope = development_validation_2024_only`, `sealed_scope = false` for all 16. The two arms are NOT paired: the Bayesian order is adaptive (dependent observations); the random order is the frozen manifest order.

**Descriptive campaign-health findings (n = 8 per arm; descriptive only, no inferential test, no causal claim).**
- **Objective distribution.** Bayesian P1–P8: mean `0.4115`, median `0.4198`, range `[0.3662, 0.4388]`, sd `0.0248`. Random R1–R8: mean `0.3830`, median `0.3708`, range `[0.3185, 0.4328]`, sd `0.0369`. The Bayesian distribution is shifted up as a whole; the two top-1 values are a near-tie (Bayesian P2 `0.4388` vs random R2 `0.4328`, Δ `0.006`). The Bayesian arm's apparent advantage is in the typical/median draw (it avoids the low region), not in the single best configuration.
- **Cumulative best vs trial count.** BOTH arms reached their final incumbent at trial 2 (Bayesian P2 `0.4388`; random R2 `0.4328`) and neither improved in trials 3–8. Six further Bayesian proposals (P3–P8) produced no new incumbent.
- **Cumulative best vs GPU-hours (loosely comparable — same L4, one GPU/trial, same fidelity; per-trial cost varies with `hidden_size`/`batch_size`).** Both arms reached the final incumbent by ~4.8–6.1 GPU-h. Brief note only; not a controlled efficiency comparison.
- **P1–P4 vs P5–P8 Bayesian concentration.** P1–P4 mean `0.4004` (sd `0.0304`, range `0.0726`); P5–P8 mean `0.4226` (sd `0.0074`, range `0.0186`) — a descriptively clear, tight upward concentration. Frequency ≥ `0.40` (descriptive reference, not a threshold): 2/4 → 4/4. Frequency ≥ `0.42`: 2/4 → 2/4. P5–P8 search-space locations: `seq_length` 108–120 (all four), `hidden_size` 64–128 (never 256), `batch_size` 128 (three of four). The controller moved toward high `seq_length` / smaller `hidden_size` / smaller `batch_size`.
- **R1–R4 vs R5–R8 (control for "later trials just score higher").** R1–R4 mean `0.4005`; R5–R8 mean `0.3654` (lower, wider). The random arm's second half did not show the same upward concentration; this argues against a simple shared later-trial trend, but does not establish a causal Bayesian advantage.
- **Six-axis behavior over all 16.** `learning_rate` sampled `1.30e-4` (R2) to `8.89e-4` (R8/P7) — nothing in `[1e-4, 1.30e-4]` or `[8.9e-4, 1e-3]`. `seq_length` levels 72 and 84 never sampled by either arm. Bayesian pinned at the `seq_length` upper bound 120 in 4/8 proposals (genuine boundary pressure at the top). Bayesian abandoned `batch_size` 512 after P1 and converged toward 128. All `hidden_size` levels used. No exact config duplicates (all 16 `configuration_id` distinct). No controller collapse: run counts 4→5→6→7→8, one new run per proposal, method stays `bayes`, all eight W&B runs `finished`.
- **Sequence-length association update (prior 4+4 finding → 8+8).** STRENGTHENED. Objective by `seq_length` bucket over all 16: sl48 n=1 `0.319`; sl60 n=6 mean `0.368`; sl96 n=1 `0.417`; sl108 n=4 mean `0.424`; sl120 n=4 mean `0.429`. Monotone increase, and it holds independently within each arm. Still descriptive and confounded with the other five axes; **no causal claim**; n = 8 per arm.

**All-16 late-epoch instability.** Only two runs show a clearly large late-epoch decline from best to epoch-12, both Bayesian: **P4** (`best − final ≈ 0.1201`, epoch-12 `0.301`, model still functional, best epoch 5) and **P7** (`best − final ≈ 0.3948`, epoch-12 `0.033`, model effectively destroyed by epoch 12, best epoch 8). These are the only two clearly large late-epoch declines among the 16 observed runs; the largest random-control `best − final` is R2 at ≈ `0.032`. Both P4 and P7 show substantial late degradation, with P7 far more severe (P4's epoch-12 model remains functional; P7's is effectively destroyed). The two share exactly one search-space corner: `{learning_rate ≥ 8e-4 AND batch_size = 256} = {P4, P7}`; R6 (`lr 7.98e-4`, `bs 256`, just below the lr value) is stable (`best − final 0.006`); P6 and R8 (`lr ≈ 8.5–8.9e-4`, `bs 128`) show only mild give-back (`≈ 0.02`). Training losses for P4 and P7 are FINITE throughout (no NaN/Inf) but diverge after the best epoch. Both P4 and P7 had a SUSTAINED good region before divergence (P4 epochs 2–5 plateau ~0.395–0.421; P7 epochs 1–9 plateau ~0.393–0.428), so the best-epoch objective is not rewarding a single transient spike. P4 and P7 each carry 12/12 model + 12/12 optimizer checkpoints with `strict_load` `all_checks_pass: true`, and each best-epoch checkpoint is from a pre-divergence epoch. `best_minus_final` is recorded for every one of the 16 trials and does capture this.

**Is there enough repeated evidence to REQUIRE escalation before P9? — NO, with mandatory user disclosure.** The P4/P7 pair (two independent trials, same describable search-space corner, same divergence-after-best failure mode) crosses from single-run noise to a pattern and MUST be an explicit, documented input to the P9–P12 scoping decision — the campaign should not roll to P9 on autopilot. It does not REQUIRE escalation now because the pattern is (a) confined to a describable sub-region of one axis pair, (b) fully captured by an existing recorded per-trial diagnostic (`best_minus_final`), and (c) downstream-gated by the already-mandatory SHARED-A5 full-development evaluation / checkpoint-selection / promotion gate: the screening-400 objective alone cannot authorize final promotion, so these runs cannot be promoted on the screening objective alone, and the existence of valid pre-divergence best checkpoints does not bypass later promotion review (this does not predict which checkpoint that gate will ultimately select). **P4 → MONITOR. P7 → MONITOR** (individually closer to escalation — model destroyed by epoch 12 — but its objective is from a valid saved pre-divergence checkpoint, its trial provenance is VALID, and it is one trial). **The objective and the frozen fidelity are NOT changed and no early stopping is introduced** — two late-deteriorating runs out of 16 do not warrant a fidelity-policy change, and the best-epoch objective already handles them correctly.

**SHARED-A5 remains binding.** Screening-400 remains the economical Stage-1 v2 HPO / candidate-screening population; it is NOT authoritative for exact within-top-group ordering, absolute expected full-development NSE, checkpoint selection, or promotion; the later full-development evaluation / checkpoint-selection / promotion gate remains mandatory. This health checkpoint performed no full-development evaluation and reopens nothing.

**Launcher-helper classification: B — small tracked operational hardening is the chosen next step before another unattended P9–P12 block; this is an operational reproducibility issue, not a scientific campaign redesign.** The home helper `/sci/home/omripo/moriah_bayes_launch.sh` (untracked) wraps the tracked bridge + `scripts/run_sweep_v2_six_axis_wandb_agent_moriah.sbatch`; a narrow inspection of the retained helper snapshot found NO embedded credential (`WANDB_API_KEY` is resolved from `~/.netrc` at runtime and never logged). Two real gaps recurred that the tracked path does not cover: (1) `SLURM_CONF`/`PATH` are not exported for detached non-interactive children (the `profile.d` Slurm hook is interactive-only); (2) the tracked sbatch has no `~/.netrc` fallback for the `wandb agent` CLI login path — the documented cause of the P5 first-attempt failure (job `46114607`, "No API key configured", no W&B run created; retry job `46114616` succeeded). Not A (the gaps are real and recurred); not C (the orchestration structure did its job). A prepared, non-applied hardening sketch (an `SLURM_CONF`/`PATH` guard block in the sbatch; a tracked `scripts/flashnh_wandb_env_harden.sh`; focused tests) is in the review directory; it is a separate tracked change with its own review, not part of this docs closure. It is not a scientific reason to redesign the HPO campaign, and is not skippable merely because the campaign-health verdict is READY: the user has approved a workflow in which this hardening is completed and reviewed before another unattended P9–P12 block.

**Readiness verdict: READY FOR A USER DECISION on Bayesian Proposals 9–12 + random-control Wave 3**, with one material item the user must explicitly weigh when scoping P9–P12: the P4/P7 `{learning_rate ≥ 8e-4, batch_size = 256}` training-loss-divergence pattern — whether to (a) constrain or watch the `learning_rate` upper region, (b) require per-proposal `best_minus_final` reporting, and/or (c) apply the classification-B launcher hardening. None of these blocks a decision; they are inputs to it. **The next project step is: (1) launcher hardening + its focused review; (2) then a separate user decision/authorization for Bayesian Proposals 9–12 + random-control Wave 3; (3) followed by the planned formal 12-plus-12 scientific review.**

**Explicitly NOT authorized / NOT done by this checkpoint.** No winner; no model/configuration promotion; no formal near-12 scientific review; no Proposal 9, 10, 11, or 12; no random-control Wave 3; no new W&B contact / controller draw / sweep mutation; no new training; no new full-development evaluation; no change to search space, objective, fidelity, screening population, or seed/sampling policy; no early stopping; no sealed / temporal-test / spatial-holdout / California access; no commit or push by this review. Full evidence, the authoritative machine-readable 16-row table, the comparability validation, the figure package, and the complete analysis are in `.scratch_local/post_p8_8plus8_review_v001/` (untracked; `.gitignore` `.scratch_local/`) — `COMPLETION_REPORT.md`, `tables/`, `figures/`, `retrieval_manifest/`, `docs_patch/`. This factual closure is carried by the docs-only change that adds this entry and the matching `docs/decision_log.md` entry; no tracked source/config/test change.

> _(Note added 2026-09-08: the classification-B launcher-hardening step identified in this entry — "the next project step is: (1) launcher hardening + its focused review" — is now COMPLETE and CLOSED; see the "Unattended Moriah W&B-agent launcher hardening CLOSED" entry at the top of this file. The maintained unattended submission path is `bash scripts/submit_sweep_v2_six_axis_wandb_agent_moriah.sh <production-manifest-path>`. This does not authorize Bayesian Proposals 9–12 or random-control Wave 3, which remain a separate user decision, and changes none of this entry's campaign-health findings, the frozen fidelity/objective, or the SHARED-A5 conclusions.)_
>
> _(Note added 2026-09-10: step (2) is COMPLETE — the user subsequently authorized the Bayesian continuation P9–P12 and the frozen random-control Wave 3 R9–R12; both ran at `5a09bce`, each adjudicated VALID, and the campaign reached the planned 12+12 hard stop. Step (3) — the formal 12-plus-12 scientific review — is not done; RD1-C1 froze its methodology (2026-09-09 design freeze) and execution/evidence closure is recorded in the "Stage-1 v2 six-axis 12+12 campaign EXECUTION/EVIDENCE CLOSURE" entry at the top of this file. The "eight valid Bayesian (P1–P8) + eight valid random-control (R1–R8)" counts in this entry are superseded by 12+12; the campaign-health findings, frozen fidelity/objective, and SHARED-A5 conclusions are unchanged.)_

## V2 Bayesian Proposal 4 adjudicated VALID (Bayesian observation 4) — explicitly user-authorized as a single sequential proposal after SHARED-A5 closed; full frozen `mf12x50000` fidelity; objective `0.42119978965037297` at epoch 5; close-human-review-through-Proposal-4 phase complete; four valid Bayesian + four valid random-control v2 observations now exist; NO winner, NO promotion, NO Proposal 5–8, NO random-control Wave 2 (2026-09-07)

**[CLOSED — DOCUMENTATION-ONLY CLOSURE; NO WINNER, NO MODEL/CONFIG PROMOTION, NO CAUSAL HYPERPARAMETER INFERENCE, NO PROPOSAL 5–8, NO RANDOM-CONTROL WAVE 2, NO NEW W&B CONTACT / CONTROLLER DRAW / SWEEP MUTATION, NO NEW TRAINING, NO NEW FULL-DEVELOPMENT EVALUATION, NO SEALED / TEMPORAL-TEST / SPATIAL-HOLDOUT / CALIFORNIA ACCESS]** Bayesian Proposal 4 from controller `wta85z3b` was explicitly user-authorized as a single sequential proposal after the SHARED-A5 screening-adequacy audit closed. It ran at exact commit `a0c5e7516944d5b2c26372408c5edc76b61ee0ae` and has been independently verified from its compact completion bundle (`.scratch_local/v2_proposal4_completion_a0c5e75_46113284_20260907T172619Z/`, local checksum verification 25/25 OK) and adjudicated **VALID (v2 Bayesian observation 4)**. This entry records the result only; it changes no scientific policy and launches no compute.
...
**Explicitly NOT authorized / NOT done by this closure.** No winner; no model/configuration promotion; no Proposal 5, 6, 7, or 8; no random-control Wave 2; no new W&B contact / controller draw / sweep mutation; no new training; no new full-development evaluation; no change to search space, objective, fidelity, screening population, or seed/sampling policy; no early stopping; no sealed / temporal-test / spatial-holdout / California access; no commit or push by this milestone. The next user decision is whether to authorize a bounded Bayesian Proposal 5–8 block (sequential) plus random-control Wave 2 (frozen manifest rows 4–7, runnable in parallel), followed by a paused eight-plus-eight campaign-health review. This factual closure is carried by the docs-only commit that adds this entry and the matching `docs/decision_log.md` entry (no tracked source/config/test change).

> _(Note added 2026-09-08: the "no Proposal 5, 6, 7, or 8; no random-control Wave 2" clause of this closure is now superseded — a bounded sequential Bayesian Proposal 5–8 block and random-control Wave 2 (frozen manifest rows 4–7) were subsequently and explicitly user-authorized, ran at repo commit `c91a0ff`, and each completed VALID (eight valid Bayesian + eight valid random-control v2 observations now exist). See the "V2 Stage-1 six-axis campaign — post-P8 8-plus-8 health checkpoint" entry at the top of this file. That authorization changes none of this entry's Proposal-4 adjudication, the frozen fidelity, the objective definition, or the SHARED-A5 conclusions. The formal near-12 scientific review, a winner, and promotion remain not done and separately gated.)_

## `[SHARED-A5]` development-population Common-120 screening-adequacy audit **CLOSED** — all seven canonical 2,307-basin audit rows complete; screening-400 vs full-development seven-configuration comparison frozen; top-3 group membership unchanged; Tier-C escalation NOT triggered; screening-400 retained as the economical Stage-1 HPO screening population but is NOT authoritative for exact top-group ordering or final promotion; a later full-development checkpoint-selection / promotion gate remains required (2026-09-07)

**[CLOSED — SCREENING-ADEQUACY AUDIT ONLY; NO WINNER, NO MODEL/CONFIG PROMOTION, NO PROPOSAL 4, NO RANDOM-CONTROL WAVE 2, NO NEW W&B CONTACT, NO NEW TRAINING, NO NEW SEARCH SPACE, NO FULL-DEVELOPMENT MULTI-EPOCH CHECKPOINT SELECTION, NO SEALED / TEMPORAL-TEST / SPATIAL-HOLDOUT / CALIFORNIA ACCESS]** SHARED-A5 is now scientifically closed. The SHARED-A3 design (population transfer only: screening-selected `best_epoch` held fixed per configuration; only the evaluation population changes from screening-400 to the canonical 2,307-basin development population) has been executed for all seven configurations and the prospectively-frozen SHARED-A3 §7 rubric applied.

**Execution — decision-relevant facts only.** P1 (`sweep_v2_cfg_26daf2a8f293007dd60b`, Bayesian Proposal 1) was the prior qualified audit-path canary and is **reused unchanged** — its canonical row is byte-identical to the pre-hardening reference (SHA-256 `3166aba529ef780c0a3ce35aaed26ac54286dd04a0eb3ae18db2fb57c0190097`, full-development median Common-120 raw-space NSE `0.36470379840322187`). The six remaining configurations P2/P3/R1/R2/R3/R4 each completed one full-development NeuralHydrology evaluation (Slurm GPU jobs `46112127` P2, `46112145` P3, `46112155` R1, `46112157` R2, `46112159` R3, `46112161` R4; partition `catfish`, one L4 each; independent, no job array) plus one dependent CPU-only canonical scoring job (`46112140`, `46112154`, `46112156`, `46112158`, `46112160`, `46112162`; partition `glacier`, `afterok`). Generic per-entry preflight ran as prep job `46112006` (6/6 PASS). All twelve jobs `COMPLETED` ExitCode `0:0`. Every one of the six canonical rows: exactly 2,307 basins, **0 missing / 0 extra / 0 excluded**; `canonical_completeness` / `canonical_population_verified` / `provenance_verified` all true; shared `membership_ids_sha256 = 397ab432564c18c3abc5158a47ada2b28840bbf6f0c213d2475444fded33858f`, shared `contract_checksum_sha256 = 1100c736693f2db3dd15f34cfdd89da6c06f59320b9afd0d3a78cfc2427ed46b`, shared `population_role = development_train`, `expected_population_size = 2307` — identical across all seven rows. Source/staged checkpoint, config and scaler byte identities were unchanged pre vs post evaluation. No W&B runtime contact; all evaluation under Slurm; no epoch reselection or training. The first replacement-run GPU launch attempt failed at Slurm allocation for a purely operational sbatch-header reason and was corrected with no scientific change; the final scientific rows come only from the valid replacement executions. The optimized evaluator (`2222a80`) scored each full-development row in ~5–11 min. Compact evidence: `reports/devpop_audit_common120_seven_config_v001/` and `.scratch_local/devpop_audit_remaining_six_v001/evidence/devpop_audit_six_bundle_20260907T123328Z/`; the six ~488 MB result pickles remain on Moriah and were not transferred.

**Frozen seven-configuration comparison.** Produced locally by the committed aggregator `aggregate_devpop_audit_seven_rows(..., allow_fixture_rows=False)` (no exact tie — it fails closed on one and did not). Output `reports/devpop_audit_common120_seven_config_v001/seven_config_comparison_v001.json`, SHA-256 `590000b2af62ed608d6999885b7c374e96fa54f26ddead6d2a4a69573d9a9f7a`; `selection_manifest_sha256 = 3b1a471164d6d85dd0071f45c069f9c5619ad89e3f4fafc269a8fb23ae19b9f9`.

| full rank | label | configuration_id | arm | screening `best_epoch` | screening-400 median NSE | full-development 2,307 median NSE | signed Δ (full − screening) | screening rank |
|---|---|---|---|---|---|---|---|---|
| 1 | R2 | `sweep_v2_cfg_5f9f8c5442fb1200d68e` | random_control | 6 | `0.43276757946464195` | `0.431472951033149` | `-0.0012946284314929724` | 2 |
| 2 | P2 | `sweep_v2_cfg_dd9f7b847381f637439d` | bayesian | 9 | `0.4388098707961096` | `0.4274593325275011` | `-0.01135053826860849` | 1 |
| 3 | R4 | `sweep_v2_cfg_54d75cc3479d174bfa7e` | random_control | 5 | `0.42770033214795594` | `0.4266591028699196` | `-0.0010412292780363486` | 3 |
| 4 | R3 | `sweep_v2_cfg_54476936ed984e3a4d64` | random_control | 5 | `0.3717623575075273` | `0.36679743945469` | `-0.004964918052837275` | 5 |
| 5 | P1 | `sweep_v2_cfg_26daf2a8f293007dd60b` | bayesian | 1 | `0.36621751839318467` | `0.36470379840322187` | `-0.0015137199899628029` | 7 |
| 6 | P3 | `sweep_v2_cfg_00be6b4182e764f24f15` | bayesian | 9 | `0.37530856722966427` | `0.36272353973025173` | `-0.012585027499412538` | 4 |
| 7 | R1 | `sweep_v2_cfg_a476ab2b0f9fb7b42863` | random_control | 10 | `0.36989789510584137` | `0.36257972359430857` | `-0.0073181715115328005` | 6 |

- **Cross-population.** Screening top-3 = `{P2, R2, R4}`; full-development top-3 = `{R2, P2, R4}` — **top-3 group membership unchanged** (`top3_membership_stable = true`, `entered_top3 = []`, `exited_top3 = []`). Internal ordering changed: P2 ↔ R2 swap at ranks 1–2 (`internal_top3_order_changed = true`). Spearman `0.7857142857142857`, Kendall τ `0.6190476190476191`, `4` pairwise rank reversals. All seven full-minus-screening median-NSE deltas are **negative** (full-development median below screening median), absolute range ≈ `0.00104` (R4) to `0.01259` (P3); the two largest absolute transfer drops are the Bayesian configurations P2 (`-0.01135`) and P3 (`-0.01259`). No absolute NSE-delta threshold is defined or introduced.
- **Rubric outcome.** `interpretation.tier = null`, `escalation_required = false`, `escalation_reason = null`. The sole mechanically-crisp SHARED-A3 §7 escalation rule (Tier C = top-3 membership change) is **not triggered**. Characterization: top-3 stable; some Tier-B-type internal movement (P2↔R2 top swap, 4 pairwise reversals, P3 rank 4→6), but not decision-changing for continued screening use. Distinguishing a formal Tier A from Tier B is human scientific judgement and is not forced here.

**Approved scientific interpretation (user-authorized 2026-09-07).**
- **Decision.** Screening-400 is **adequate as the economical working screening population for continued Stage-1 v2 HPO / candidate screening.** The basis is not perfect rank preservation: the same three configurations form the top group under both populations, that top-3 group stays clearly separated from the remaining four, and no result changes the substantive conclusion about which configurations occupy the strong-candidate group.
- **Qualification.** Screening-400 is **not authoritative for exact within-top-group ordering or for absolute expected full-development NSE** (P2/R2 swap ranks 1–2; four pairwise reversals; per-configuration population-transfer shifts vary; all seven full-population medians sit below their screening medians). Therefore: continue using screening-400 as the economical screening population; treat its exact within-top-group ranking cautiously; do not use screening scores alone for final promotion; the already-required later **full-development evaluation / checkpoint-selection / promotion gate remains mandatory**.
- Screening and full-development evaluation are **not** claimed equivalent. No causal claim is made that Bayesian optimization produces poorer population transfer.

**Explicitly NOT authorized by this closure.** No winner; no model/configuration promotion; no Proposal 4; no random-control Wave 2; no new W&B contact; no new training; no new candidate/search space; no full-development multi-epoch checkpoint selection; no sealed / temporal-test / spatial-holdout / California access. Those remain later user decisions. The next phase is a return to optimization planning; a fresh review conversation will determine the next bounded optimization milestone. This factual closure is carried by the docs-only commit that adds this entry (no tracked source/config/test change).

> _(Note added 2026-09-07, later the same day: the "no Proposal 4" clause above is now superseded — Proposal 4 was subsequently and explicitly user-authorized as a single sequential Bayesian proposal and completed VALID as Bayesian observation 4. See the "V2 Bayesian Proposal 4 adjudicated VALID" entry at the top of this file. That authorization is separate from this audit closure and changes none of the SHARED-A5 screening-adequacy conclusions. Random-control Wave 2 remains unlaunched and separately gated.)_

## `[SHARED-A5]` Phase A — remaining-six audit execution plumbing CLOSED: configuration-generic per-entry preflight, one-entry canonical scorer, seven-row SHARED-A3 comparison collector, and execution-time scaler-provenance path are implemented and tested (commit `f6ad90b`); Codex found one seven-row-collector provenance-binding defect, now repaired (embedded checkpoint identity + evaluator provenance are bound to the row and the frozen manifest entry; the P1→P2 relabel is rejected); finding-specific re-review PASS; 212 focused tests pass; the six P2/P3/R1–R4 evaluations are still NOT launched (2026-09-07) _(SUPERSEDED — see the SHARED-A5 screening-adequacy audit CLOSED entry above; all six ran on 2026-09-07)_

**[CLOSED — EXECUTION PLUMBING ONLY; NO PRODUCTION RUN, NO SEVEN-ROW COMPARISON, NO SCREENING-ADEQUACY CONCLUSION, NO WINNER / PROMOTION / PROPOSAL 4 / WAVE 2 / SEALED ACCESS]** The producer/consumer vertical is now closed for the remaining-six execution path: a validated frozen selection-manifest entry drives a configuration-generic preflight and one-entry canonical scoring through the unmodified SHARED-A2 evaluator, and seven canonical rows feed the thin SHARED-A3 §7 comparison collector (only the mechanically-crisp top-3-membership-change → Tier C escalation rule is encoded; no tie rule, no NSE-delta threshold invented). Execution-time scaler byte provenance is recorded (`audit_execution_time_scaler_provenance` + optional `expected_scaler_sha256` gate); historical screening-time scaler identity remains unavailable and is not claimed.

**Codex review.** Codex initially found one material provenance-binding defect in the seven-row collector: the embedded `checkpoint_identity` / verified evaluator provenance were not cross-checked against the row's outer identity or the frozen manifest entry, so a valid row could be relabelled to another configuration's identity while keeping the original nested provenance. Repaired by binding trial/configuration identity, checkpoint epoch, checkpoint filename, checkpoint SHA-256, provenance schema, and `provenance_verified` to both the row's own authoritative identity and the corresponding frozen manifest entry; the exact P1→P2 relabel regression is now rejected. Finding-specific Codex re-review returned PASS. Final focused Phase-A test result: 212 passed, 1 pre-existing warning. Committed as `f6ad90bd33b6aa05b697fa35a18997e0c9e786c7` (parent `2222a8094360718fac00c75c4e4e61c9df143ed0`); this factual closure is carried by the docs-only commit that adds this entry.

**Explicitly NOT done.** _(HISTORICAL / SUPERSEDED by the SHARED-A5 screening-adequacy audit CLOSED entry at the top of this file, 2026-09-07: the six P2/P3/R1–R4 evaluations, the seven-row comparison, and the screening-adequacy conclusion are now done. Still not done / not authorized: any winner or promotion, full-development multi-epoch checkpoint selection, Proposal 4, random-control Wave 2, new W&B contact, and sealed-scope access.)_ At the time this plumbing entry was written: the six P2/P3/R1–R4 development-population evaluations had not been launched; there was no seven-row real scientific comparison, no screening-vs-full ranking-stability result, no screening-adequacy conclusion, no winner, no promotion, no full-population checkpoint-selection study, no Proposal 4 / random-control Wave 2 authorization, and no sealed / temporal-test / spatial-holdout / California access. No objective, search-space, fidelity, sampling-policy, or interpretation-rule change.

## `[SHARED-A5]` development-population Common-120 audit production execution — canonical 2,307-basin artifact materialized + verified; the P1 audit-path qualification canary is COMPLETE and passed every gate; the per-basin evaluator performance defect is repaired and committed (`34138c27`) and now qualified byte-identical on real P1 data (Slurm `46103139`); the seven-entry selection manifest is materialized on Moriah; P1 canary + evaluator-hardening qualification CLOSED; remaining six configs NOT run; no winner/promotion (2026-09-06) _(SUPERSEDED — see the SHARED-A5 screening-adequacy audit CLOSED entry above; the remaining six ran and the audit is scientifically closed on 2026-09-07)_

**[SUPERSEDED 2026-09-07 — this entry recorded the P1-only audit-path canary state; the full seven-configuration screening-adequacy audit is now CLOSED (see the top entry). Retained for the P1 canary + evaluator-hardening qualification detail, which stands unchanged.]** SHARED-A5 opens the real screening-400 vs full-2,307-basin development-population audit designed in SHARED-A3 and made implementation-ready in SHARED-A4. The `[SHARED-A4]` entry below is now **historical** for the "explicitly NOT done" list — the real artifact and the first real evaluation exist; its design/review content stands.

**Canonical Common-120 support artifact — materialized and verified.** `common120_devpop_audit_support_contract_v001.json`, file SHA-256 **`6868a1b2cb11454b538deda225369993b7a70bf06876ee3311d895e9712dc339`**, internal canonical checksum **`1100c736693f2db3dd15f34cfdd89da6c06f59320b9afd0d3a78cfc2427ed46b`**, built once via `common120_support_builder.build_common120_support_for_development_population` over the canonical development population (membership-ids SHA-256 **`397ab432564c18c3abc5158a47ada2b28840bbf6f0c213d2475444fded33858f`**, 2,307 basins, role `development_train`); lead 6, target `qobs_mm_per_h_lead06`, 2024 validation window, seq-length floor 120, zero zero-support basins. Shared unchanged and checksum-pinned across all seven configurations.

**Seven-entry checkpoint-selection manifest — populated, frozen, and materialized on Moriah.** `.scratch_local/devpop_audit_a5/devpop_audit_selection_manifest_v001.json`, materialized byte-identically for the production audit at `/sci/labs/efratmorin/omripo/Flash-NH/runs/devpop_audit_common120_v001/devpop_audit_selection_manifest_v001.json`; file SHA-256 **`4ce75f1e7fb74becf765ab59e17ffdaea44f2dd5f71cf827468e87388b9ac1a0`**, internal order-independent `manifest_sha256` **`3b1a471164d6d85dd0071f45c069f9c5619ad89e3f4fafc269a8fb23ae19b9f9`**. Committed validator (`load_devpop_audit_selection_manifest`) passes: exactly seven entries — P1/P2/P3 (Bayesian {1,2,3}) + R1/R2/R3/R4 (random-control {1,2,3,4}) — no duplicate trial/config identities, each entry's frozen screening score + `screening_best_epoch` + `checkpoint_filename` + `checkpoint_sha256` present and grammar-cross-checked. The Moriah copy carries the same file SHA-256 and is shared and immutable across all seven configurations.
...
- **Two producer defects found and repaired (both committed before this patch).** (1) frozen PT dynamic inputs were initially omitted from the prepared audit run dir — fixed in `012ffd6` ("Preserve frozen PT inputs in devpop audit runs"); (2) a reused pilot setting `validate_n_random_basins=1000` silently truncated the intended full 2,307-basin evaluation — fixed in `4bce760` ("Evaluate full development population in devpop audit"). The corrected P1 NeuralHydrology full-development evaluation is Slurm job **46099909** (`COMPLETED`); its result object contains exactly the canonical 2,307 basins (set equality, membership SHA-256 match, 0 missing / 0 extra).
- **Canonical scoring — Slurm job 46101232 — passed all gates.** Gate 0 identity preflight, Gate 1 canonical expected-population spec, Gate 2 result-object membership, Gate 3 result-shape (2,307 basins, 2024 window), Gate 4 provenance receipt, Gate 5 canonical contract validation, Gate 6 `evaluate_devpop_common120_audit_row(require_canonical=True)`, Gate 7 paired comparison. Row: `canonical_completeness=true`, `canonical_population_verified=true`, `provenance_verified=true`, `n_basins_evaluated=2307`, `n_basins_excluded=0`, **full-development P1 median Common-120 raw-space NSE `0.36470379840322187`**.
- **Paired observation (one data point, not a conclusion).** Screening-400 P1 median `0.36621751839318467`; full-2,307 P1 median `0.36470379840322187`; signed delta (full − screening) **`-0.0015137199899628029`**. This is a technical audit-path qualification plus a single paired point; **P1 alone does not establish whether the screening-400 population is globally adequate** and does not touch the SHARED-A3 §7 prospectively-frozen interpretation rubric (top-3 membership change remains the escalation criterion).

**Evaluator performance defect — confirmed from code, repaired, and committed as `34138c27` ("Harden devpop audit evaluator").** `deserialize_contract_support` unconditionally re-ran `validate_devpop_audit_contract`, whose checksum step re-serializes (`json.dumps`) and SHA-256s the entire ~contract-sized payload; the SHARED-A2 evaluator calls it once per basin inside its per-basin loop, so the canonical 2,307-basin contract was fully re-serialized and hashed ~2,307 times — an O(N_basins × contract_size) pathology (**Gate 6 evaluator wall time 7,763.6 s ≈ 2.16 h** for P1). Repair (semantics-preserving, additive): a new **private, internal** fast accessor `_deserialize_contract_support_prevalidated(contract, basin_id)` in `devpop_common120_audit_contract.py` (underscore-prefixed, deliberately **not** in `__all__`, not a public unvalidated entry point) performs only the O(1) membership check + shared date-array decode and is used by `_basin_support_metrics` because `evaluate_devpop_common120_audit_row` already validates the immutable contract once at its boundary. That evaluator now builds exactly one local plain-dict snapshot (`validated_contract = dict(contract)`), validates that object, and reads every subsequent contract field — identity checks, target/lead/period, the per-basin support accessor, the completeness gate, the returned identity/checksum fields — from that same validated snapshot (it never validates one mapping and later consumes another). `deserialize_contract_support` stays the public, safe accessor with its scientific meaning unchanged (it now validates, then delegates to the private helper). The full contract validator is now invoked a small constant number of times per audit row (boundary + completeness gate), independent of basin count. No scientific/evaluator semantics change: schema, checksum, canonical population identity, Common-120 support identity, provenance, timestamp matching, completeness, finite-NSE, zero-exclusion, and sealed-scope protections are all unchanged and still enforced before any successful canonical row. Focused + adjacent regression tests added and green.

**Real-data equivalence qualification — COMPLETE and PASSED (CPU-only Slurm job `46103139`, `COMPLETED 0:0`, elapsed `00:11:53`; no GPU, no NH inference, P1 run dir untouched).** The optimized canonical evaluator at `34138c27` re-scored real P1 and reproduced the pre-hardening result exactly: **`nse_median == 0.36470379840322187`**, contract internal checksum `1100c736…`, membership-ids SHA-256 `397ab432…`, checkpoint SHA-256 `71c920e2…`, `canonical_completeness` / `canonical_population_verified` / `provenance_verified` all true, 2,307 evaluated, 0 excluded, `basins_excluded=[]`, zero non-finite sim at admitted support. Stronger: full canonical audit-row structural equality against the pre-hardening reference row (Slurm `46101232`) — differing paths `[]`, all 2,307 per-basin rows deep-equal, aggregate / provenance / completeness deep-equal, **28/28** explicit equivalence checks passed, and the new canonical row is **byte-identical** to the reference (SHA-256 `3166aba529ef780c0a3ce35aaed26ac54286dd04a0eb3ae18db2fb57c0190097`). Evaluator call time fell from **`7763.6 s` to `649.4 s` (~12×)** — engineering evidence only, not a scientific acceptance threshold. The evaluator optimization is therefore **qualified on real P1 data as scientifically equivalent**; the P1 audit-path canary and the evaluator-hardening qualification are **CLOSED**. Compact evidence: Moriah `/sci/labs/efratmorin/omripo/Flash-NH/evidence/devpop_audit_p1_equivalence_46103139/`, local `.scratch_local/devpop_audit_a5/evidence_devpop_audit_p1_equivalence_46103139/`.

**Explicitly NOT done / NOT decided.** _(HISTORICAL / SUPERSEDED by the SHARED-A5 screening-adequacy audit CLOSED entry at the top of this file, 2026-09-07: the remaining six evaluations, the seven-row aggregation, and the screening-adequacy conclusion are now complete. Still not done / not authorized: any winner or configuration promotion, full-population multi-epoch checkpoint selection, Proposal 4, random-control Wave 2, new W&B contact, and sealed-scope access.)_ At the time this entry was written the campaign was ready for the remaining six P2/P3/R1–R4 development-population evaluations, but those six had **not** been launched; there was no seven-row aggregation, no screening-vs-full ranking-stability result, no screening-adequacy conclusion, no winner, no configuration promotion, no full-population checkpoint-selection study, no Proposal 4 or random-control Wave 2 authorization, no sealed / temporal-test / spatial-holdout / California access, and no change to objective, search space, fidelity, sampling policy, or interpretation rules. The evaluator-hardening commit `34138c27` is on `master`; this factual closure is recorded by the docs-only commit that carries this update.

## `[SHARED-A4]` closes the LOCAL/SYNTHETIC development-population Common-120 audit orchestration implementation (seven-entry selection-manifest contract + audit evaluation-run producer + vertical producer→A2 path); independently reviewed PASS; NO real checkpoints, NO real artifact, NO execution (2026-09-05)

**[CLOSED — LOCAL/SYNTHETIC IMPLEMENTATION READINESS ONLY; NOT THE SCIENTIFIC AUDIT; NO REMOTE/W&B/SLURM/NH/SEALED ACCESS]** This closes SHARED-A4 *implementation readiness* for the seven-configuration screening-400 vs full-2,307-basin development-population audit. It does **not** open, run, or conclude the scientific audit itself — that remains SHARED-A5+ gated work requiring separate authorization.

**Implemented — four new files, local/synthetic tests only** (`src/baseline/devpop_audit_selection_manifest.py`, `src/baseline/devpop_audit_eval_run_producer.py`, `tests/test_devpop_audit_selection_manifest.py`, `tests/test_devpop_audit_eval_run_producer.py`):
- One **atomic seven-entry checkpoint-selection manifest contract** — Bayesian proposals 1–3 + frozen random-control Wave-1 rows 0–3 (`proposal_order` 1–4); composition is exact and fail-closed.
...
- **Fail-closed checkpoint identity**: `checkpoint_filename` must equal the existing `weight_stem(screening_best_epoch) + ".pt"` convention (reused, not duplicated); filename, SHA-256, and source bytes are all verified before any copy/rename.
- **Audit-specific evaluation-run producer** consumes the *validated full seven-entry manifest plus an entry selector*, proves selected-entry membership, preserves the exact `selection_manifest_sha256` into its persisted run manifest, prepares the full-development validation configuration via existing lower-level machinery (freshly re-derived development population), and verifies canonical population, target, lead 6, the 2024 validation window, screening configuration identity against the loaded fixed-support contract, checkpoint identity, and copied scaler/checkpoint provenance.
- **One vertical local/synthetic path demonstrated end to end**: validated seven-entry manifest → selected member → producer → persisted manifest/checkpoint/config provenance → A2 provenance receipt → existing unmodified A2 evaluator → one canonical synthetic audit row.

**Independent review.** First Codex review found two material provenance defects: (1) the producer was not tied to the exact atomic seven-entry manifest (it accepted an arbitrary standalone entry); (2) `screening_best_epoch` was not bound to the source checkpoint filename/epoch. Both were repaired at a minimal, additive level. Focused Codex re-review returned **PASS** — exact seven-entry manifest binding closed, epoch/checkpoint mismatch closed, vertical producer→A2 contract closed, no new material defect.

**Test evidence (not rerun for this docs closure).** 22 SHARED-A4 tests passed; 206 relevant regression tests passed.

**Explicitly NOT done — SHARED-A5+ gated, separate authorization required.** No recovery of the seven real training-run/checkpoint locations; no real checkpoint hashes computed or frozen; no real populated seven-entry selection manifest; no canonical real 2,307-basin Common-120 support artifact materialized; no Moriah/h2o/W&B/Slurm contact; no NeuralHydrology execution; no canary; none of the seven real development-population evaluations; no real audit-row aggregation; no scientific comparison result; no winner selection or configuration promotion; no sealed-scope access; no Proposal 4 / random-control Wave 2 authorization; no change to objective, search space, fidelity, sampling policy, or interpretation rules.

> **HISTORICAL / SUPERSEDED (see the SHARED-A5 entry at the top).** The "Explicitly NOT done" list above was accurate at the 2026-09-05 A4 closure. As of the 2026-09-06 SHARED-A5 entry it is stale: the canonical 2,307-basin Common-120 support artifact **is** materialized and verified, the seven-entry selection manifest **is** populated and validator-passing, and the P1 audit-path qualification canary **has** run end to end on real NeuralHydrology output (one real development-population evaluation). Still genuinely not done: the remaining six evaluations, the seven-row aggregation, any screening-vs-full conclusion, any winner/promotion, Proposal 4 / Wave 2, and any sealed-scope access. The A4 design and review content below remains valid.

## `[SHARED-A3]` closes the development-population Common-120 audit scientific-entry and producer-contract design freeze; still no execution, checkpoint population, or real artifact (2026-09-05)

**[CLOSED — DOCUMENTATION-ONLY DESIGN FREEZE; NO PRODUCTION AUDIT, NO CHECKPOINT-HASH POPULATION, NO REMOTE/W&B/SEALED ACCESS]** Full detail is frozen in `docs/stage1_devpop_common120_audit_contract_v001.md`; only a concise summary is kept here.

**Frozen.** The seven-configuration audit isolates population transfer only (screening-selected `best_epoch` held fixed per configuration; only the evaluation population changes from screening-400 to the canonical 2,307-basin development population) — it does not select a winner, determine what full-population validation itself would pick, or promote anything. Screening-selected `best_epoch` is explicitly **not** automatically authoritative for eventual promotion, which requires its own future full-population checkpoint-selection study. A single seven-entry, hash-pinned checkpoint-selection manifest design is frozen (fields + authority only — not populated with real checkpoint paths/hashes here). The canonical Common-120 support builder (`common120_support_builder.build_common120_support_for_development_population`) is confirmed as the sole authoritative entry point, built once and shared unchanged across all seven configurations. The producer -> A2-consumer vertical path is frozen; neither `pilot_full_validation.build_pilot_full_validation_bundle` nor `nh_seed_evaluation.prepare_development_population_eval_run_dir` is frozen as the reused producer as-is (wrong scope/marker semantics) — SHARED-A4 must build an additive, explicitly cross-checked producer atop lower-level shared primitives. A thin seven-row aggregation contract is frozen. A pre-result interpretation rubric is frozen **prospectively** (evidence dimensions, escalation tiers keyed on top-3 membership stability) with no post-hoc NSE-delta threshold. Failure/restart/overwrite fail-closed principles are frozen. The milestone boundary is corrected: **SHARED-A4** is local/synthetic implementation only (no Moriah/W&B/Slurm, no real checkpoint hashes, no real artifact, no real NH run); **SHARED-A5** is the separately-gated future milestone for real evidence recovery, artifact materialization, and production execution.

**Not done (at the A3 design-freeze; see the SHARED-A4 closure above for current local/synthetic implementation status).** No checkpoint paths/hashes populated; no Common-120 2,307-basin artifact materialized; no NH run; no remote/W&B contact; no promotion; no Proposal 4 / Wave 2 authorization.

## `[SHARED-A1]` + `[SHARED-A2]` close the LOCAL reusable development-population Common-120 audit foundation; production orchestration, checkpoint-selection, the real seven-configuration 2,307-basin audit, and scientific acceptance criteria remain future gated work (2026-09-03)

**[CLOSED — LOCAL REUSABLE FOUNDATION ONLY; NO PRODUCTION AUDIT, NO CHECKPOINT SELECTION, NO REMOTE/W&B/SEALED ACCESS]** The screening-400 vs full-population development-validation audit (the next gate, below) no longer has a wholly undefined implementation. Two local increments now establish its reusable foundation.

- **`[SHARED-A1]` — committed at HEAD `ffc798b` ("Add development-population Common-120 audit contract").** The canonical development-population Common-120 audit contract and its strict, fail-closed completeness boundary: an explicit `ExpectedPopulationSpec`, the diagnostic `common120_raw_space_nse_devpop_audit_v001` contract, frozen per-basin admitted-timestamp support, and a completeness gate that forbids any excluded basin (`src/baseline/devpop_common120_audit_contract.py`, `src/baseline/common120_support_builder.py`, `fixed_support_contract_v2.py`; contract tests).

- **`[SHARED-A2]` — this closure.** The one-configuration audit consumer/evaluator seam (`src/baseline/devpop_common120_audit_evaluator.py` + `tests/test_devpop_common120_audit_evaluator.py`): explicit caller-supplied trial/checkpoint identity → an authoritative producer→consumer provenance receipt binding trial id, configuration id, period, checkpoint epoch, checkpoint filename/hash, and canonical period-results relative path/hash → fail-closed verification of the actual checkpoint and result-artifact **bytes** → consumption of the already-produced NH period-results → exact Common-120 support alignment (no silent realignment or truncation) → the qualified raw-space path (`derive_basin_area_km2_from_netcdf`, `evaluate_basin_raw_space`, `aggregate_raw_space_metrics`) reused verbatim → the A1 completeness gate → one fixture-or-canonical-labelled audit row. A single synthetic vertical test drives the whole boundary through the real qualified raw-space helpers (not monkeypatched) and preserves fixture-vs-canonical label separation; canonical completeness is recognised only by routing through A1's gate, with no forgeable row-level "assert canonical" helper.

**Review.** Independent review of A2 found two material interface defects — a result↔checkpoint/trial provenance gap and a forgeable canonical-row assertion. One consolidated correction resolved both; targeted re-review found no new material regressions and closed the Interface / Consumer Contract Gate. Focused A2 tests: 18 passed; A1 contract tests unchanged and passing.

**Still future gated work (not decided, not executed here).** Checkpoint-selection policy (which checkpoint(s) to audit); production evaluation/inference orchestration; real producer provenance-receipt emission during that evaluation; compute/remote plan and any Moriah/Slurm/W&B execution; the actual seven-configuration 2,307-basin development-validation audit; and the scientific acceptance / ranking-stability interpretation criteria. Each requires a separate inspected planning milestone.

**Preserved.** Seven valid, materially varied v2 configurations (three Bayesian + four random-control) still stand. The screening-400 ranking remains provisional and descriptive. No winner, no promotion, no causal hyperparameter inference. Proposal 4 and random-control Wave 2 remain unlaunched and separately gated behind this audit.

## V2 Proposal 3 adjudicated VALID (Bayesian observation 3); random-control Wave 1 (frozen rows 0–3) independently adjudicated VALID; seven valid materially-varied v2 configurations now exist; screening-400 vs full-population development-validation audit is the next gate; Proposal 4 and random Wave 2 remain unlaunched and separately gated (2026-09-02)

**[CLOSED — DOCUMENTATION-ONLY; NO WINNER, NO PROMOTION, NO CAUSAL HYPERPARAMETER INFERENCE]** Two v2 Stage-1 validation-optimization executions that ran concurrently at exact commit `b4fe5eda96f15276354fa4de882c1291c93710a7` have completed and been independently adjudicated by read-only scientific + operational completion review: the third sequential Bayesian proposal from controller `wta85z3b`, and the first four-job wave of the frozen IID random-control arm. Each result is a valid observation, not a selected, promoted, or winning configuration; the frozen search domain, fidelity, objective (`flashnh/common120_raw_space_nse_v001`, maximize), sampling design, and scientific policy are unchanged.

**Proposal 3 — VALID, v2 Bayesian observation 3.** Controller `omri-porat1-huji/flashnh-stage1/wta85z3b`; W&B run `voo2a4i1` (state `finished`, `flashnh/valid=true`); Slurm job `46017631` (`catfish`/`catfish-02`, one NVIDIA L4, `COMPLETED 0:0`, runtime `02:52:51`). `search_arm=bayesian`, `proposal_order=3`, `execution_generation=1`, `configuration_id=sweep_v2_cfg_00be6b4182e764f24f15`, `trial_id=…__bayesian__proposal003__sweep_v2_cfg_00be6b4182e764f24f15__mf12x50000__seedA967139__attempt001`; Seed A `model_seed=967139`; fidelity `mf12x50000`; `evaluation_scope=development_validation_2024_only`; `sealed_scope=false`. Launch-manifest SHA-256 `7b96c3636a13b1c76a3c9c33ba49d2c2dd3648eccedb7017efeb2e239d8d95c8`; generated NH config SHA-256 `ef134d8b2dd9cd4b37e1caecabcd150554d45b78776ab410fafb94c4ba336d72`; fixed-support contract SHA-256 `cb4ebe86afa501ef3d5929ead5b455f8df06e7d38b58ebf4148f8545fe6851ef`; screening basin-ids SHA-256 `d4395d93ebc567cf09e149c0121463d75cf4f7ecc02c07a7c4a7999763baa372` (policy `stage1_provisional_operational_screening_subset_v001`, 400 basins). `execution_provenance.json`: `provenance_stage=execution_status=VALID`, `artifact_identity_status=PASS`, `prepare_status=PASS`, `objective_eligible=true`, `retry_history=[]`, `retry_of_trial_id=null`. `review_records.json`: `trial_summary.workflow_status=pass`, `failure_category=null`.

**Proposal 3 — six proposed axes (ONE Bayesian observation, not selected/promoted).** `learning_rate=0.00049221944330504037`, `hidden_size=256`, `embedding_dropout=0.32902259001994533`, `output_dropout=0.37382255853365531`, `batch_size=256`, `seq_length=60` (raw controller axes ≡ normalized ≡ W&B run config; `seq_length=60` on the `q_uniform(48,120,12)` grid). These values are optimizer input/telemetry only.

**Proposal 3 — coverage and outcome.** Exactly 12 model checkpoints + 12 optimizer states (epochs 1–12); the direct-count evidence supplement confirms exactly **50,000 actual optimizer updates every epoch** (cumulative 50,000 → 600,000, read from per-parameter `state['step']` in each `optimizer_state_epochNNN.pt` via `pilot_orchestration.actual_optimizer_updates_by_epoch`); screening PASS for epochs 1–12 with **exactly the same frozen 400 screening basins each epoch** (per-epoch `validation_results.p` sorted-id SHA-256 `3d7203b134987de9ab9d2571b22808252a82e82edc2579169a575ada0b97d0ab` = frozen `validation_basins.txt`, identical across all 12 epochs, no duplicate/missing/extra); fixed-support Common-120 and natural-support raw-space evaluations complete and finite for epochs 1–12; `blocked=false`, `stopped=false`, `stop_reason=null`, `final_status=monolithic_training_and_screening_complete`. **Objective `0.37530856722966427` at epoch 9** (`best_epoch=9`); **epoch-12 fixed-support score `0.36099580201812653`**; **best-minus-final `0.014312765211537737`**; `best_score_10 = best_score_12 = 0.37530856722966427`; `late_gain_10_to_12=0.0`; `late_best=false`; runtime 10,264.16 s. Maximum absolute fixed-support / natural-support epoch-wise divergence ≈ `0.005318` at epoch 4. W&B publishes one finite objective-history row; `run.summary`, `run.history()`, `execution_provenance.objective_score`, and the review record agree within ordinary float representation. `publication_state="not_published"` / `valid_result_order=null` is the downstream human-review ledger state, not a defect or a W&B publication failure. No retry; no sealed access; Proposal 4 not requested or launched.

**Controller now holds exactly three valid completed Bayesian observations** (observations, not selected or promoted configurations; no causal hyperparameter effect is inferred from three adaptive observations): Proposal 1 `0.36621751839318467` at epoch 1; Proposal 2 `0.4388098707961096` at epoch 9; Proposal 3 `0.37530856722966427` at epoch 9.

**Random-control Wave 1 — frozen rows 0–3, all four independently VALID.** Frozen manifest `config/stage1_phase_b_sweep_v2_six_axis_random_control_v001_random_control_manifest.json` (byte SHA-256 `59be6726b60863aeed1c25e86782bd7a5e1623434ce23c956e66eb54c527c095`), frozen **after Bayesian observation 1** — scientifically independent of Proposal 1 but **not** a fully pre-outcome-frozen prospective control. Wave 1 = frozen rows 0–3. All four trials ran from `b4fe5eda…` with `search_arm=random_control`, `execution_generation=1`, Seed A `967139`, fidelity `mf12x50000`, `wandb_sweep_id=None` (zero W&B contact), `sealed_scope=false`; each completed 12 epochs, performed exactly 50,000 actual optimizer updates per epoch, evaluated exactly the frozen 400 screening basins per epoch, completed 12/12 screening evaluations, had no retry, and is **independently adjudicated VALID** (no row's classification depended on another row's outcome). Deployed output root `…/data/stage1_phase_b_sweep_v2_six_axis_random_control_v001/wave1_20260901T154518Z`.

| Row | proposal_order | Slurm job | elapsed | config suffix | lr | hidden | emb_do | out_do | batch | seq | best (epoch) | epoch-12 | best−final |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 0 | 1 | 46018446 | 02:25:28 | a476ab2b0f9fb7b42863 | 0.00038637832918302144 | 256 | 0.16324049036760591 | 0.32315075660579679 | 256 | 60 | 0.36989789510584137 (10) | 0.36001047867320574 | 0.009887416432635632 |
| 1 | 2 | 46018447 | 02:22:54 | 5f9f8c5442fb1200d68e | 0.00013016182151779704 | 64 | 0.19310104981894816 | 0.31371031433387786 | 512 | 108 | 0.43276757946464195 (6) | 0.4003737377555962 | 0.032393841709045745 |
| 2 | 3 | 46018448 | 02:16:35 | 54476936ed984e3a4d64 | 0.00027138903592439174 | 64 | 0.27494538782218808 | 0.19665000313541278 | 512 | 60 | 0.3717623575075273 (5) | 0.35945905704764963 | 0.012303300459877653 |
| 3 | 4 | 46018449 | 02:14:49 | 54d75cc3479d174bfa7e | 0.00025723895769944868 | 128 | 0.21073170668108956 | 0.29293576343030359 | 128 | 108 | 0.42770033214795594 (5) | 0.4217216973619983 | 0.005978634785957615 |

All four Slurm states `COMPLETED 0:0`; all `late_gain_10_to_12=0.0`, `late_best=false`. Maximum fixed/natural-support divergence per row ≈ 0.00633 (row 0, epoch 4), 0.00286 (row 1, epoch 11), 0.01348 (row 2, epoch 3), 0.00307 (row 3, epoch 5) — small in absolute NSE terms, with no defect pattern. Descriptive best-score ordering within Wave 1: row 1 > row 3 > row 2 > row 0 (descriptive only — not a selection).

**Shared scientific status (bounded observations, not decisions).** There are now **seven valid, materially varied v2 configurations**: three sequential Bayesian observations + four frozen IID random-control observations. Their seven best Common-120 objectives span ≈ **0.3662 to 0.4388**. Provisional descriptive best-score order across all seven, restricted to the 400-basin screening subset: Bayesian Proposal 2 `0.4388098707961096` > random row 1 `0.43276757946464195` > random row 3 `0.42770033214795594` > Bayesian Proposal 3 `0.37530856722966427` > random row 2 `0.3717623575075273` > random row 0 `0.36989789510584137` > Bayesian Proposal 1 `0.36621751839318467`. The four random draws vary materially across learning rate, hidden size, batch size, dropout, and sequence length. These results do **not** justify a winner declaration, model promotion, a causal hyperparameter-effect claim, or any change to the search space, objective, fidelity, or sampling design.

**Next gate.** The approved roadmap's trigger for comparing screening-400 conclusions with the full development-validation population was approximately **3–5 materially different valid configurations**. The campaign reached seven valid configurations because Proposal 3 and Wave 1 executed concurrently — a **bounded operational overshoot, not a policy failure**. The **screening-400 vs full-population development-validation audit is now due and should occur before authorizing further Proposal-4 or Wave-2 execution.** That audit's implementation, checkpoint-selection rule, compute plan, and acceptance threshold are deliberately **not** predefined here; they require a new inspected planning milestone. **Proposal 4 and random Wave 2 remain unlaunched and separately gated.**

**Evidence (untracked per `docs/repo_policy.md`; local + Moriah).** Proposal 3: `.scratch_local/v2_proposal3_completion_b4fe5ed_46017631_20260901T201745Z/` (22-file bundle, `SHA256SUMS.txt` verified 22/22 OK) with immutable sibling `supplement_direct_counts_20260901T204205Z/` (`SUPPLEMENT_SHA256SUMS.txt` verified OK; the original `SHA256SUMS.txt` was not modified). Wave 1: `.scratch_local/v2_random_control_wave1_completion_b4fe5ed_20260901T203307Z/` (`CHECKSUMS.txt` verified 12/12 byte-identical against the Moriah copies; per-row `execution_provenance.json` / `review_records.json` / `generation_manifest.json` + `cross_row_summary.md`). Full per-epoch trajectories live in those bundles and are not duplicated here. See `docs/decision_log.md`'s 2026-09-02 entry (top).

## V2 production Proposal 2 executed and adjudicated VALID; countable as v2 Bayesian observation 2/N; a separate execution step from Proposal 1; not a winner, promotion, or Stage-1 optimum; Proposal 3 remains unlaunched and separately gated (2026-09-01)

**[CLOSED — TWO VALID v2 PRODUCTION TRIALS; NO PROMOTION; NO HYPERPARAMETER-EFFECT INFERENCE]** The second real proposal from the accepted v2 production controller `wta85z3b` (`omri-porat1-huji/flashnh-stage1/wta85z3b`) ran to completion at exact commit `0090e94606383209ec9cb58e5dec6295e9bc5179` and is adjudicated **VALID** by a read-only scientific + operational completion review. It counts as **v2 Bayesian observation 2**. No configuration is selected, promoted, or called a winner; the frozen search domain, fidelity, objective, and scientific policy are unchanged.

**Identities.** `proposal_order=2`, `execution_generation=1`, `search_arm=bayesian`; campaign `stage1_phase_b_sweep_v2_six_axis_common120_v001`; domain `six_axis_q12_48_120_v001`; objective `common120_raw_space_nse_v001` (`flashnh/common120_raw_space_nse_v001`, maximize). `configuration_id=sweep_v2_cfg_dd9f7b847381f637439d`; `proposal_id=…__bayesian__proposal002`; `trial_id=…__proposal002__sweep_v2_cfg_dd9f7b847381f637439d__mf12x50000__seedA967139__attempt001`. Slurm job `46013192` (`catfish`/`catfish-04`, `COMPLETED 0:0`, `02:37:07`, `gres/gpu=1` one NVIDIA L4, 8 CPU / 128 GB, MaxRSS.batch `83,463,732K` ≈ 79.6 GiB, TotalCPU `02:36:43`); `squeue` shows the job absent; only two v2 production agent jobs have ever existed (`46009495` Proposal 1, `46013192` Proposal 2) — no retry, no duplicate, no replacement. W&B run `y4przhjs` (`swept-sweep-2`), terminal state `finished`; controller run count exactly **2** (`txd8mer9` finished, `y4przhjs` finished). Launch-manifest canonical SHA-256 `f48d5ef57d9c0eb3188a77354f9c79c512b8068ba3ba306c6fe200751b064397`; generated NH config SHA-256 `775e1ee67eb4bd352716bb361cc99414cf7d8f408c45c0acd5e41f968c7ffd2d`; fixed-support contract `common120_raw_space_nse_v001` SHA-256 `cb4ebe86afa501ef3d5929ead5b455f8df06e7d38b58ebf4148f8545fe6851ef`; screening basin-ids SHA-256 `d4395d93ebc567cf09e149c0121463d75cf4f7ecc02c07a7c4a7999763baa372` (policy `stage1_provisional_operational_screening_subset_v001`, 400 basins); development-split SHA-256 `397ab432564c18c3abc5158a47ada2b28840bbf6f0c213d2475444fded33858f` (identity pin only). Seed A `model_seed=967139`; fidelity `mf12x50000` (frozen 12 epochs × 50,000 updates); `target_epoch=12`; `evaluation_scope=development_validation_2024_only`; `sealed_scope=false`.

**Six proposed axes — ONE BAYESIAN OBSERVATION, NOT selected or promoted hyperparameters.** `learning_rate=0.0007223610500908302`, `hidden_size=128`, `embedding_dropout=0.27237327958287133`, `output_dropout=0.3494365141157241`, `batch_size=128`, `seq_length=120`. Raw controller axes ≡ normalized controller axes ≡ W&B run config; `seq_length=120` lies on the `q_uniform(min=48, max=120, q=12)` grid (upper endpoint). These values are optimizer input/telemetry only.

**Execution coverage (complete, frozen fidelity).** Exactly 12 model checkpoints `model_epoch001..012.pt` and 12 `optimizer_state_epoch001..012.pt`; actual optimizer updates exactly **50,000 per epoch** for all 12 epochs (cumulative 600,000; tfevents `train/loss` 12,000 points, last global step 600,000), every epoch cap-compliant; screening PASS for epochs 1–12 with **400/400** basins each (12 identical-size `validation_results.p`; `validation_basins.txt` = 400 lines; `require_full_screening_population` enforced by `fixed_support_contract_v2`); fixed-support Common-120 and natural-support raw-space evaluations complete and finite for epochs 1–12. `blocked=false`, `stopped=false`, `stop_reason=null`, `final_status=monolithic_training_and_screening_complete`, performance early stopping disabled; all 12 `epoch_trajectory` rows `evaluation_status=PASS`. No truncation.

**Authoritative outcome.** `execution_provenance.json`: `provenance_stage=VALID`, `execution_status=VALID`, `artifact_identity_status=PASS`, `prepare_status=PASS`, `objective_eligible=true`. `review_records.json`: `trial_summary.workflow_status=pass`, `failure_category=null`. **Objective `0.4388098707961096` at epoch 9** (`best_epoch=9`). **Epoch-12 fixed-support score `0.3983907128864448`.** **best-minus-final `0.04041915790966483`**; `best_score_10 = best_score_12 = 0.4388098707961096`; `late_gain_10_to_12=0.0`; `late_best=false`. Runtime 9,234.52 s (~2.57 h).

**Fixed-support / natural-support trajectory equality.** The 12-epoch fixed-support and natural-support median-NSE trajectories are **identical at every epoch** (maximum epoch-wise divergence **0.0**). This is expected, not a defect: `seq_length=120` exactly equals the frozen Common-120 120-hour common-support lookback floor, so each screening basin's naturally-available prediction window coincides with its frozen per-basin support window and the per-basin NSE — hence the medians — match. (Contrast Proposal 1 at `seq_length=60` below the floor, where natural support spans extra earlier timesteps and the two trajectories differ by ≈ 0.0047 at most.)

**W&B objective-publication equality.** The published metric `flashnh/common120_raw_space_nse_v001 = 0.4388098707961096` is **exactly equal** across `run.summary`, `run.history()`, `execution_provenance.objective_score`, and the review record `trial_summary.objective_score` — no float-repr difference (unlike Proposal 1). `flashnh/valid=true`; `flashnh/trial_id` matches the attempt trial_id; `scan_history` is 1 row (objective + W&B internals only; no placeholder/rehearsal metric).

**Descriptive Proposal-1 comparison (two observations only — no winner, no hyperparameter-effect inference).** Proposal 1 best objective `0.36621751839318467` at epoch 1; Proposal 2 best objective `0.4388098707961096` at epoch 9; difference **≈ +0.0725923524** in favour of Proposal 2 (epoch-12 difference ≈ +0.0512528762). Proposal 2 is materially higher on the optimizer objective and its trajectory improves over training (peak at epoch 9) whereas Proposal 1 never exceeds its epoch-1 value. With only two observations, no causal hyperparameter effect is inferred, no winner is declared, and the search domain / fidelity / objective are not touched.

**Boundary.** Development validation 2024 only; no temporal-test / spatial-holdout / California / sealed-set access (`sealed_scope=false`; only the 12 validation-period `validation_results.p` exist); `retry_history=[]`, `retry_of_trial_id=null`; exactly one `wandb agent` job and one launched command; no second run; no Proposal 3; no promotion/winner decision; no change to domain, fidelity, objective, or scientific policy. Counters: manifests inspected 1 · agents submitted by this review 0 · W&B runs for Proposal 2 = 1 · W&B proposals total 2 · retries 0 · Proposal 3 = 0 · sealed-scope accesses 0 · W&B mutations 0.

**Completion evidence.** `.scratch_local/v2_proposal2_completion_0090e94_46013192_20260901T140200Z/` (37 files incl. consolidated `SHA256SUMS.txt`; `adjudication_summary.{json,md}`, `proposal1_vs_proposal2.md`, `trajectory_table.txt`, `BOUNDARY_AND_COUNTERS.txt`, `attempt_execution_provenance.json`, `attempt_review_records.json`, `attempt_generation_manifest.json`, `optimizer_update_counts.json`, checkpoint/optimizer/eval inventories, `wandb_readonly.json`, `slurm_sacct.txt`, NH `output.log`/`config.yml`, agent log head/tail/keylines). Local checksum verification **36/36 OK, 0 failures**. Read-only Moriah collection scratch remains under `$HOME/tmp/` (home, not repo). Untracked per `docs/repo_policy.md`.

**Limitation (non-blocking, identical to Proposal 1).** `gpu_hours`, `operations.slurm_job_id`, and `operations.slurm_state` are `null` in `review_records.json` (post-run Slurm-accounting enrichment not attached). Job identity is independently fixed by `sacct` and the agent `.err`/`.out` logs (~2.62 L4-GPU-h from Slurm elapsed).

**Related status.** The v2 six-axis IID random-control readiness milestone remains committed and pushed at `45b7a830a0c2d6122d8879d231d57a716297ffc9` ("Add v2 six-axis random-control readiness"); **random Wave 1 remains unlaunched and separately gated**. Proposal 3 (the next sequential Bayesian proposal) **remains unlaunched and separately gated** — under the approved roadmap it requires separate explicit user authorization; the controller is healthy with exactly 2 completed valid observations. See `docs/decision_log.md`'s 2026-09-01 Proposal-2 entry (top).

## v2 six-axis IID random-control manifest FROZEN (after Bayesian observation 1) + offline readiness implemented; campaign roadmap approved as policy; no execution authorized (2026-09-01)

**[AUDIT → AUTHORIZED FREEZE → OFFLINE READINESS; NOT COMMITTED, NOT EXECUTED]** A read-only audit at commit `0090e94606383209ec9cb58e5dec6295e9bc5179` (`master` = `origin/master`) first established that the repository held **only** the historical Sweep-v1 five-axis random-control contract and no compatible v2 six-axis manifest. The user then authorized exactly one v2 six-axis IID random-control manifest and the lean offline execution-readiness path for it. All new/edited files are uncommitted; no W&B contact, no Slurm submission, no training.

**Frozen manifest.** `config/stage1_phase_b_sweep_v2_six_axis_random_control_v001_random_control_manifest.json` — 12 rows, `proposal_order` 1..12, byte SHA-256 **`59be6726b60863aeed1c25e86782bd7a5e1623434ce23c956e66eb54c527c095`** (sidecar `.sha256`; pinned as `RANDOM_CONTROL_MANIFEST_SHA256_V2`). Drawn once from `random.Random(158987734)` over the exact `SEARCH_DOMAIN_V2` priors, per-row draw order `_AXES_V2` = (`learning_rate`, `hidden_size`, `embedding_dropout`, `output_dropout`, `batch_size`, `seq_length`); `seq_length` uses the committed `q_uniform(48,120,12)` wire semantics `int(round(U(48,120)/12)*12)` — not a categorical prior. One-shot realization: no filtering/stratification/balancing/Sobol/LHS/dedup/coverage optimization. Realized rows are all-distinct (12 distinct `configuration_id`s); `seq_length` distribution `{48:1, 60:4, 84:1, 96:3, 108:2, 120:1}`. Identity: `campaign_id="stage1_phase_b_sweep_v2_six_axis_common120_v001"`, `domain_version="six_axis_q12_48_120_v001"`, `objective_id="common120_raw_space_nse_v001"`, `fidelity_id="mf12x50000"` (Seed A `967139`, 12 epochs, `max_updates_per_epoch=50000`, `save_weights_every=1`), `support_contract_sha256="cb4ebe86afa501ef3d5929ead5b455f8df06e7d38b58ebf4148f8545fe6851ef"`.

**Seed derivation (recorded).** Namespace `stage1_phase_b_sweep_v2_six_axis_random_control_v001` → SHA-256 `0979f5d60aa60db35d6f0b5c248bfdf73ac24b734b1b5fcb9753db8517299ea2` → first 8 hex `0979f5d6` → integer **`158987734`**. Verified ≠ Seed A (`967139`) and ≠ v1 manifest seed (`20260822`); collision would be a hard stop with no fallback.

**Label — frozen after Bayesian observation 1.** Scientifically independent of Proposal 1 (its config/trajectory/objective play no part in generation) but **not** a fully pre-outcome-frozen prospective control, because Bayesian observation 1 was already recorded when it was authorized. Documented honestly as such in the manifest payload (`frozen_after_bayesian_observation: 1`, `prospective_pre_outcome_frozen: false`).

**Offline implementation (uncommitted).** New: `src/baseline/sweep_v2_six_axis_random_control.py` (static contract/generator/validator — no launcher/NH/W&B; never contacts `wta85z3b`); `scripts/prepare_phase_b_sweep_v2_six_axis_random_control_manifest.py` (offline write/verify/audit); `scripts/run_sweep_v2_six_axis_random_control_trial.py` + `scripts/run_sweep_v2_six_axis_random_control_moriah.sbatch` (one Slurm allocation = one committed row; W&B-free; runtime provenance pinned via the shared `sweep_v1_runtime_contract.verify_commit_and_interpreter` — canonical Moriah workdir/interpreter, explicit full-40-hex expected commit, exact HEAD match, tracked-tree-clean refusal that preserves untracked artifacts — before any execution; explicit retry lineage via `sweep_v2_six_axis_retry.derive_exact_retry_identity_v2`; identical prepare→write→execute→interpret spine as the Bayesian bridge); `tests/test_sweep_v2_six_axis_random_control.py` (13 generator/identity tests) + `tests/test_sweep_v2_six_axis_random_control_consumer.py` (25 focused tests: vertical frozen-row → prepare → persist → synthetic execution seam → interpreted result, adapter refusals, Bayesian-front-door isolation, retry-lineage resolution, and static runner/launcher contract). Additive edits: `SEARCH_ARMS_V2` → `{"bayesian","random_control"}` (Bayesian front door still pinned via `expected_arm="bayesian"`; W&B sweep config untouched); `prepare_random_control_proposal_v2` in `sweep_v2_six_axis_production_adapter.py`; `run_prepared_trial_in_production_v2` in `sweep_v2_six_axis_execution.py`; stale "v2 has no random_control arm" comments corrected in `sweep_v2_six_axis_review_analysis.py` + test. Full offline `prepare` still needs the external ~130 MB fixed-support contract + package root + splits, so it completes only at Moriah launch time. Intended execution shape: **three waves of four concurrent independent jobs** (row indices 0–3, 4–7, 8–11), each writing to `output_root/<trial_id>`. Infrastructure retries retain `configuration_id`/`proposal_id`, advancing only the attempt suffix (`execution_generation`).

**Reusable components adapted:** `sweep_v1_campaign` generator/manifest pattern; `sweep_v2_six_axis_campaign` identity/normalization; `sweep_v2_six_axis_execution` execution spine + provenance + Slurm accounting; `sweep_v2_six_axis_retry` recovery identity; `sweep_v1_execution._derive_validity` (via the v2 module); `fixed_support_contract_v2` + Common-120 contract; `sweep_v2_six_axis_objective_recovery`; non-W&B launch template `run_sweep_v1_random_control_trial.py` + `run_sweep_v1_random_control_moriah.sbatch`.

**Approved campaign operating roadmap (policy — does not authorize execution).** (1) random-control arm = 12 independent IID trials; (2) intended execution = three waves of four concurrent independent jobs (four-way random concurrency allowed); (3) Bayesian optimization stays sequential — each valid result reaches `wta85z3b` before the next proposal; (4) close human review through ~Proposal 4, no routine approval before every later ordinary Bayesian trial; (5) bounded sequential Bayesian execution after initial stability; (6) formal Bayesian scientific reviews near 12 and 24 valid observations, proposal issuance pauses near those boundaries with bounded overshoot only; (7) Common-120 (`flashnh/common120_raw_space_nse_v001`) is the sole optimizer objective, secondary metrics stay diagnostic; (8) compare screening-400 vs full-population development-validation conclusions after ~3–5 materially different valid configurations before freezing screening-400 for final selection.

**Status separation.** Approved operating policy = the 8 points above. Frozen scientific manifest = DONE (seed `158987734`, SHA-256 `59be6726…`). Offline implementation readiness = DONE (generator/validator/runner/sbatch; the production consumer and the Moriah launcher contract are focused-tested — 38 focused tests across the two random-control test files; Moriah-time `prepare` still needs external artifacts). Live execution authorization = NOT granted (random arm; Bayesian beyond Proposal 2 — which has since run to completion and been adjudicated VALID as Bayesian observation 2, not a winner or promotion; Proposal 3 remains not launched and separately gated). No commit/push. See `docs/decision_log.md` 2026-09-01 top entry.

## V2 production Proposal 1 executed and adjudicated VALID; countable as v2 Bayesian observation 1/N; proposed axes are one observation, not a selected or promoted configuration; Proposal 2 is a separate execution step (2026-09-01)

**[CLOSED — ONE VALID v2 PRODUCTION TRIAL; NOT A WINNER; NO PROMOTION]** The first real proposal from the accepted v2 production controller `wta85z3b` (`omri-porat1-huji/flashnh-stage1/wta85z3b`) ran to completion at exact commit `6eb301e8206f8c4b6d76a33be5ed888966ef5afd` and is adjudicated **VALID** by a read-only scientific + operational completion review. It counts as **v2 Bayesian observation 1**. No configuration is selected, promoted, or called a winner.

**Identities.** `proposal_order=1`, `execution_generation=1`, `search_arm=bayesian`; campaign `stage1_phase_b_sweep_v2_six_axis_common120_v001`; domain `six_axis_q12_48_120_v001`; canonicalization `sweep_v2_six_axis_canonical_json_v001`; objective `common120_raw_space_nse_v001` (`flashnh/common120_raw_space_nse_v001`, maximize). `configuration_id=sweep_v2_cfg_26daf2a8f293007dd60b`; `proposal_id=stage1_phase_b_sweep_v2_six_axis_common120_v001__bayesian__proposal001`; `trial_id=…__proposal001__sweep_v2_cfg_26daf2a8f293007dd60b__mf12x50000__seedA967139__attempt001`. Slurm job `46009495` (`catfish`/`catfish-03`, `COMPLETED 0:0`, `03:32:42`, `gres/gpu=1` one NVIDIA L4, 8 CPU / 128 GB, MaxRSS.batch `91,478,352K` ≈ 87.2 GiB, TotalCPU `05:09:17`). W&B run `txd8mer9` (`revived-sweep-1`), terminal state `finished`; controller run count exactly **1**. Launch-manifest canonical SHA-256 `09a324f670575f62618479b6a7270344064a9b3fe994780c283965b05838688a` (manifest file byte SHA-256 `43bf8e5f51ef11854dbb05a80386f6523d4ecff74fd6079382c362c1da1db4e2`); generated NH config SHA-256 `dc8711c17f473439284fab9bec0877dba42a865fafeaf765ab96597ac8e14607`; fixed-support contract `common120_raw_space_nse_v001` SHA-256 `cb4ebe86afa501ef3d5929ead5b455f8df06e7d38b58ebf4148f8545fe6851ef`; screening basin-ids SHA-256 `d4395d93ebc567cf09e149c0121463d75cf4f7ecc02c07a7c4a7999763baa372` (policy `stage1_provisional_operational_screening_subset_v001`, 400 basins). Seed A `model_seed=967139`; fidelity `mf12x50000` (frozen 12 epochs × 50,000 updates); `target_epoch=12`; `evaluation_scope=development_validation_2024_only`; `sealed_scope=false`.

**Six proposed axes — ONE BAYESIAN OBSERVATION, NOT selected or promoted hyperparameters.** `learning_rate=0.00026727483167567154`, `hidden_size=256`, `embedding_dropout=0.1203941658997464`, `output_dropout=0.3947011313328828` (≡ `0.39470113133288282`), `batch_size=512`, `seq_length=60`. Raw controller axes ≡ normalized controller axes ≡ W&B run config; `seq_length=60` lies on the `q_uniform(min=48, max=120, q=12)` grid. These values are optimizer input/telemetry only.

**Execution coverage (complete).** Exactly 12 model checkpoints `model_epoch001..012.pt` and 12 `optimizer_state_epoch001..012.pt`; `pilot_orchestration.actual_optimizer_updates_by_epoch` = exactly **50,000 per epoch** for all 12 epochs (cumulative 600,000), every epoch cap-compliant; screening PASS for epochs 1–12 with **400/400** basins each (12 identical-size `validation_results.p`); fixed-support Common-120 and natural-support raw-space evaluations complete and finite for epochs 1–12. `blocked=false`, `stopped=false`, `final_status=monolithic_training_and_screening_complete`, performance early stopping disabled.

**Authoritative outcome.** `execution_provenance.json`: `provenance_stage=VALID`, `execution_status=VALID`, `artifact_identity_status=PASS`, `prepare_status=PASS`, `objective_eligible=true`. `review_records.json`: `trial_summary.workflow_status=pass`, `failure_category=null`. **Objective `0.36621751839318467` at epoch 1** (`best_epoch=1`). **Epoch-12 fixed-support score `0.3471378366389816`.** **best-minus-final `0.01907968175420305`**; `best_score_10 = best_score_12 = 0.36621751839318467`; `late_gain_10_to_12=0.0`; `late_best=false`. Runtime 12,686.18 s (~3.52 h). The fixed-support and natural-support 12-epoch trajectories were both complete and **closely aligned** (identical epoch-1 peak; max epoch-wise divergence ≈ 0.0047 at epoch 6).

**W&B objective-publication equality.** The published metric-history value `flashnh/common120_raw_space_nse_v001 = 0.36621751839318467` **exactly equals** the Flash-NH authoritative objective. `flashnh/valid=true`; `flashnh/trial_id` matches the attempt trial_id; `scan_history` is 1 row. (`run.summary` shows the 16-digit float repr `0.3662175183931846` — display only; the optimizer consumed the exact value.)

**Boundary.** No sealed-scope access (`sealed_scope=false`; spatial-holdout / development split SHA-256 present as identity pins only); `retry_history=[]`, `retry_of_trial_id=null`; exactly one `wandb agent` job and one launched command; no second run; no Proposal 2 within this trial. Counters: manifests 1 · agents 1 · controller runs 1 · retries 0.

**Completion evidence.** `.scratch_local/v2_proposal1_completion_6eb301e_46009495_20260901T072828Z/` (33 files incl. `SHA256SUMS.txt`; adjudication summary JSON/Markdown, terminal provenance, review records, optimizer-update counts, checkpoint/optimizer/eval inventories, compact trajectory table + figure, W&B read-only snapshot, boundary/counter record). Local checksum verification **32/32 OK, 0 failures**; Moriah mirror re-verified **ALL OK**. Untracked per `docs/repo_policy.md`.

**Limitation (non-blocking).** `gpu_hours`, `operations.slurm_job_id`, and `operations.slurm_state` are `null` in `review_records.json` (post-run Slurm-accounting enrichment was not attached). Job identity is independently fixed by W&B `wandb-metadata.json` and `sacct` (~3.55 L4-GPU-h from Slurm elapsed).

**Scientific note — frozen fidelity is not reopened.** The objective peaking at epoch 1 and declining across the frozen 12 × 50,000-update budget (this configuration: `hidden_size=256`, `lr≈2.67e-4`, `output_dropout≈0.39`) is **a scientific observation about this single configuration only**. It is not grounds to change the frozen Seed-A 12-epoch × 50,000-update fidelity after one configuration; the fidelity remains as designed.

**Proposal 2.** Executed as a **separate** step under the same controller (`proposal_order=2`, `execution_generation=1`) — see the 2026-09-01 decision-log entry (top). Proposal 1 being VALID does not make its axes selected or promoted.

## V2 production W&B controller `wta85z3b` registered exactly once and read-only verified; accepted as the v2 production controller; Proposal 1 is a distinct execution step (2026-08-31)

**[CLOSED — ONE REAL PRODUCTION CONTROLLER REGISTRATION; NO PROPOSAL / RUN / AGENT / TRAINING]** Exactly one real `wandb.sweep(...)` call registered the reusable v2 six-axis *production* controller `wta85z3b` (`omri-porat1-huji/flashnh-stage1/wta85z3b`) at exact commit `ddd39646599b610ebb850f6b7207ec7ef6356ba3`. CPU Slurm job `46009198` (`glacier`/`glacier-25`, 2 CPU / 8 GB, `COMPLETED 0:0`, `00:01:24`) ran the single `register-controller` invocation, which exited 0 with `Create sweep with ID: wta85z3b` and a `status=success` receipt. No `register-controller` retry and no job resubmission.

**Read-only verification.** A live `wandb.Api()` read (no mutation) confirmed method `bayes`; optimizer metric exactly `flashnh/common120_raw_space_nse_v001` (goal `maximize`); program exactly `scripts/run_sweep_v2_six_axis_wandb_bridge.py`; static command exactly `["${interpreter}", "${program}"]` (no `${args}`, no embedded manifest); the authoritative committed v2 six-axis domain (`learning_rate` log-uniform-values `1e-4`–`1e-3`; `hidden_size` `{64,128,256}`; `embedding_dropout` / `output_dropout` uniform `0.0`–`0.4`; `batch_size` `{128,256,512}`; `seq_length` `q_uniform(min=48, max=120, q=12)`) with no extra parameters; controller state `PENDING`; run count exactly zero. All 16 identity/config checks plus 7 per-parameter checks passed (`phase4_readonly_verification.json`, wandb 0.28.1). The forbidden sweeps `4x3btz2s` (frozen v1) and `oz5p4csb` (disposable rehearsal) were not contacted or queried.

**Boundary at registration.** No production launch manifest, proposal, `proposal_order`/`execution_generation` reservation, `wandb agent`, W&B run, GPU allocation, training, inference, scientific evaluation, or objective-value publication existed or occurred; no Common-120 regeneration; no sealed-set access; no documentation edit, commit, or push during the registration job (`phase5_boundary_statement.json`: `wandb_sweep_calls_attempted = 1`, all other counters `0`/`null`).

**Evidence.** Compact untracked bundle `.scratch_local/v2_production_controller_registration_ddd3964_20260831T131151Z/moriah_evidence/` (29 files, `SHA256SUMS.txt`), independently re-verified locally 29/29 with 0 failures. Untracked per `docs/repo_policy.md`.

**Decision.** `wta85z3b` is accepted as the v2 production controller. Proposal 1 — one real Bayesian proposal, one strict `mode=production` manifest, one `wandb agent --count 1`, and one L4 GPU training run — remains a **distinct execution step** performed separately. See `docs/decision_log.md`'s 2026-08-31 entry (top).

## Offline v2 production controller-registration and one-agent launch-readiness tooling implemented, focused-tested, independently approved, and committed (`8b80a96`); NO production sweep, proposal, manifest, agent, or training run (2026-08-30)

**[IMPLEMENTED AND COMMITTED — NOT LIVE REGISTRATION OR EXECUTION AUTHORIZATION]** Local, offline tooling now exists to (later, under separate explicit authorization) register the reusable v2 six-axis *production* W&B controller and launch exactly one production agent. It was focused-tested and independently approved, and is committed and pushed as `8b80a960394bb70c8c3aa4454c2a4588587a3c32` ("Add v2 production controller launch readiness"). No production sweep, proposal, manifest, agent, or training run has been created or executed; no W&B/Slurm/Moriah/h2o contact occurred. Live production registration and agent/training execution remain separately gated and unauthorized.

- `scripts/build_sweep_v2_six_axis_production_sweep_config.py` — serializes `build_production_sweep_config_v2(...)` offline (no `import wandb`). The production controller command is static (`["${interpreter}", "${program}"]`) and embeds no proposal-specific manifest.
- `scripts/create_sweep_v2_six_axis_wandb_bridge_production_sweep.py` — one narrow injectable `wandb.sweep(...)` boundary (never auto-retries; preserves the returned controller id in a durable partial-failure receipt), a strict `mode=production` manifest builder for a given `proposal_order`/`execution_generation` (fresh proposal/attempt output path; refuses overwrite), a `wandb agent --count 1` invocation builder that now *consumes a loader-valid `mode=production` manifest and derives the agent's sweep id from it* (a redundant supplied id must match exactly or the build fails; rehearsal-mode, malformed, and forbidden-id manifests are rejected before any invocation is produced), and a maintained `validate-launch` CLI subcommand / `resolve_validated_production_launch(...)` helper — the authoritative pre-agent manifest/controller join used by the launcher: it loads the manifest through the strict authoritative loader (checksum + schema + full v2 identity), requires `mode=production` / `stop_before_training=false` / `max_agents=1`, rejects both forbidden production sweep ids, cross-checks an optional redundant `WANDB_SWEEP_ID` for exact equality, prints only the one authoritative sweep id, and imports/contacts no W&B (proven by a poisoned-`wandb` subprocess test).
- `src/baseline/sweep_v2_six_axis_wandb_bridge_registration.py` — shared offline primitives (descriptor authority, validators, error/receipt classes) reused by both the closed rehearsal registration script and the new production script; the generic `_assert_safe_sweep_id` stays permissive so historical rehearsal manifests remain valid, and a new production-only `_assert_production_safe_sweep_id` refuses any id in the authoritative `FORBIDDEN_PRODUCTION_SWEEP_IDS` set before delegating.
- `src/baseline/sweep_v2_six_axis_campaign.py` — single authoritative forbidden-set location: `FORBIDDEN_PRODUCTION_SWEEP_IDS = {FORBIDDEN_V1_SWEEP_ID "4x3btz2s", CLOSED_DISPOSABLE_REHEARSAL_SWEEP_ID "oz5p4csb"}`.
- `src/baseline/sweep_v2_six_axis_wandb_bridge_manifest.py` — the strict loader keeps the all-modes v1 refusal and adds a mode-sensitive rule: a `mode=production` manifest must not target any forbidden production sweep id; historical `mode=rehearsal` manifests naming `oz5p4csb` stay loader-valid.
- `scripts/run_sweep_v2_six_axis_wandb_bridge.py` — bridge resolves its manifest source (preserved rehearsal positional arg **or** `FLASHNH_SWEEP_V2_PRODUCTION_MANIFEST`; agreement required if both; clear refusal on absence/contradiction) *before* importing/initializing W&B or writing proposal intake, and adds a mode-aware defense-in-depth guard that refuses a `mode=production` manifest targeting a forbidden production sweep id before `wandb.init()`.
- `scripts/run_sweep_v2_six_axis_wandb_agent_moriah.sbatch` — one-agent production launcher: the strict manifest path is the sole launch identity; before any W&B contact it refuses a non-empty inherited `FLASHNH_SWEEP_V2_BRIDGE_SELFTEST` (then unsets it), mirrors both forbidden sweep-id literals for shell-level pre-contact refusal (a test guards them against drift), runs the `validate-launch` pre-agent join with the canonical interpreter, and passes the validated manifest-derived sweep id to the sole `wandb agent --count 1` line; holds no credentials and dumps no environment. The rehearsal launcher is unchanged.

Authority flow: strict `mode=production` manifest → `validate-launch` pre-agent join (all checks before any W&B) → validated sweep id → `wandb agent --count 1 <validated id>` → bridge (re-loads and re-rejects via the strict loader; mode-aware guard before `wandb.init()`).

Registration of the production controller, creation of a real production proposal, and any production agent/training remain **separately gated** and unauthorized. The frozen v1 sweep `4x3btz2s` and the disposable rehearsal sweep `oz5p4csb` are forbidden for every production path; historical `mode=rehearsal` manifests for `oz5p4csb` remain valid and its closed rehearsal evidence is unchanged.

## V2 disposable CPU W&B one-agent rehearsal CLOSED / SUCCEEDED; disposable proposal 900001 and run `8aj5ldxq` are operational-only; production planning and disposable-sweep retirement remain separately gated (2026-08-30)

**[CLOSED — OPERATIONAL REHEARSAL, NO SCIENTIFIC RESULT]** This entry supersedes only the "REAL CPU AGENT REHEARSAL — NOT PERFORMED / SEPARATELY GATED" line of the V2 registration entry immediately below; that entry's registration and preparation history is unchanged and remains the historical record.

**What this means in practice.** The disposable v2 six-axis controller/manifest/bridge/stop path now has an end-to-end operational proof: a single `wandb agent --count 1` against disposable sweep `oz5p4csb` consumed exactly one disposable proposal, created exactly one W&B run (`8aj5ldxq`, `usual-sweep-1`, terminal state `finished`), crossed the manifest-driven proposal-intake and preparation path, and stopped before training. Exact-commit pinning (`1dc27478ff9b9e49edaecfd8da689b8d1ca324bd`, clean detached HEAD on Moriah), the pre-agent read-only identity/configuration gate, `--count 1`, absolute no-retry after the first agent contact, and `stop_before_training=true` all behaved as designed.

**Operational conclusion.** Pre-agent gate: Slurm job `45986888` (`glacier`/`glacier-04`, 2 CPU / 8 GB / no GPU, `COMPLETED 0:0`, `00:00:44`) recorded a fresh read-only sweep state of `PENDING` with run count 0, the exact controller identity/configuration, and the qualified helper SHA-256 `0140bf60156652a2d67b6f7a2e9e350a4d9acdf662063abdf7e454b011a2b9f0`. Mutation-boundary rehearsal: Slurm job `45986910` (`glacier`/`glacier-37`, same resources, `COMPLETED 0:0`, `00:01:19`) ran the one agent invocation. Bridge outcome: `prepare_only=true`, `prepare_status=PASS`, final pre-stop provenance stage `executor_mode_selected`, selected executor mode `monolithic`, `objective_score=null`; the only Flash-NH/project-specific run-summary fields are `flashnh/rehearsal_stopped_before_training=true` and `flashnh/trial_id` (the exact local trial identity), with zero W&B history rows and no metric value published. Authoritative post-run read-only verification: Slurm job `45986928` (`glacier`/`glacier-33`, `COMPLETED 0:0`, `00:00:36`) returned `POSTRUN_VERIFIED` with 24/24 checks passing, sweep run count exactly 1, run `8aj5ldxq` `finished`, W&B trial ID equal to local intake, and no metric values anywhere. The strict rehearsal manifest byte SHA-256 stayed `78abc0483b314843d85d772d3be0338270fc0f55c1c9b7d5643a5cfa411f735c` throughout.

**Exact non-scientific boundary.** The proposed axes that crossed the controller/bridge interface (`batch_size=256`, `embedding_dropout=0.24247769820599452`, `hidden_size=128`, `learning_rate=0.00028381477465092816`, `output_dropout=0.13121329813483537`, `seq_length=84`) are recorded **only** as operational evidence that a well-formed six-axis proposal was received and prepared. They are not a model result, a winning configuration, a candidate promotion, an optimization observation, or a recommendation. No second agent, proposal, or run; no training; no model inference; no scientific evaluation; no sealed-scope access (`sealed_scope=false`, `evaluation_scope=development_validation_2024_only`); no production-objective value and no placeholder-metric value published; no frozen-v1 or production-sweep contact; no new sweep registration; no manifest mutation; Common-120 was loaded and validated through the intended read-only code path only, with no regeneration, copying, rewriting, requalification, training, or scientific evaluation (filesystem non-mutation is inferred from the inspected read-only code path, not independently syscall-audited during this rehearsal); no commit or push during execution.

**Evidence-hygiene incidents (non-blocking).** The first post-run verifier (job `45986919`) established the major result but skipped the local/W&B trial-ID equality check because its local-discovery predicate still expected the earlier `proposal_intake` stage after the single `execution_provenance.json` had already advanced in place to `executor_mode_selected`. Only the read-only verifier was repaired (superseded launcher SHA `4692989e…` → authoritative `c65f498c95211de2e86c41bf4c1c9899a3bace2683b3ed755a25c274884f38e1`); the `wandb agent` was **not** retried. Separately, the original Moriah post-run evidence used a deeply nested trial path that was not portable to the local Windows transfer; the six authoritative output-root payloads were preserved byte-identically under `evidence_postrun_45986928/output_root_snapshot_flat/`, all six hashes matching both the original long-path entries and `output_root_SHA256SUMS.txt`, and a complete authoritative `PORTABLE_SHA256SUMS.txt` was added. Original historical checksum manifests and all evidence payload bytes were unchanged by either repair.

**Compact local evidence.** Portable, checksum-verified bundle `.scratch_local/v2_real_disposable_rehearsal_1dc2747_20260830T121257Z/` with authoritative manifest `PORTABLE_SHA256SUMS.txt` (88 entries) and portability note `EVIDENCE_PORTABILITY.md`. Portable archive `.scratch_local/v2_real_disposable_rehearsal_1dc2747_20260830T121257Z_portable.tar` (SHA-256 `df2deffadc073d1f96df36a0becf133e1e7b5e0e4c470be8ec3bc8caab7d6460`, 378,880 bytes); fresh-extraction validation returned 88/88 OK, 0 missing, 0 mismatch, no unexpected regular files, one safe top-level directory, no absolute paths or traversal, no credential material. Untracked per `docs/repo_policy.md`.

**Next phase, separately gated.** Production planning/readiness for a **separately designed v2 production controller and execution path** is the next gate and requires separate explicit user authorization; passing this rehearsal proves only the operational controller/manifest/bridge/stop path, not production readiness. No v2 production sweep has been created or registered. Retirement of the disposable rehearsal sweep `oz5p4csb` is likewise a separate user gate and is **not** authorized or performed here. The frozen v1 production sweep `4x3btz2s` is not a target and must not be contacted; `oz5p4csb` is disposable and its single authorized operational proposal is already consumed — it is not reusable for production.

**Status, precisely.** PREPARATION A — CLOSED. PREPARATION B — CLOSED. LINUX/OFFLINE QUALIFICATION — CLOSED. REAL DISPOSABLE REGISTRATION — CLOSED. PRE-AGENT EXECUTION PREFLIGHT — QUALIFIED. REAL ONE-AGENT CPU REHEARSAL — CLOSED / SUCCEEDED. DISPOSABLE PROPOSAL 900001 — CONSUMED FOR OPERATIONAL REHEARSAL ONLY. DISPOSABLE W&B RUN 8aj5ldxq — FINISHED / STOPPED BEFORE TRAINING. TRAINING — NOT PERFORMED. SCIENTIFIC EVALUATION — NOT PERFORMED. OPTIMIZER OBJECTIVE — NOT PUBLISHED. PRODUCTION SWEEP — NOT CREATED / NOT AUTHORIZED. PRODUCTION AGENT OR TRAINING — NOT AUTHORIZED. DISPOSABLE SWEEP RETIREMENT — NOT PERFORMED / SEPARATELY GATED. See `docs/decision_log.md`'s 2026-08-30 rehearsal-closure entry (top) for the durable decision.

## V2 disposable rehearsal sweep registration CLOSED; disposable sweep exists and is inert; CPU agent rehearsal separately gated (2026-08-30)

**[CLOSED — ONE REAL DISPOSABLE REGISTRATION]** This entry supersedes the pre-registration status snapshot immediately below (the 2026-08-29 entry's "REAL DISPOSABLE REGISTRATION — NOT PERFORMED / SEPARATELY GATED" line): the one real, disposable v2 registration that entry gated has now been performed exactly once and independently reviewed for documentation.

**What was created.** Exactly one real `wandb.sweep(...)` call created the disposable, non-production v2 six-axis controller sweep `oz5p4csb` (`omri-porat1-huji/flashnh-stage1/oz5p4csb`). Its swept domain is the frozen v2 six-axis domain: `learning_rate` log-uniform-values `1e-4`–`1e-3`; `hidden_size` `{64,128,256}`; `embedding_dropout` uniform `0.0`–`0.4`; `output_dropout` uniform `0.0`–`0.4`; `batch_size` `{128,256,512}`; `seq_length` `q_uniform(min=48, max=120, q=12)`. The optimizer metric is the rehearsal placeholder `qualification/rehearsal_placeholder_metric_v2` (goal `maximize`) — deliberately **not** the production objective `flashnh/common120_raw_space_nse_v001`. Method `bayes`; program `scripts/run_sweep_v2_six_axis_wandb_bridge.py`; command `["${interpreter}", "${program}", "<canonical rehearsal manifest path>"]`.

**Strict manifest identity.** The registration published the strict rehearsal manifest at `.scratch_local/sweep_v2_six_axis_wandb_bridge_launch_manifests/rehearsal_v001.json` via the atomic no-clobber writer. The semantic manifest checksum (`manifest_sha256`, the canonical-JSON identity) is `329125a56e29c22e1ccfe12b58d88400ba1be8cb1d694bffd8af44e3ffae2797`; the manifest loader re-validated it (`checksum_match: true`, `validation: "PASS"`). Manifest fields: `mode=rehearsal`, `proposal_order=900001`, `execution_generation=900001`, `max_agents=1`, `stop_before_training=true`, `expected_commit=1dc27478ff9b9e49edaecfd8da689b8d1ca324bd`, screening binding `screening_basin_ids_sha256=d4395d93ebc567cf09e149c0121463d75cf4f7ecc02c07a7c4a7999763baa372`, Common-120 binding `fixed_support_contract_sha256=cb4ebe86afa501ef3d5929ead5b455f8df06e7d38b58ebf4148f8545fe6851ef` (the descriptor's *internal canonical* contract checksum, not the external serialized-file SHA-256 — the two remain distinct identities per the Common-120 entry below). `proposal_order=execution_generation=900001` is a disposable, non-scientific rehearsal namespace: it is not Proposal 1, not a production execution generation, and not countable toward any HPO/trial budget.

**Read-only verification (final preserved record).** A separate read-only verification (W&B `Api` query only, no mutation) recorded sweep `oz5p4csb` in state `PENDING` with run count zero; method `bayes`; the exact placeholder metric; the exact program and command (the command embeds the canonical rehearsal manifest path); all six swept parameters exactly matching the manifest with no extra parameters; and the frozen v1 production sweep `4x3btz2s` not targeted (`all_checks_pass: true`, timestamp `2026-08-30T07:57:19Z`). W&B normalises the `min: 0.0` dropout bounds to `0`; parameter equality still holds.

**Non-blocking verification-harness incident.** Phase 2 (registration) exited 0, W&B returned sweep id `oz5p4csb`, and the success receipt plus the strict manifest were already durable and loader-valid. The *initial* in-job read-only verifier then failed: it parsed the whole receipt file as JSON and could not tolerate the two W&B stdout banner lines (`Create sweep with ID: …` / `Sweep URL: …`) preceding the JSON object (`json.decoder.JSONDecodeError: Expecting value: line 1 column 1`). The orchestrator recorded `phase3_rc=1 phase2_rc=0` and, correctly, **never retried registration**. A separate read-only job with corrected receipt parsing then completed verification successfully against the same sweep. Neither verifier mutated any W&B state. This was a non-blocking read-only verification-harness defect only; it does not affect registration success or sweep identity. The pre-repair verifier *body* was not byte-preserved, but its SHA-256, the quoted failing source line inside the preserved traceback, the triggering receipt, and the exit-code records all survive in the evidence.

**Scheduler and local evidence.** Job `45983252` (`flashnh-v2-rehearsal-real-register`, partition `glacier`, node `glacier-34`, `COMPLETED`, exit `0:0`, elapsed `00:00:46`, 2 CPU / 8 GB) performed the Phase 1 gate (PASS), Phase 2 registration (exit 0), Phase 2b loader validation (PASS), and the failed initial verify. Job `45983254` (`flashnh-v2-rehearsal-phase3-readonly`, `glacier`/`glacier-34`, `COMPLETED`, exit `0:0`, elapsed `00:00:08`, 2 CPU / 8 GB) performed the successful read-only re-verification. Local compact evidence, independently checksum-verified and untracked per `docs/repo_policy.md`: `.scratch_local/v2_real_disposable_registration_1dc2747_20260830T074704Z/` (22/22 payload files) and a follow-up scheduler/failure-material bundle `.scratch_local/v2_real_disposable_registration_followup_1dc2747_20260830T082056Z/` (14/14 payload files, adding final authoritative `sacct` for both jobs and the preserved raw first-verifier failure material). The follow-up collection briefly used a transient helper script under `$HOME` on Moriah rather than a project-local path; it was removed after use and did not touch Git, credentials, evidence integrity, or user material. Corrective rule: future Flash-NH temporary helpers must be created only in project-local `tmp/` or `.scratch_local/`.

**What did not happen.** No `wandb agent` ran; no W&B run exists in the preserved verification; no proposal was consumed; no manifest `output_root` was created; no training or evaluation occurred; no optimizer objective was published; no production sweep was created. Common-120 runtime non-access is inferred from the recorded descriptor-only registration, preflight, and read-only code paths; it was not syscall-audited. Registration success is **not** CPU-agent-rehearsal success — real CPU agent rehearsal remains a distinct, separately gated later step contingent on inspecting this registration evidence first. Production sweep / production training remains unauthorized.

**Status, precisely.** PREPARATION A — CLOSED. PREPARATION B — CLOSED. LINUX/OFFLINE QUALIFICATION — CLOSED. REAL DISPOSABLE REGISTRATION — CLOSED. DISPOSABLE SWEEP — EXISTS, PENDING, ZERO RUNS AT FINAL PRESERVED CHECK. REAL CPU AGENT REHEARSAL — NOT PERFORMED / SEPARATELY GATED. PROPOSAL CONSUMPTION — NONE. TRAINING/EVALUATION/OBJECTIVE PUBLICATION — NONE. PRODUCTION SWEEP/TRAINING — NOT AUTHORIZED. See `docs/decision_log.md`'s 2026-08-30 entry (top) for the durable operational decision and the scratch-policy corrective rule.
