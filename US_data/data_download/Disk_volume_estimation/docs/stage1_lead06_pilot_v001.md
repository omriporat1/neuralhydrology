...
## Embedding-Dropout-A implementation complete, preparation-only validated, ready for Moriah launch review (2026-08-11)

Implementation task following the design freeze immediately above. Full decision text: `docs/decision_log.md`'s topmost 2026-08-11 entry ("Embedding-Dropout-A implementation and preparation-only validation complete"); technical detail: `docs/stage1_validation_optimization_foundation.md` Part L.18. **Still not launched: no dropout candidate has been trained, no Slurm job submitted.**

**Status table (updated).**

| run_id | embedding_dropout | status |
|---|---|---|
| `emb128x32_seedA_drop00_h128_lr3em4_cap25k_cal` | 0.00 | implemented, preparation-only validated, not launched |
| `emb128x32_seedA_drop05_h128_lr3em4_cap25k_cal` | 0.05 | implemented, preparation-only validated, not launched |
| `emb128x32_seedA_drop10_h128_lr3em4_cap25k_cal` | 0.10 | implemented, preparation-only validated, not launched — **fresh, not a reuse** |
| `emb128x32_seedA_drop20_h128_lr3em4_cap25k_cal` | 0.20 | implemented, preparation-only validated, not launched |
| `emb128x32_seedA_drop40_h128_lr3em4_cap25k_cal` | 0.40 | implemented, preparation-only validated, not launched |
| `emb128x32_seedA_h128_lr3em4_cap25k_cal` | 0.10 | **reused historical comparator** — already trained and closed under Hidden-size-A; read-only, reachable only via `--status-only`, never a sixth campaign member |

**Implementation complete, with one deliberate, reviewed deviation from the design freeze's wording.** All 8 items from the design freeze's minimum implementation plan are now built: `PilotRunSpec.embedding_dropout` override field; `load_pilot_policy()`'s **per-profile** `statics_embedding.dropout` `0.1` gate made override-aware — the **top-level, policy-wide `embedding_dropout: 0.1` gate is deliberately left strict and unchanged**, not loosened as the design freeze's plan wording literally described. This is safe and requires no change because Embedding-Dropout-A's five new `PilotRunSpec`s are spliced into the already-loaded/validated base policy in memory (the campaign never edits or reloads the committed policy file), so explicit candidate variation reaches the generated config only through `PilotRunSpec.embedding_dropout` → the per-profile gate → `build_nh_config_mapping()`'s post-merge override, while 100% of existing non-overridden top-level-gate behavior is preserved. Pre-existing test `test_load_pilot_policy_rejects_wrong_top_level_embedding_dropout` (untouched by this task) already proves the top-level gate still rejects policy-wide drift away from `0.1`. `validate_embedding_dropout_override()` + `build_nh_config_mapping()` post-profile-merge threading (`[0,1)` bound, `0.00` recorded as an explicit `0.0`, never confused with "no override"); manifest/run-identity provenance (`embedding_dropout_override`/`resolved_embedding_dropout`); `enforce_pilot_embedding_dropout_identity()` continuation-safety guard mirroring `enforce_pilot_hidden_size_identity()`; `scripts/run_stage1_embedding_dropout_range_seedA_closure.py` + `..._moriah.sbatch` (`EMBEDDING_DROPOUT_A_MAX_TARGET_EPOCH=6` fixed, exactly five trainable run_ids, historical H=128 comparator reachable only via `--status-only`, default offline-enabled W&B policy); tests across all 8 planned categories.

**Preparation-only validation (real, unmocked, two independent layers).** (a) The pytest suite's real `prepare_pilot_run_only()` calls for all five candidates against a synthetic package covering the actual full 2,557-basin development/spatial-holdout union and the real committed policy/split files — confirms pairwise config diffs limited to `experiment_name`/basin-list paths/`run_dir`/`statics_embedding`, `data_dir` and basin-list contents identical, every run identity pairwise-unique, `training_started`/`evaluation_started`/`wandb_backend_initialized` all `False`. (b) A standalone, non-pytest audit script additionally invoked the real closure-launcher CLI as a real subprocess with `--prepare-only` for all five run_ids, writing real `config.yaml`/`generation_manifest.json` files to disk. All five returned `status=PREPARED_ONLY`; `drop00`/`drop40` spot-checked directly: `statics_embedding.dropout` resolved to exactly `0.0`/`0.4`, with `hidden_size=128`, `learning_rate=0.0003`, `output_dropout=0.25`, `seed=967139`, `seq_length=24` identical between them — direct evidence the frozen invariants hold and only `statics_embedding.dropout`/provenance vary.

**Tests.** `test_nh_config_generation.py` (154), `test_pilot_tracking.py` (42), `test_pilot_orchestration.py` (102, 5 skipped), `test_pilot_lead06_config.py` (52), plus `test_run_stage1_embedding_dropout_range_seedA_closure_cli.py`, `test_embedding_dropout_range_seedA_closure_sbatch_launcher.py`, `test_embedding_dropout_range_seedA_closure_preparation.py` (130 combined) — all passing, plus a clean combined regression (2070 passed/5 skipped/1 pre-existing unrelated Windows file-locking flake) against the full local suite excluding 6 pre-existing torch/neuralhydrology-dependent collection-error files.

**Not done by this entry.** No embedding-dropout candidate launched, no Slurm job submitted, no real NeuralHydrology training or checkpoint evaluation call, no W&B Sweep, no scientific-design change, no hydrograph panel rendered, no reproducibility comparison against the historical H=128/dropout=0.10 run performed, nothing committed automatically.

## Embedding-Dropout-A closed: weak sensitivity over `0.00`-`0.40`, `drop10` retained as provisional anchor, hydrograph sanity check clean, revised Phase-A/Phase-B roadmap adopted (2026-08-13)

Documentation-only closure task following the implementation task immediately above. Full decision text: `docs/decision_log.md`'s 2026-08-13 entry (topmost); technical detail: `docs/stage1_validation_optimization_foundation.md` Part L.19.

**Status table (final).**

| run_id | embedding_dropout | status |
|---|---|---|
| `emb128x32_seedA_drop00_h128_lr3em4_cap25k_cal` | 0.00 | complete, closed — epoch-6 endpoint leader |
| `emb128x32_seedA_drop05_h128_lr3em4_cap25k_cal` | 0.05 | complete, closed |
| `emb128x32_seedA_drop10_h128_lr3em4_cap25k_cal` | 0.10 | complete, closed — **provisional working anchor**, strongest late-window summary, best single observed checkpoint, exact/deterministic reproduction of the historical comparator |
| `emb128x32_seedA_drop20_h128_lr3em4_cap25k_cal` | 0.20 | complete, closed — among the most stable late-window candidates |
| `emb128x32_seedA_drop40_h128_lr3em4_cap25k_cal` | 0.40 | complete, closed — no validation-performance cliff at this fidelity |
| `emb128x32_seedA_h128_lr3em4_cap25k_cal` | 0.10 | historical reproducibility comparator (L.15/L.16) — read-only, never a sixth campaign member; confirmed exact/deterministic reproduction against fresh `drop10` |

**Result.** All five candidates completed six-epoch/25k-cap training on Moriah (`drop10` job 45789423, `drop00` job 45790661, `drop05` job 45790662, `drop20` job 45790663, `drop40` job 45790664) and were evaluated at all six epochs via retrospective diagnostic-evaluation jobs 45790996-45791000; optimizer/update verification and paired-comparison job 45791007. Embedding dropout is weakly sensitive over the tested range — no candidate robustly dominates across epoch-6 median NSE, epochs-4-6 late-window behavior, matched-basin paired differences, or the hydrograph review; differences are comparable to ordinary epoch-to-epoch variation. Higher dropout raises transformed-space training loss monotonically without a matching monotonic raw-space validation-NSE relationship.

**Hydrograph sanity check.** The frozen `phase_a_validation_hydrograph_panel_v001` 8-basin panel (same basins/windows/scales as the Hidden-size-A review) was rendered for `drop00` (epoch 6), `drop10` (epoch 5, its own best observed checkpoint), and `drop20` (epoch 6) — Moriah job 45791211, evidence `tmp/embdrop_sanity_panels_v001_evidence/drop00_drop10_drop20_v001/`, 48/48 files checksum-verified, untracked. Broad similarity across most basins; basin/event-specific divergences at 07261000 (`drop20` false peak), 08072300 (`drop10` false peak), 14301500 (`drop10` damped peak) — each implicating a different candidate in a different direction, judged basin/event-specific, not a repeated candidate-specific pathology. Does not contradict the aggregate quantitative near-tie.

**Provisional anchor.** `embedding_dropout=0.10` remains the provisional working anchor — not because it was proven optimal, but because it sits safely inside the broad viable tested region, has a strong sustained late-window result, and exactly reproduces the nominally equivalent historical run. Final selection deferred to Phase B joint HPO; the tested `0.00`-`0.40` region remains broadly viable and must not be aggressively narrowed by this result.

**Revised roadmap (adopted; supersedes this document's own sequence-length-framing passage above — see the forward-pointing note added there, preserved as historical).** (1) Reusable Phase-A/HPO campaign infrastructure consolidation, including a new durable artifact/evidence self-identification requirement (generic hydrograph-evidence filenames/titles become ambiguous once separated from their parent directory — not a defect requiring regeneration of existing evidence). (2) Sequence-Length-A: `seq_length={12,24,48,72}` at the best-supported anchor, now a bounded/calibratable structural parameter rather than permanently fixed. (3) Dynamic-input family characterization. (4) Phase B joint HPO.
...
## Sequence-Length-A closed: `seq_length=72` adopted as provisional working anchor, `seq_length=48` nearest alternative, hydrograph sanity check clean, dynamic-input family characterization next (2026-08-15)

Documentation-only closure task recording the completed campaign. Full decision text: `docs/decision_log.md`'s 2026-08-15 entry (topmost); technical detail: `docs/stage1_validation_optimization_foundation.md` Part L.20.

**Status table (final).**

| run_id | seq_length | status |
|---|---|---|
| `stage1_lead06_pilot_emb128x32_seedA_seq12_h128_lr3em4_cap25k_cal_v001` | 12 | complete, closed |
| `stage1_lead06_pilot_emb128x32_seedA_seq24_h128_lr3em4_cap25k_cal_v001` | 24 | complete, closed — best observed checkpoint epoch 5 |
| `stage1_lead06_pilot_emb128x32_seedA_seq48_h128_lr3em4_cap25k_cal_v001` | 48 | complete, closed — nearest credible alternative; best observed checkpoint epoch 6 |
| `stage1_lead06_pilot_emb128x32_seedA_seq72_h128_lr3em4_cap25k_cal_v001` | 72 | complete, closed — **provisional working anchor**; best observed checkpoint epoch 5 |

All four share the Embedding-Dropout-A-closed contract: Seed A (967139), `[128,32]` learned static embedding (tanh activation, embedding dropout 0.10), `hidden_size=128`, `learning_rate=3e-4`, `output_dropout=0.25`, lead 6h, target `qobs_mm_per_h_lead06`, `max_updates_per_epoch=25000`, six epochs, one uninterrupted segment, the fixed development-training population, the fixed ~400-basin development-validation screening population, strict offline W&B throughout. Campaign infrastructure (`campaign_registry.py`, `campaign_spec.py`, the closure-splice launcher/sbatch pair, and `common_support_audit.py`'s scientific groundwork) committed `4646a55`; training jobs `45861222`-`45861225` (seq12/24/48/72). All four completed cleanly, no resource failures, approximately comparable runtime across candidates.

**Result.** Natural-support raw-space median NSE, a common-support-corrected evaluation, true per-basin paired comparisons, and late-window behavior all show the same ordering at every evaluated epoch: `seq72 > seq48 > seq24 > seq12`. Transformed-space training loss corroborates but is diagnostic-only. The common-support correction (`src/baseline/common_support_audit.py`, restricting comparison to `(basin, timestamp)` positions admitted by every candidate simultaneously) did not materially change the ranking.

**Hydrograph sanity check.** The frozen `phase_a_validation_hydrograph_panel_v001` 8-basin panel (same basins/scales as the Hidden-size-A/Embedding-Dropout-A reviews) was rendered for `seq24` (epoch 5), `seq48` (epoch 6), `seq72` (epoch 5) — each candidate's own best observed checkpoint — with the displayed antecedent window widened from ~24h to 72h (`derive_display_window()`, presentational only; the frozen event/peak identity itself was never re-selected). Evidence: `tmp/sequence_length_a_hydrograph_sanity_v001/frozen_panel/`, 14/14 files checksum-verified, untracked. **Result: CONSISTENT with the quantitative ranking — no repeated `seq72`-specific pathology.** A supplemental single-basin diagnostic (`tmp/sequence_length_a_hydrograph_sanity_v001/basin_06131200_diagnostic/`, 7/7 files checksum-verified) confirmed basin `06131200`'s severe failure at its May 2024 extreme event is shared identically across all three rendered sequence lengths (NSE −6.6 to −52.3) — a near-zero-flow basin/model pathology and NSE-denominator sensitivity, not a `seq72`-specific defect.

**Decision.** `seq_length=72` becomes the provisional Stage-1 working anchor within the tested 12-72h range — not because 72h was proven optimal, but because it is the strongest-performing tested value and the hydrograph review found nothing to contradict it. `seq_length=48` remains the nearest credible alternative. Performance had not clearly saturated by 72h; the upper-bound question is deferred, not resolved. Final sequence-length selection remains deferred to Phase B joint HPO.

**Comparative-hydrograph convention.** See `docs/stage1_validation_optimization_foundation.md` Part L.20 for the newly adopted convention (same frozen event/window/scale, all candidates overlaid on one panel for small candidate sets; only the *displayed* antecedent window may widen, and only for experiments that specifically vary historical context).
