...
Before scientific sweep launch, complete online W&B qualification on a CPU Slurm allocation—not the login node—using the exact runtime and a trivial toy/no-training objective. It must create/use a small real online sweep, receive a proposal, log a metric online, obtain a subsequent proposal, avoid silent offline fallback, and keep credentials out of logs/evidence. No GPU science smoke is required solely for this qualification. Note that this in-process (`wandb.agent(function=...)`) qualification does not by itself exercise the CLI-subprocess agent's launch-command/argv mechanics below; it qualifies the online sweep/run/proposal lifecycle, not the OS-level command contract.

### Launch-command operational contract

The production sweep config's `command` is explicit (`["${interpreter}", "${program}"]`), never the W&B default that would append the five swept hyperparameters as `--key=value` CLI flags to `scripts/run_sweep_v1_wandb_bridge.py` (its argparse does not accept them; it reads proposed values from `run.config`). Because a real `wandb agent` — not the sbatch launcher — constructs the bridge's argv directly from that template, the launcher cannot inject `--flag value` arguments the way the non-W&B random-control launcher does. Instead, `scripts/run_sweep_v1_wandb_agent_moriah.sbatch` requires and exports four `FLASHNH_SWEEP_V1_*` environment variables (`PACKAGE_ROOT`, `SCREENING_BASIN_IDS`, `OUTPUT_ROOT`, `PROPOSAL_ORDER`) into its own shell environment, which `wandb agent` and every child process it spawns inherit by standard OS subprocess semantics. The bridge resolves each of its four operational inputs from either the corresponding CLI flag (kept for direct/manual/test invocation) or its environment variable, with strict precedence: agreement between both is accepted, exactly one supplied is accepted, a contradiction or the absence of both is a hard error before any proposal intake. Proposal order is never inferred from W&B run count, directory enumeration, or a prior qualification sweep — it is a required, explicit, positive-integer value the launcher freezes per allocation (`1` for the first real production attempt). `scripts/build_sweep_v1_production_sweep_config.py --output` refuses to overwrite an existing file unless `--force` is passed.

### Exact-retry launch contract

An INVALID attempt may be retried with its EXACT ORIGINAL five hyperparameters (never resampled or hand-edited) via the dedicated operator-run bridge `scripts/run_sweep_v1_exact_retry_bridge.py` plus `src/baseline/sweep_v1_retry.py`. This path is manually invoked per retry; it never calls `wandb.agent()` and never requests a new Bayesian proposal. It: (1) loads the original attempt's own frozen `execution_provenance.json` and independently re-derives `configuration_id`/`proposal_id`/`trial_id` from its own axes rather than trusting persisted ids; (2) cross-checks the result against an operator-authored, explicitly reviewed pinned-identity file, refusing on any contradiction; (3) derives a retry identity with identical hyperparameters/`configuration_id`/`proposal_id`, a strictly later `execution_generation`, a fresh `trial_id`, and `retry_of_trial_id` pointing at the original; (4) refuses before any W&B call if the derived retry output directory already exists, so the original attempt's directory is never opened for writing and a re-invocation can never overwrite or duplicate an attempt; (5) joins the run to the existing production sweep via `wandb.init(settings=wandb.Settings(sweep_id=...))`, not `wandb.agent()`; (6) logs a finite objective only if the retry is VALID, and always records `retry_of_trial_id`/`execution_generation`/`flashnh/exact_retry=True` in run summary and provenance.

The join mechanism in step (5) was empirically qualified, not assumed, on a disposable non-scientific W&B sweep (Moriah job 45939203, sweep `bn367mj8`, real wandb 0.28.1 online, `all_checks_passed: true` across 16 checks): a manually joined run (no `wandb.agent()`) is incorporated into the sweep's run population and visible to the same `wandb.Api()` tooling the controller/reviewer would use; no additional controller-issued proposal is consumed by a manual join; a VALID retry's objective is visible while an INVALID retry's objective is correctly absent, never fabricated; the Bayesian controller continues to serve a legitimate next real proposal after manually joined runs are mixed into its history; and nothing about the three prior runs' config/summary is mutated after the fact. Evidence: `/sci/labs/efratmorin/omripo/Flash-NH/evidence/wandb_exact_retry_join_qualification_v001/run_45939203/` (project-local, untracked, per repo policy).

This qualifies the retry-join mechanism only. Launching a real production attempt002 against sweep `4x3btz2s` still requires an explicit, separate authorization step; qualification of the mechanism is not itself that authorization.
