"""Section 14 (preparation-only validation) for the LR-A learning-rate
range-characterization campaign (``lr_range_seedA_25k_v001``).

For each of the four NEW, approved LR-A candidates
(``LR_A_RUN_SPECS`` in scripts/run_stage1_lr_range_seedA_closure.py), calls
the REAL, unmocked ``prepare_pilot_run_only`` -- against a real synthetic
package covering the actual full 2,557-basin development/spatial-holdout
union (``tests._pilot_support.build_full_union_package``) and the real
committed pilot/baseline policies and split files -- then inspects the
generated ``config.yaml``/``generation_manifest.json`` to confirm:

  * architecture/target/lead/seq_length/embedding-shape/dropout/seed/cap
    invariants match the frozen LR-A contract (Section 1);
  * each candidate's ``learning_rate`` is exactly its assigned LR-A value,
    with explicit provenance in its generation manifest
    (``learning_rate_override``/``resolved_learning_rate``);
  * no candidate's generated config or manifest differs from its siblings
    in anything other than ``learning_rate`` plus unavoidable identity/path
...
"structural-comparison-only" PilotRunSpec (learning_rate=1e-3, a run_id
distinct from and never equal to REFERENCE_RUN_ID) through the exact same
real generation pathway, solely to confirm the code path injects
learning_rate as the only scientific difference between sibling candidates.
This makes no claim about reproducing the real, historical, Moriah-trained
emb128x32_seedA_cap25k_cal reference itself (see Section 3's closure and its
one flagged-not-blocking caveat in docs/decision_log.md) -- that reference's
own PilotRunSpec/generation manifest was never committed to this repository
and is not accessible from this local/text-only session.
...
_SEEDA = 967139
_CAP25K = 25_000
_EXPECTED_LR_A_LEARNING_RATES = {
    "emb128x32_seedA_lr1em4_cap25k_cal": 1e-4,
    "emb128x32_seedA_lr3em4_cap25k_cal": 3e-4,
    "emb128x32_seedA_lr3em3_cap25k_cal": 3e-3,
    "emb128x32_seedA_lr1em2_cap25k_cal": 1e-2,
}
...
# artifact_sha256 necessarily changes whenever learning_rate/run_id do.
_ALLOWED_DIFFERING_MANIFEST_KEYS = {
    "learning_rate_override", "resolved_learning_rate", "artifact_sha256",
...
_STRUCTURAL_COMPARISON_RUN_ID = "emb128x32_seedA_lr1em3_structural_comparison_only"


def _load_cli_module():
    spec = importlib.util.spec_from_file_location(
        "run_stage1_lr_range_seedA_closure_preparation_under_test", SCRIPT_PATH
    )
...
def package_root(tmp_path_factory):
    root = tmp_path_factory.mktemp("lr_a_prep_package") / "package"
    build_full_union_package(root)
    return root


def _prepare(cli_module, lr_a_policy, package_root, run_id, out_root):
...
@pytest.fixture(scope="module")
def prepared_candidates(cli_module, lr_a_policy, package_root, tmp_path_factory):
    out_root = tmp_path_factory.mktemp("lr_a_prep_runs")
    prepared = {}
    for run_id in sorted(cli_module.LR_A_RUN_SPECS):
        result, config_out_dir = _prepare(cli_module, lr_a_policy, package_root, run_id, out_root)
...
# --- per-candidate invariants (Section 1's frozen contract) -----------------
...
@pytest.mark.parametrize("run_id", sorted(_EXPECTED_LR_A_LEARNING_RATES))
def test_candidate_config_matches_frozen_lr_a_contract(prepared_candidates, run_id):
    ...
    assert config["learning_rate"] == pytest.approx(_EXPECTED_LR_A_LEARNING_RATES[run_id])


@pytest.mark.parametrize("run_id", sorted(_EXPECTED_LR_A_LEARNING_RATES))
def test_candidate_manifest_records_explicit_lr_provenance(prepared_candidates, run_id):
    manifest = prepared_candidates[run_id]["manifest"]
    expected_lr = _EXPECTED_LR_A_LEARNING_RATES[run_id]
    assert manifest["learning_rate_override"] == pytest.approx(expected_lr)
    assert manifest["resolved_learning_rate"] == pytest.approx(expected_lr)
    assert manifest["max_updates_per_epoch"] == _CAP25K


# --- cross-candidate: only learning_rate (+ identity/path) may differ ------


def test_configs_differ_only_in_learning_rate_and_identity_metadata(prepared_candidates):
...
def test_manifests_differ_only_in_lr_provenance_and_identity_metadata(prepared_candidates):
    run_ids = sorted(prepared_candidates)
    reference_run_id = run_ids[0]
...
def test_basin_list_file_contents_are_identical_across_all_candidates(prepared_candidates):
    ...
    for run_id in run_ids[1:]:
        ...
        for key, expected_text in reference_contents.items():
            ...
            # And the paths themselves must actually be distinct per candidate
            # (identity metadata is expected to differ, just not content).
...
def test_locally_synthesized_lr1em3_candidate_differs_from_siblings_only_in_lr(
    cli_module, lr_a_policy, package_root, tmp_path_factory
):
    ...
    augmented_runs = dict(lr_a_policy.runs)
    assert structural_spec.run_id not in augmented_runs
    augmented_runs[structural_spec.run_id] = structural_spec
    augmented_policy = dataclasses.replace(lr_a_policy, runs=augmented_runs)

    out_root = tmp_path_factory.mktemp("lr_a_structural_comparison")
    ...
    another_run_id = sorted(_EXPECTED_LR_A_LEARNING_RATES)[0]
    another_result, another_config_out_dir = _prepare(
        cli_module, lr_a_policy, package_root, another_run_id, out_root
    )
...
def test_reference_run_id_cannot_be_prepared_through_prepare_pilot_run_only(
    ...
    out_root = tmp_path_factory.mktemp("lr_a_reference_guard")
    with pytest.raises(Exception):
        _prepare(cli_module, lr_a_policy, package_root, cli_module.REFERENCE_RUN_ID, out_root)
