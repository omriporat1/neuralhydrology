"""Focused tests for
scripts/run_stage1_embedding_dropout_range_seedA_closure_moriah.sbatch -- the
Embedding-Dropout-A embedding-dropout range-characterization campaign Slurm
launcher.
...
tests/test_hidden_size_range_seedA_closure_sbatch_launcher.py.
"""
...
SBATCH_PATH = REPO_ROOT / "scripts" / "run_stage1_embedding_dropout_range_seedA_closure_moriah.sbatch"
...
ALL_FIVE = [DROP00, DROP05, DROP10, DROP20, DROP40, REFERENCE]
...
def test_sbatch_script_delegates_to_embedding_dropout_closure_cli_not_other_pilot_clis():
    text = _text()
    assert "python scripts/run_stage1_embedding_dropout_range_seedA_closure.py" in text
    code_lines = [
...
# --- (6) exact allowlist (five trainable + one status-only-only) -----------

def test_sbatch_script_allowlist_is_exactly_the_five_embedding_dropout_a_run_ids():
    text = _text()
    assert f"{DROP00}|{DROP05}|{DROP10}|{DROP20}|{DROP40}|{REFERENCE}" in text
...
@pytest.mark.parametrize("bad_run_id", [
    "raw_seedA", "emb128x64_seedA", "emb128x32_seedA_drop15_h128_lr3em4_cap25k_cal",
...
def test_reference_guard_block_never_blocks_the_five_real_candidates(tmp_path, run_id):
    script_path = tmp_path / "reference_guard.sh"
    script_path.write_text(
        "#!/usr/bin/env bash\nset -uo pipefail\n"
        f'RUN_ID="{run_id}"\n'
...
# --- absence of --force / wandb sync / EMBEDDING_DROPOUT|HIDDEN_SIZE|
...
def test_sbatch_script_never_defines_an_embedding_dropout_hidden_size_or_learning_rate_override_knob():
    code_lines = [
        line for line in _text().splitlines() if line.strip() and not line.strip().startswith("#")
    ]
    ...
    assert not any(line.strip().startswith("EMBEDDING_DROPOUT=") for line in code_lines)
    assert not any("--embedding-dropout" in line for line in code_lines)
...
def test_wandb_policy_path_default_is_unconditional_not_empty_string_gated():
    ...
    # (empty-string default, conditionally applied later), Embedding-Dropout-A's
...
# --- epoch-6 Embedding-Dropout-A horizon: fixed, non-overridable -----------

def test_sbatch_script_defines_fixed_embedding_dropout_a_max_target_epoch_constant():
    text = _text()
    assert "EMBEDDING_DROPOUT_A_MAX_TARGET_EPOCH=6" in text
    code_lines = [
        line for line in text.splitlines() if line.strip() and not line.strip().startswith("#")
    ]
...
def test_sbatch_script_echoes_embedding_dropout_a_horizon_in_always_printed_info_block():
    text = _text()
    echo_idx = text.index(
        'echo "Approved Embedding-Dropout-A horizon (fixed, non-overridable): epoch ${EMBEDDING_DROPOUT_A_MAX_TARGET_EPOCH}"'
    )
    prepare_only_branch_idx = text.index('if [ "${PREPARE_ONLY}" = "1" ] || [ "${STATUS_ONLY}" = "1" ]; then')
