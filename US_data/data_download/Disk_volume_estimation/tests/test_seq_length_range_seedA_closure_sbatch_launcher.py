"""Focused tests for
scripts/run_stage1_seq_length_range_seedA_closure_moriah.sbatch -- the
Sequence-Length-A sequence-length range-characterization campaign Slurm
launcher.
...
tests/test_embedding_dropout_range_seedA_closure_sbatch_launcher.py.
"""
...
SBATCH_PATH = REPO_ROOT / "scripts" / "run_stage1_seq_length_range_seedA_closure_moriah.sbatch"

SEQ12 = "emb128x32_seedA_seq12_h128_lr3em4_cap25k_cal"
SEQ24 = "emb128x32_seedA_seq24_h128_lr3em4_cap25k_cal"
SEQ48 = "emb128x32_seedA_seq48_h128_lr3em4_cap25k_cal"
SEQ72 = "emb128x32_seedA_seq72_h128_lr3em4_cap25k_cal"
REFERENCE = "emb128x32_seedA_h128_lr3em4_cap25k_cal"
ALL_FIVE = [SEQ12, SEQ24, SEQ48, SEQ72, REFERENCE]
...
def test_sbatch_script_delegates_to_seq_length_closure_cli_not_other_pilot_clis():
    text = _text()
    assert "python scripts/run_stage1_seq_length_range_seedA_closure.py" in text
    code_lines = [
    ...
    assert not any("run_stage1_hidden_size_range_seedA_closure.py" in line for line in code_lines)
    assert not any("run_stage1_embedding_dropout_range_seedA_closure.py" in line for line in code_lines)


# --- exact allowlist (four trainable + one status-only-only) ---------------

def test_sbatch_script_allowlist_is_exactly_the_four_seq_length_a_run_ids():
    text = _text()
    assert f"{SEQ12}|{SEQ24}|{SEQ48}|{SEQ72}|{REFERENCE}" in text
    assert "raw_seedA" not in text
    assert "emb128x64_seedA" not in text
    assert "emb128x32_seedA_drop00_h128_lr3em4_cap25k_cal" not in text
    assert "emb128x32_seedA_h64_lr3em4_cap25k_cal" not in text
    assert "emb128x32_seedA_h256_lr3em4_cap25k_cal" not in text
...
def test_run_id_case_block_accepts_all_approved_run_ids(tmp_path, run_id):
...
@pytest.mark.parametrize("bad_run_id", [
    "raw_seedA", "emb128x64_seedA", "emb128x32_seedA_seq36_h128_lr3em4_cap25k_cal",
    "emb128x32_seedA_h64_lr3em4_cap25k_cal", "not_a_real_run_id", "",
])
...
@pytest.mark.skipif(not _BASH_AVAILABLE, reason="bash not available in this environment")
@pytest.mark.parametrize("run_id", [SEQ12, SEQ24, SEQ48, SEQ72])
...
# --- absence of --force / wandb sync / SEQ_LENGTH|HIDDEN_SIZE|LEARNING_RATE
# --- override ----------------------------------------------------------------
...
def test_sbatch_script_never_defines_a_seq_length_hidden_size_or_learning_rate_override_knob():
    code_lines = [
        line for line in _text().splitlines() if line.strip() and not line.strip().startswith("#")
    ]
    ...
    assert not any(line.strip().startswith("SEQ_LENGTH=") for line in code_lines)
    assert not any("--seq-length" in line for line in code_lines)
...
def test_wandb_policy_path_default_is_unconditional_not_empty_string_gated():
    ...
    # (empty-string default, conditionally applied later), Sequence-Length-A's
...
# --- epoch-6 Sequence-Length-A horizon: fixed, non-overridable -------------

def test_sbatch_script_defines_fixed_seq_length_a_max_target_epoch_constant():
    text = _text()
    assert "SEQ_LENGTH_A_MAX_TARGET_EPOCH=6" in text
    code_lines = [
        line for line in text.splitlines() if line.strip() and not line.strip().startswith("#")
    ]
...
def test_sbatch_script_echoes_seq_length_a_horizon_in_always_printed_info_block():
    text = _text()
    echo_idx = text.index(
        'echo "Approved Sequence-Length-A horizon (fixed, non-overridable): epoch ${SEQ_LENGTH_A_MAX_TARGET_EPOCH}"'
    )
    prepare_only_branch_idx = text.index('if [ "${PREPARE_ONLY}" = "1" ] || [ "${STATUS_ONLY}" = "1" ]; then')
...
# --- Sequence-Length-A-specific: RESULT_DIR namespaced distinctly from -----
# --- Embedding-Dropout-A's (no cross-campaign evidence-directory collision) -

def test_result_dir_is_namespaced_for_seq_length_range_not_embedding_dropout():
    text = _text()
    assert 'RESULT_DIR="${FLASHNH_BASE}/evidence/seq_length_range_closure_${RUN_ID}_${SLURM_JOB_ID}"' in text
    assert "embedding_dropout_range_closure_" not in text
